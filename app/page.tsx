...
export default function ShopsFinal(){
  ...
  const placeOrder = async ()=>{ if(selectedShop?.vip_active===false) return; if(!selectedShop || cart.length===0 ||!customerAddr.trim()) return; const items=cart.map(c=>({id:c.product.id,name:c.product.name,price:c.product.price,qty:c.qty})); const total=getCartTotal(); await supabase.from('orders').insert({shop_id:selectedShop.id,customer_phone:currentUser.phone,customer_name:`${currentUser.firstName} ${currentUser.lastName}`,address:customerAddr,items,total,status:'new',driver_requested:false}); setCart([]); setCustomerAddr(''); await loadCustomerOrders(); };
  ...
  // Функции които ползват CALCULATIONS - след тях
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            ...
            {availableDriverOrders.length>1 && sortedOrders.length===0 && (
              <button onClick={()=>sortAndNavigateToNearest(false)} disabled={isLocating||isSorting} style={{width:'100%',background:'#FFD60A',color:'black',border:'3px solid black',padding:'14px',borderRadius:'14px',fontWeight:'900',fontSize:'14px'}}>
                {isLocating? '📍 Взимам GPS...' : isSorting? '⏳ Сортирам...' : `🤖 Автоматично подреди ${availableDriverOrders.length} поръчки по близост (като Uber)`}
              </button>
            )}
                  ...
                  return (
                    <>
                      <div style={{background:'black',color:'#FFD60A',padding:'12px',borderRadius:'12px',border:'2px solid #FFD60A'}}>
                        ...
                        {activeOrder._dist!==undefined && <div style={{fontSize:'11px',marginTop:'4px'}}>📍 Най-близък е автоматично избран - {activeOrder._dist.toFixed(1)}км</div>}
                        {queue.length>0 && <div style={{fontSize:'10px',marginTop:'4px',opacity:0.8}}>Следва: {queue.map((o:any)=>`#${o.id.slice(0,6)} (${o._dist!==undefined? o._dist.toFixed(1)+'км' : ''})`).join(' → ')}</div>}
                      </div>
                      ...
                      <div style={{background:'white',border:'4px solid black',borderRadius:'16px',padding:'14px'}}>
                        ...
                        <div style={{background:'#e0f2fe',padding:'10px',borderRadius:'10px',marginTop:'10px'}}><b>🏪 {shop?.name} {shop?.vip_active===false? '🔴' : '🟢 Отворен'}</b><br/>📍 {shop?.address}</div>
                        <div style={{background:'#fff3cd',padding:'10px',borderRadius:'10px',marginTop:'8px'}}><b>👤 {activeOrder.customer_name}</b> • {activeOrder.customer_phone}<br/>📍 {activeOrder.address}</div>

                        <div style={{marginTop:'12px'}}>
                          {(activeOrder.status==='ready_for_driver' || activeOrder.status==='on_the_way_to_shop') && (
                            <>
                              <button onClick={()=>openNavigation(shop?.address||'')} style={{width:'100%',background:'#0F4C75',color:'white',border:'none',padding:'14px',borderRadius:'12px',fontWeight:'800'}}>🗺️ До магазина {shop?.name}</button>
                              <button onClick={()=>updateOrderStatus(activeOrder.id,'picked_up')} style={{width:'100%',marginTop:'8px',background:'#8b5cf6',color:'white',border:'none',padding:'14px',borderRadius:'12px',fontWeight:'900'}}>📦 Взех от магазина - Готов за клиент #{activeOrder.id.slice(0,6)}</button>
                            </>
                          )}
                          {activeOrder.status==='picked_up' && (
                            <button onClick={async()=>{ await updateOrderStatus(activeOrder.id,'on_the_way'); openNavigation(activeOrder.address); }} style={{width:'100%',background:'black',color:'#FFD60A',border:'none',padding:'18px',borderRadius:'14px',fontWeight:'900',fontSize:'18px'}}>
                              🧭 Започни пътуване до клиента #{activeOrder.id.slice(0,6)}
                            </button>
                          )}
                          {activeOrder.status==='on_the_way' && (
                            <button onClick={async()=>{
                              ...
                              ✅ Доставена - {queue.length>0? `Продължи към следващ клиент #${queue[0].id.slice(0,6)}` : 'Край на доставките'}
                            </button>
                          )}
                          <div style={{display:'flex',gap:'6px',marginTop:'8px'}}>
                            <button onClick={()=>openNavigation(activeOrder.address)} style={{flex:1,background:'white',border:'2px solid black',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>🧭 До клиента</button>
                      ...
                      {queue.length>0 && (
                        <div style={{background:'#f9fafb',border:'2px dashed #666',borderRadius:'12px',padding:'10px'}}>
                          ...
                          {queue.map((o:any,i:number)=>(
                            <div key={o.id} style={{display:'flex',justifyContent:'space-between',background:'white',padding:'8px',borderRadius:'8px',marginTop:'6px'}}>
                              <span style={{fontSize:'11px'}}>{i+2}. #{o.id.slice(0,6)} • {o.customer_name}</span>
                              <span style={{fontSize:'10px',background:'#f3f4f6',padding:'4px 6px',borderRadius:'6px'}}>{o._dist!==undefined? `${o._dist.toFixed(1)}км` : ''}</span>
                            </div>
                          ))}
                          <div style={{fontSize:'10px',color:'#666',marginTop:'6px',textAlign:'center'}}>След доставката автоматично тръгваш към следващия най-близък</div>
                        </div>
                      )}
                    </>
                  );
            ...
            {availableDriverOrders.length===0 && <div style={{background:'white',padding:'20px',borderRadius:'12px',textAlign:'center',color:'#666'}}>Няма активни поръчки • Днес: {todayEarnings.toFixed(2)}€ от {todayDeliveredOrders.length}</div>}

            {availableDriverOrders.length===1 && sortedOrders.length===0 && (
              <div style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}>
                <b>Само 1 поръчка - няма нужда от сортиране</b>
                <div style={{fontSize:'11px',marginTop:'6px'}}>#{availableDriverOrders[0].id.slice(0,8)} • {availableDriverOrders[0].address}</div>
                <button onClick={()=>assignDriverToOrder(availableDriverOrders[0].id)} style={{width:'100%',marginTop:'8px',background:'#0F4C75',color:'white',border:'none',padding:'12px',borderRadius:'10px'}}>🚚 Вземи</button>
              </div>
            )}
          </div>
        )}
