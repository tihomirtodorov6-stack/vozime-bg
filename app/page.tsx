...
export default function Home(){
  ...
  const loadMyShopProfile = async ()=>{
    ...
    for(const sid of shopIds){
      const {data:prods, error:pErr}=await supabase.from('products').select('*').eq('shop_id', sid).order('created_at',{ascending:false});
  ...
  const loadMarketProducts = async (shopId:string)=>{ const {data, error}=await supabase.from('products').select('*').eq('shop_id', shopId).order('active',{ascending:false}).order('created_at',{ascending:false}); if(error){ alert('Грешка зареждане продукти: '+error.message); return; } if(data) setMarketProducts(data.filter((x:any)=> x.active!==false)); setSelectedMarketShop(marketShops.find(s=>s.id===shopId)); setMarketSearch(''); setMarketCat('Всички'); setCart([]); setCartOpen(false); };
        ...
        {tab==='my' && <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>
          {myShopProfile && myShopProfile.role==='shop_owner' && !isAdmin && (
            <div style={{background:'white',border:'3px solid #22c55e',borderRadius:'14px',padding:'14px'}}>
              ...
              <div style={{marginTop:'12px',background:'#e6f9ed',padding:'10px',borderRadius:'8px'}}><b>Поръчки - {myShopOrders.length}</b>{myShopOrders.map((o:any)=><div key={o.id} style={{background:'white',padding:'8px',borderRadius:'6px',marginTop:'6px',fontSize:'11px'}}><b>{o.customer_name} - {o.customer_phone}</b><br/>{o.address}<br/>Тотал: {o.total}€ - {o.status} <button onClick={()=>updateOrderStatus(o.id,'accepted')} style={{marginLeft:'6px',background:'#22c55e',color:'white',border:'none',padding:'2px 6px',borderRadius:'4px',fontSize:'10px'}}>Приеми</button></div>)}</div>
            </div>
          )}
          ...
          <div style={{background:'white',border:'1px solid #e5e7eb',borderRadius:'12px',padding:'12px'}}><b>🛒 Моите поръчки като клиент - {customerOrders.length}</b>{customerOrders.map((o:any)=><div key={o.id} style={{background:'#f9fafb',padding:'8px',borderRadius:'6px',marginTop:'6px',fontSize:'11px'}}>{o.shop_id.slice(0,6)} - {o.total}€ - {o.status} - {o.address}</div>)}</div>
        </div>}
      </div>
    </main>
  );
