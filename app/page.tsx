...
export default function ShopsFinal(){
  ...
  const openNavigation = (address:string) => {
    ...
    if(isIOS){
      // iPhone - отваря Apple Maps, ако има Google Maps го предлага също
      window.open(`http://maps.apple.com/?daddr=${q}`, '_blank');
    } else {
      // Android - geo: отваря системен chooser с всички навигации (Google Maps, Waze и т.н.)
      window.location.href = `geo:0,0?q=${q}`;
      // fallback ако браузъра не поддържа geo:
      setTimeout(()=>{
  ...
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'#f9fafb',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      ...
      <div style={{display:'flex',gap:'4px',padding:'6px',background:'white',borderBottom:'1px solid #e5e7eb'}}>
        ...
        {myShopProfile?.role==='driver' && <button onClick={()=>setTab('driver')} style={{flex:1,padding:'10px',borderRadius:'10px',border:'none',fontWeight:'bold',fontSize:'11px',background:tab==='driver'?'#0F4C75':'#f3f4f6',color:tab==='driver'?'white':'#666'}}>🚚 Доставки ({availableDriverOrders.length})</button>}
      ...
      <div style={{flex:1,overflowY:'auto',padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
        ...
        {tab==='my_orders' && (
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}><b>📦 Моите поръчки LIVE - {customerOrders.length}</b></div>
        ...
        {tab==='my_shop' && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#22c55e',color:'white',padding:'14px',borderRadius:'14px'}}><b>🏪 Моят магазин LIVE</b><div style={{fontSize:'11px'}}>{shops.find(s=>s.id===myShopProfile?.shop_id)?.name||''} • {myShopOrders.length} поръчки</div></div>
        ...
        {tab==='driver' && (
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}>
              <div style={{display:'flex',justifyContent:'space-between'}}><b>🚚 Доставки LIVE - {driverOrders.length} чакащи</b><span style={{fontSize:'10px',background:'#22c55e',padding:'4px 8px',borderRadius:'10px'}}>LIVE ✅</span></div>
              <div style={{background:activeDriverCount>=3?'#fee2e2':'#f0fdf4',border:'1px solid #fff',padding:'8px',borderRadius:'8px',marginTop:'8px',color:activeDriverCount>=3?'#991b1b':'#166534'}}><b style={{fontSize:'12px'}}>📦 Лимит: {activeDriverCount}/3</b></div>
              ...
              return (
                <div key={o.id} style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}>
                  ...
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#e0f2fe',padding:'8px',borderRadius:'8px'}}>
                    ...
                    <button onClick={()=>openNavigation(shop?.address || shop?.city || '')} style={{width:'100%',marginTop:'6px',background:'#0F4C75',color:'white',border:'none',padding:'10px',borderRadius:'8px',fontWeight:'bold'}}>🗺️ До магазина - автоматична навигация</button>
                  </div>
                  ...
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#fff3cd',padding:'8px',borderRadius:'8px'}}>
                    ...
                    📍 {o.address}
                    <button onClick={()=>openNavigation(o.address)} style={{width:'100%',marginTop:'8px',background:'#22c55e',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'900',fontSize:'14px'}}>🧭 До клиента - автоматична навигация</button>
                  </div>
        ...
        {tab==='admin' && isAdmin && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#FFD60A',padding:'14px',borderRadius:'14px',border:'2px solid black'}}><b>👑 Админ LIVE</b><div style={{fontSize:'11px'}}>Магазини: {shops.length} • Поръчки за шофьор: {myShopOrders.length}</div></div>
            <div style={{background:'white',borderRadius:'14px',padding:'12px'}}><b>🏪 Всички магазини - {shops.length}</b>{shops.map((s:any)=>{ return (<div key={s.id} style={{background:'#f9fafb',padding:'12px',borderRadius:'12px',border:'1px solid #e5e7eb',marginTop:'10px'}}><div style={{display:'flex',justifyContent:'space-between'}}><div><b>{s.name}</b><div style={{fontSize:'11px',color:'#666'}}>📍 {s.address} • 💶 {s.delivery_fee}€</div></div><button onClick={()=>startEditShop(s)} style={{background:'#0F4C75',color:'white',border:'none',padding:'8px 12px',borderRadius:'8px',fontSize:'11px'}}>✏️</button></div></div>)})}</div>
          </div>
        )}
      </div>
