...
export default function ShopsFinal(){
  ...
  const loadMyShopProfile = async ()=>{
    ...
    if(myProfiles.length===0 && isAdmin){
      // FIX: Админ без профил да вижда всички заявки за шофьор за теста
  ...
  const addShop = async ()=>{
    ...
    if(editShopData){
      const {error}=await supabase.from('shops').update({
        name:shopName,
        city:shopCity||shopAddress||'',
        address:shopAddress||'',
        phone:ownerPhone,
        delivery_fee: parseFloat(shopDeliveryFee)||4.99,
      }).eq('id',editShopData.id);
      if(error){ alert(error.message); return; }
  ...
  const startEditShop = (shop:any)=>{
    ...
    const driverProfile = shopProfiles.find((p:any)=> p.shop_id===shop.id && p.role==='driver');
    setDriverPhone(driverProfile?.phone||'');
    window.scrollTo({top:0,behavior:'smooth'});
    showToast(`✏️ Редактираш ${shop.name} - промени и натисни Запази`);
  };

  const cancelEditShop = ()=>{
    setEditShopData(null);
    setShopName(''); setShopAddress(''); setShopCity(''); setShopDeliveryFee('4.99'); setOwnerPhone(''); setDriverPhone('');
  };
  ...
  const deleteShop = async (id:string)=>{
    ...
    await loadShops();
    if(editShopData?.id===id) cancelEditShop();
  };

  const updateMyShop = async ()=>{
    if(!myShopProfile?.shop_id){ alert('Нямаш магазин'); return; }
    if(!shopName ||!shopAddress){ alert('Име и адрес!'); return; }
    const {error}=await supabase.from('shops').update({
      name: shopName,
      address: shopAddress,
      city: shopCity||shopAddress,
      delivery_fee: parseFloat(shopDeliveryFee)||4.99,
      phone: ownerPhone||myShopProfile.phone
    }).eq('id',myShopProfile.shop_id);
    if(error){ alert(error.message); return; }
    showToast('✅ Моят магазин е променен!');
    setShopName(''); setShopAddress(''); setShopCity(''); await loadShops(); await loadMyShopProfile();
  };

  const startEditMyShop = ()=>{
    const myShop = shops.find(s=> s.id===myShopProfile?.shop_id);
    if(!myShop) return;
    setShopName(myShop.name); setShopAddress(myShop.address||''); setShopCity(myShop.city||''); setShopDeliveryFee(String(myShop.delivery_fee||4.99)); setOwnerPhone(myShop.phone||'');
  };
  ...
  // FIX: когато дойдат нови поръчки, изчисти старото сортиране
  useEffect(()=>{
    if(myShopOrders.length>0){
      // ако има нови поръчки които не са в sortedOrders, чистим
      if(sortedOrders.length>0 && myShopOrders.length!== sortedOrders.length){
        setSortedOrders([]);
      }
    }
  }, [myShopOrders.length]);
  ...
  // FIX: шофьорски поръчки - винаги взимаме пресни данни
  ...
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'#f9fafb',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      <header style={{background:'#0F4C75',color:'white',padding:'10px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div><b>🛒 Магазини {isAdmin?'👑 АДМИН':''}</b><div style={{fontSize:'10px',opacity:0.8}}>{currentUser.firstName} • {currentUser.phone}</div></div>
      ...
      <div style={{display:'flex',gap:'4px',padding:'6px',background:'white',borderBottom:'1px solid #e5e7eb'}}>
        ...
        {/* FIX: махнах!isAdmin за да виждаш и двете роли докато тестваш */}
      ...
      <div style={{flex:1,overflowY:'auto',padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
        ...
        {tab==='market' && (
          <>
            {!selectedShop? (
              <>
                <div style={{background:'#22c55e',color:'white',padding:'14px',borderRadius:'14px'}}><b>🛒 Пазар - Магазини</b><div style={{fontSize:'11px',marginTop:'4px'}}>Без треперене - само LIVE</div><input value={marketSearch} onChange={e=>setMarketSearch(e.target.value)} placeholder="🔍 Търси магазин..." style={{width:'100%',marginTop:'10px',padding:'10px',borderRadius:'10px',border:'none'}}/></div>
        ...
        {tab==='my_shop' && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#22c55e',color:'white',padding:'14px',borderRadius:'14px'}}>
              <div style={{display:'flex',justifyContent:'space-between'}}><b>🏪 Моят магазин - {shops.find(s=>s.id===myShopProfile?.shop_id)?.name||''}</b><button onClick={startEditMyShop} style={{background:'white',color:'#22c55e',border:'none',padding:'6px 12px',borderRadius:'8px',fontWeight:'800',fontSize:'11px'}}>✏️ Редактирай магазина</button></div>
              <div style={{fontSize:'11px',marginTop:'6px'}}>{shops.find(s=>s.id===myShopProfile?.shop_id)?.address||''} • Тел: {shops.find(s=>s.id===myShopProfile?.shop_id)?.phone||''} • Доставка: {shops.find(s=>s.id===myShopProfile?.shop_id)?.delivery_fee||'4.99'}€</div>
              {shopName && <div style={{background:'white',color:'#166534',padding:'10px',borderRadius:'10px',marginTop:'10px'}}><b>✏️ Редактираш моя магазин:</b><input value={shopName} onChange={e=>setShopName(e.target.value)} placeholder="Име" style={{width:'100%',padding:'8px',borderRadius:'6px',border:'1px solid #22c55e',marginTop:'6px'}}/><input value={shopAddress} onChange={e=>setShopAddress(e.target.value)} placeholder="Адрес" style={{width:'100%',padding:'8px',borderRadius:'6px',border:'1px solid #22c55e',marginTop:'6px'}}/><div style={{display:'flex',gap:'6px',marginTop:'6px'}}><input value={shopCity} onChange={e=>setShopCity(e.target.value)} placeholder="Град" style={{flex:1,padding:'8px',borderRadius:'6px',border:'1px solid #ddd'}}/><input value={shopDeliveryFee} onChange={e=>setShopDeliveryFee(e.target.value)} placeholder="Доставка €" style={{flex:1,padding:'8px',borderRadius:'6px',border:'1px solid #ddd'}}/></div><div style={{display:'flex',gap:'6px',marginTop:'8px'}}><button onClick={updateMyShop} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'10px',borderRadius:'8px',fontWeight:'800'}}>💾 Запази магазина</button><button onClick={()=>{setShopName(''); setShopAddress('');}} style={{background:'#e5e7eb',border:'none',padding:'10px',borderRadius:'8px'}}>Откажи</button></div></div>}
            </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}>
              ...
              <div style={{background:activeDriverCount>=3?'#fee2e2':'#f0fdf4',border:'1px solid #fff',padding:'8px',borderRadius:'8px',marginTop:'8px',color:activeDriverCount>=3?'#991b1b':'#166534'}}>
                <b style={{fontSize:'12px'}}>📦 Лимит: {activeDriverCount}/3 активни</b><span style={{fontSize:'10px', marginLeft:'8px'}}>Всички: {myShopOrders.length} поръчки</span>
              </div>
            ...
            {availableDriverOrders.length===0 && <div style={{background:'white',padding:'20px',borderRadius:'12px',textAlign:'center',color:'#666'}}>Няма заявки за шофьор в момента.<br/><span style={{fontSize:'11px'}}>Маганзер трябва да натисне 🚚 Поискай шофьор</span></div>}
            {availableDriverOrders.map((o:any)=>{
              ...
              return (
                <div key={o.id} style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}>
                  ...
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#e0f2fe',padding:'8px',borderRadius:'8px'}}><b>🏪 {shop?.name || 'Магазин'}</b><br/>📍 {shop?.address||shop?.city || o.shop_id}<br/><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(shop?.address||'')}`} target="_blank" style={{color:'#0F4C75',fontWeight:'bold'}}>🗺️ До магазина</a></div>
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#fff3cd',padding:'8px',borderRadius:'8px'}}><b>👤 {o.customer_name}</b> • {o.customer_phone}<br/>📍 {o.address}<br/><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(o.address)}`} target="_blank" style={{color:'#b45309',fontWeight:'bold'}}>🗺️ До клиента</a></div>
                  <div style={{display:'flex',gap:'8px',marginTop:'10px'}}>
                    ...
                    {o.driver_phone===currentUser.phone && (o.status==='on_the_way' || o.status==='picked_up') && <button onClick={()=>updateOrderStatus(o.id,'delivered')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>✅ Доставена</button>}
                    {o.driver_phone===currentUser.phone && <button onClick={()=>refuseOrder(o.id)} style={{background:'#fee2e2',color:'#991b1b',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>❌</button>}
                  </div>
                </div>
              )
            })}
        ...
        {tab==='admin' && isAdmin && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#FFD60A',padding:'14px',borderRadius:'14px',border:'2px solid black'}}>
              <b>👑 Админ - ВСИЧКИ ЕКСТРИ</b>
              <div style={{fontSize:'11px',marginTop:'4px'}}>✅ Редактирай адрес, телефон, доставка • ✅ Без треперене • ✅ LIVE: {myShopOrders.length} поръчки за шофьор</div>
            </div>

            <div style={{background: editShopData? '#fef3c7' : 'white',border:`3px solid ${editShopData? '#f59e0b' : '#22c55e'}`,borderRadius:'16px',padding:'14px'}}>
              <b style={{fontSize:'15px'}}>{editShopData? `✏️ Редактираш: ${editShopData.name}` : '➕ Нов магазин - всички полета'}</b>
              {editShopData && <div style={{fontSize:'10px',background:'#f59e0b',color:'white',padding:'4px 8px',borderRadius:'8px',display:'inline-block',marginLeft:'8px'}}>РЕЖИМ РЕДАКТИРАНЕ</div>}
              <input value={shopName} onChange={e=>setShopName(e.target.value)} placeholder="Име: HALASTRA" style={{width:'100%',padding:'12px',borderRadius:'10px',border:'2px solid #0F4C75',marginTop:'12px',fontWeight:'bold'}}/>
              <div style={{display:'flex',gap:'8px',marginTop:'8px'}}>
                <input value={shopAddress} onChange={e=>setShopAddress(e.target.value)} placeholder="📍 Адрес: ул. Вапцаров 2" style={{flex:2,padding:'12px',borderRadius:'10px',border:'2px solid #0F4C75'}}/>
                <input value={shopCity} onChange={e=>setShopCity(e.target.value)} placeholder="Град: Две Могили" style={{flex:1,padding:'12px',borderRadius:'10px',border:'1px solid #ddd'}}/>
              </div>
              <div style={{display:'flex',gap:'8px',marginTop:'8px'}}>
                <input value={ownerPhone} onChange={e=>setOwnerPhone(e.target.value)} placeholder="📞 Тел. собственик" style={{flex:1,padding:'12px',borderRadius:'10px',border:'2px solid #22c55e'}}/>
                <input value={shopDeliveryFee} onChange={e=>setShopDeliveryFee(e.target.value)} placeholder="Доставка € 4.99" style={{flex:1,padding:'12px',borderRadius:'10px',border:'1px solid #ddd'}}/>
              </div>
              <input value={driverPhone} onChange={e=>setDriverPhone(e.target.value)} placeholder="🚚 Шофьор тел (по желание)" style={{width:'100%',padding:'12px',borderRadius:'10px',border:'1px solid #ddd',marginTop:'8px'}}/>
              <div style={{display:'flex',gap:'8px',marginTop:'12px'}}>
                <button onClick={addShop} style={{flex:2,padding:'14px',background: editShopData? '#f59e0b' : '#22c55e',color:'white',border:'none',borderRadius:'12px',fontWeight:'900',fontSize:'14px'}}>{editShopData? '💾 ЗАПАЗИ ПРОМЕНИТЕ' : '➕ СЪЗДАЙ МАГАЗИН'}</button>
                {editShopData && <button onClick={cancelEditShop} style={{flex:1,padding:'14px',background:'#e5e7eb',border:'none',borderRadius:'12px',fontWeight:'bold'}}>Откажи</button>}
              </div>
            </div>
            ...
            <div style={{background:'white',borderRadius:'14px',padding:'12px'}}>
              ...
              {shops.map((s:any)=>{
                const driverP = shopProfiles.find((p:any)=> p.shop_id===s.id && p.role==='driver');
                return (
                  <div key={s.id} style={{background: editShopData?.id===s.id? '#fef3c7' : '#f9fafb',padding:'12px',borderRadius:'12px',border: editShopData?.id===s.id? '2px solid #f59e0b' : '1px solid #e5e7eb',marginTop:'10px'}}>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                      <div style={{flex:1}}><b style={{fontSize:'14px'}}>{s.name} {s.vip_active===false && '(СПРЯН)'}</b><div style={{fontSize:'11px',color:'#666',marginTop:'2px'}}>📍 {s.address||s.city||'Без адрес'} • 🏙️ {s.city||''} • 💶 {s.delivery_fee}€ • 📞 {s.phone}</div><div style={{fontSize:'10px',color:'#0F4C75',marginTop:'2px'}}>Собственик: {s.phone} {driverP && `• Шофьор: ${driverP.phone}`}</div></div>
                      <div style={{display:'flex',flexDirection:'column',gap:'4px',marginLeft:'8px'}}>
                        <button onClick={()=>startEditShop(s)} style={{background:'#0F4C75',color:'white',border:'none',padding:'8px 12px',borderRadius:'8px',fontSize:'11px',fontWeight:'bold'}}>✏️ Редактирай</button>
                        <div style={{display:'flex',gap:'4px'}}>
                          <button onClick={()=>toggleShopActive(s)} style={{background:s.vip_active===false?'#22c55e':'#FF3B30',color:'white',border:'none',padding:'6px 8px',borderRadius:'6px',fontSize:'9px'}}>{s.vip_active===false?'ПУСНИ':'СПРИ'}</button>
                          <button onClick={()=>deleteShop(s.id)} style={{background:'black',color:'white',border:'none',padding:'6px 8px',borderRadius:'6px',fontSize:'9px'}}>🗑️</button>
                        </div>
                      </div>
                    </div>
                  </div>
