...
const PRODUCT_CATS = ['Месо','Кайма','Колбаси','Млечни','Хляб','Напитки','Консерви','Други'];
...
Ние НЕ сме таксиметрова фирма. Сайтът VoziMe е 100% безплатен, без комисионна.`,
    terms: `1. Какво е VoziMe? Информационна платформа. Не е превозвач.
2. Декларация на шофьора - пътуваш лично, само споделени разходи, БЕЗ печалба.
3. Плащане - сайтът не обработва плащания.
4. Отговорност - докладвай с 🚩.
5. Верификация - снимка 1 година, изтрит акаунт 6 месеца.`,
    privacy: `Администратор: DropOffPay.co.uk / VoziMe.bg
Контакт: dropoffpay.co.uk@gmail.com
Данни: Име, телефон, обяви, снимка за верификация 1г, IP.
Основание: Легитимен интерес чл.6(1)(е)`,
    contact: `VoziMe.bg е част от DropOffPay.co.uk
📧 dropoffpay.co.uk@gmail.com
🌐 www.dropoffpay.co.uk
📞 +44 7935463970`
  },
  en: {
    about: `VoziMe.bg is a project of DropOffPay.co.uk - free, no commission.`,
    terms: `Information platform, not a carrier. Shared costs only.`,
    privacy: `Data Controller: DropOffPay.co.uk`,
    contact: `Contact: dropoffpay.co.uk@gmail.com`
  }
};
...
export default function Home(){
  ...
  const loadMyShopProfile = async ()=>{
    ...
    if(isAdminPhone(currentUser.phone)){
      ...
      for(const sid of shopIds){
        const {data:prods}=await supabase.from('products').select('*').eq('shop_id', sid).order('created_at',{ascending:false});
      ...
      setMyShopProducts(allProds); setMyShopOrders(allOrders); return;
    }
    const myClean10 = clean(currentUser.phone);
    const myCleanFull = cleanFull(currentUser.phone);
    const {data:allProfiles}=await supabase.from('shop_profiles').select('*');
  ...
  const loadCustomerOrders = async ()=>{
    if(!currentUser?.phone) return;
    const {data}=await supabase.from('orders').select('*').eq('customer_phone', currentUser.phone).order('created_at',{ascending:false}).limit(50);
  ...
  const addProductWithImage = async ()=>{
    ...
    const baseData:any = {name:newProdName.trim(), price:priceNum, description:`[${newProdCat}] ${newProdDesc.trim()}`, image_url:imageUrl, active:newProdActive};
    if(editingProduct){
      await supabase.from('products').update(baseData).eq('id', editingProduct.id);
    } else {
      await supabase.from('products').insert({shop_id:myShopProfile.shop_id, ...baseData});
    }
    setNewProdName(''); setNewProdPrice(''); setNewProdDesc(''); setNewProdCat('Месо'); setNewProdActive(true); setNewProdFile(null); setEditingProduct(null);
    await loadMyShopProfile();
  };
  const startEditProduct = (p:any)=>{ 
    if(myShopProfile?.role!=='shop_owner'){ alert('Само собственика!'); return; }
    const catMatch = p.description?.match(/^\[(.*?)\]/); const cat = catMatch? catMatch[1] : 'Други'; const desc = p.description?.replace(/^\[.*?\]\s*/, '') || '';
    setEditingProduct(p); setNewProdName(p.name); setNewProdPrice(p.price.toString()); setNewProdDesc(desc); setNewProdCat(PRODUCT_CATS.includes(cat)?cat:'Други'); setNewProdActive(p.active!==false); window.scrollTo({top:0, behavior:'smooth'}); 
  };
  const cancelEditProduct = ()=>{ setEditingProduct(null); setNewProdName(''); setNewProdPrice(''); setNewProdDesc(''); setNewProdCat('Месо'); setNewProdActive(true); setNewProdFile(null); };
  const deleteProduct = async (id:string)=>{ if(myShopProfile?.role!=='shop_owner'){ alert('Само собственика!'); return; } if(!confirm('Изтрий?')) return; await supabase.from('products').delete().eq('id',id); setMyShopProducts(myShopProducts.filter(p=>p.id!==id)); };
  const toggleProductActive = async (p:any)=>{ const newActive = !p.active; await supabase.from('products').update({active:newActive}).eq('id', p.id); setMyShopProducts(myShopProducts.map(x=> x.id===p.id ? {...x, active:newActive}: x)); setMarketProducts(marketProducts.map(x=> x.id===p.id ? {...x, active:newActive}: x)); };
  const loadMarketProducts = async (shopId:string)=>{ const {data}=await supabase.from('products').select('*').eq('shop_id', shopId).order('active',{ascending:false}).order('created_at',{ascending:false}); if(data) setMarketProducts(data.filter((x:any)=> x.active!==false)); setSelectedMarketShop(marketShops.find(s=>s.id===shopId)); setMarketSearch(''); setMarketCat('Всички'); setCart([]); setCartOpen(false); };
  ...
  const decCart = (pid:string)=> setCart(prev=> { const updated = prev.map(c=> c.product.id===pid ? {...c, qty:c.qty-1}:c).filter(c=> c.qty>0); return updated; });
  ...
  const getCartTotal = ()=> getCartSubtotal() + parseFloat(selectedMarketShop?.delivery_fee||'4.99');
  const placeOrder = async ()=>{
    ...
    const {error}=await supabase.from('orders').insert({shop_id:selectedMarketShop.id, customer_phone:currentUser.phone, customer_name:`${currentUser.firstName} ${currentUser.lastName}`, address:customerAddr, items, total, status:'new'});
    if(error){ alert(error.message); return; }
    alert('Поръчка изпратена! Магазина ще я види в Моите.'); setCart([]); setCustomerAddr(''); setCartOpen(false); await loadCustomerOrders(); await loadMyShopProfile();
  };
  ...
  const handleVerificationUpload = async ()=>{
    ...
    try{
      ...
      alert(t.photoSent); setVerifyFile(null); await syncCurrentUser();
    ...
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'white',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      <div style={{flexShrink:0}}>
        ...
        <div style={{height:'52px',display:'flex',gap:'4px',padding:'6px',background:'#f1f3f4'}}>
          ...
          <button onClick={()=>setTab('market')} style={{flex:1,borderRadius:'10px',border:'none',fontWeight:'bold',background:tab==='market'?'#22c55e':'white',color:tab==='market'?'white':'#666',fontSize:'10px'}}>🛒 Пазар {getCartCount()>0?`(${getCartCount()})`:''}</button>
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {tab==='admin' && isAdmin && (<div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}><div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}><div style={{fontWeight:'bold',fontSize:'14px'}}>👑 ADMIN • DropOffPay.co.uk</div><div style={{fontSize:'11px',opacity:0.8}}>All users kept 6 months • Verification required</div><button onClick={syncCurrentUser} style={{marginTop:'8px',background:'#2ECC71',border:'none',padding:'6px 12px',borderRadius:'8px',fontSize:'11px',fontWeight:'bold'}}>{t.refresh}</button></div><div style={{background:'white',border:'2px solid #2ECC71',borderRadius:'12px',padding:'12px'}}><div style={{fontWeight:'bold',fontSize:'13px'}}>👥 ALL USERS - {filteredUsers.length}</div><input placeholder="Търси..." value={userSearch} onChange={e=>setUserSearch(e.target.value)} style={{width:'100%',marginTop:'8px',padding:'10px',borderRadius:'8px',border:'1px solid #ddd',fontSize:'12px'}}/><div style={{marginTop:'10px',display:'flex',flexDirection:'column',gap:'6px',maxHeight:'400px',overflowY:'auto'}}>{filteredUsers.map(u=>{ const country = u.country_code||getCountryByPhone(u.phone||''); const isDeleted =!!u.deleted_at; return (<div key={u.id} style={{background:isDeleted?'#ffeaea':u.is_verified?'#e6f9ed':'#fffbe6',border:`1px solid ${isDeleted?'#FF3B30':u.is_verified?'#2ECC71':'#FFD60A'}`,padding:'8px',borderRadius:'8px'}}><div style={{display:'flex',justifyContent:'space-between'}}><div style={{fontWeight:'bold',fontSize:'12px'}}>{getFlag(country)} {u.first_name} {u.last_name} {u.is_verified?'✅':''}</div><div style={{fontSize:'10px'}}>{u.phone}</div></div>{u.verification_photo_url && <div style={{marginTop:'4px',display:'flex',gap:'6px'}}><a href={u.verification_photo_url} target="_blank" style={{fontSize:'10px',background:'#0F4C75',color:'white',padding:'4px 8px',borderRadius:'6px',textDecoration:'none'}}>📸 View</a>{!u.is_verified && <button onClick={()=>approveUser(u.id)} style={{fontSize:'10px',background:'#2ECC71',border:'none',padding:'4px 8px',borderRadius:'6px',fontWeight:'bold'}}>✅ Approve</button>}</div>}</div>)})}</div></div><div style={{background:'white',border:'2px solid black',borderRadius:'12px',padding:'12px'}}><div style={{fontWeight:'bold',fontSize:'13px'}}>⛔ BAN - {bans.length}</div><div style={{display:'flex',gap:'6px',marginTop:'8px'}}><input placeholder="Phone" value={banPhoneInput} onChange={e=>setBanPhoneInput(e.target.value)} style={{flex:1,padding:'8px',borderRadius:'8px',border:'1px solid #ddd',fontSize:'11px'}}/><select value={banDuration} onChange={e=>setBanDuration(e.target.value)} style={{padding:'8px',borderRadius:'8px',border:'1px solid #ddd',fontSize:'11px'}}><option value="1h">1h</option><option value="24h">24h</option><option value="7d">7d</option><option value="forever">Forever</option></select></div><input placeholder="Причина" value={banReasonInput} onChange={e=>setBanReasonInput(e.target.value)} style={{width:'100%',marginTop:'6px',padding:'8px',borderRadius:'8px',border:'1px solid #ddd',fontSize:'11px'}}/><button onClick={()=>{if(!banPhoneInput) return; handleBan(banPhoneInput, banReasonInput||'Bad', banDuration);}} style={{width:'100%',marginTop:'6px',background:'black',color:'white',padding:'10px',borderRadius:'8px',fontWeight:'bold',border:'none'}}>⛔ BAN</button><div style={{marginTop:'10px',display:'flex',flexDirection:'column',gap:'6px'}}>{bans.map(b=><div key={b.id} style={{background:'#ffeaea',padding:'8px',borderRadius:'8px',display:'flex',justifyContent:'space-between'}}><div><div style={{fontWeight:'bold',fontSize:'11px'}}>{b.originalPhone} ({b.phone})</div><div style={{fontSize:'10px'}}>{b.reason}</div></div><button onClick={()=>unban(b.id)} style={{background:'#2ECC71',border:'none',padding:'6px 10px',borderRadius:'6px',fontSize:'10px'}}>UNBAN</button></div>)}</div></div><div style={{background:'white',border:'2px solid #FFD60A',borderRadius:'12px',padding:'12px'}}><div style={{fontWeight:'bold',fontSize:'13px'}}>🚩 REPORTS - {reports.length}</div><div style={{marginTop:'10px',display:'flex',flexDirection:'column',gap:'8px'}}>{reports.map(rep=><div key={rep.id} style={{background:'#fffbe6',border:'1px solid #FFD60A',padding:'10px',borderRadius:'10px'}}><div style={{fontSize:'11px',fontWeight:'bold'}}>🚩 {rep.reportedName} • {rep.reportedPhone}</div><div style={{display:'flex',gap:'6px',marginTop:'6px'}}><button onClick={()=>banFromReport(rep,'24h')} style={{flex:1,background:'#FF3B30',color:'white',border:'none',padding:'6px',borderRadius:'6px',fontSize:'10px'}}>BAN 24h</button><button onClick={()=>banFromReport(rep,'forever')} style={{flex:1,background:'black',color:'white',border:'none',padding:'6px',borderRadius:'6px',fontSize:'10px'}}>Forever</button><button onClick={()=>dismissReport(rep.id)} style={{padding:'6px 10px',borderRadius:'6px',border:'1px solid #ddd',background:'white',fontSize:'10px'}}>❌</button></div></div>)}</div></div></div>)}
        {tab==='shops' && isAdmin && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#22c55e',color:'white',padding:'12px',borderRadius:'12px'}}><b>👑 Магазини - само ти</b><div style={{fontSize:'11px'}}>СПРИ, име, такса, телефон собственик, шофьори. Без цени.</div></div>
            <div style={{background:'white',border:'2px solid #22c55e',borderRadius:'12px',padding:'12px'}}>
              ...
              <input value={ownerPhoneInput} onChange={e=>setOwnerPhoneInput(e.target.value)} placeholder="Собственик тел" style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #ddd',marginTop:'8px'}}/>
              ...
              <button onClick={addShop} style={{width:'100%',marginTop:'10px',padding:'12px',background:'#22c55e',color:'white',border:'none',borderRadius:'8px',fontWeight:'bold'}}>СЪЗДАЙ</button>
            </div>
            {shops.map((s:any)=>{
        ...
        {tab==='market' && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>
            {!selectedMarketShop? (
              <>
                <div style={{background:'#22c55e',color:'white',padding:'12px',borderRadius:'12px'}}><b>🛒 Пазар - Магазини</b><div style={{fontSize:'11px'}}>VoziMe 0% - доставката е при магазина. Без София.</div></div>
                {marketShops.map((s:any)=><div key={s.id} onClick={()=>loadMarketProducts(s.id)} style={{background:'white',border:'2px solid #22c55e',borderRadius:'14px',padding:'14px',cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><b style={{fontSize:'15px'}}>{s.name}</b><br/><span style={{fontSize:'12px',background:'#f0fdf4',padding:'2px 8px',borderRadius:'12px',border:'1px solid #22c55e'}}>🚚 {s.delivery_fee}€ доставка</span></div><div style={{background:'#22c55e',color:'white',padding:'8px 14px',borderRadius:'20px',fontWeight:'bold',fontSize:'12px'}}>Виж →</div></div>)}
                {marketShops.length===0 && <div style={{textAlign:'center',padding:'20px',color:'#666'}}>Няма активни магазини</div>}
              </>
            ) : (
              <>
                <button onClick={()=>{setSelectedMarketShop(null); setMarketProducts([]); setCart([]); setCartOpen(false);}} style={{padding:'10px',borderRadius:'10px',border:'1px solid #ddd',background:'white',fontWeight:'bold'}}>← Назад към магазини</button>
                <div style={{background:'#0F4C75',color:'white',padding:'14px',borderRadius:'14px'}}><b style={{fontSize:'16px'}}>{selectedMarketShop.name}</b><br/><span style={{fontSize:'12px'}}>🚚 Доставка {selectedMarketShop.delivery_fee}€ • {marketProducts.length} продукта</span></div>
                <input placeholder="🔍 Търси продукт..." value={marketSearch} onChange={e=>setMarketSearch(e.target.value)} style={{padding:'12px',borderRadius:'12px',border:'2px solid #22c55e',fontSize:'14px'}}/>
                <div style={{display:'flex',gap:'6px',overflowX:'auto',paddingBottom:'4px'}}>{['Всички',...PRODUCT_CATS].map(cat=><button key={cat} onClick={()=>setMarketCat(cat)} style={{whiteSpace:'nowrap',padding:'8px 14px',borderRadius:'20px',border:'none',fontWeight:'bold',fontSize:'11px',background:marketCat===cat?'#22c55e':'white',color:marketCat===cat?'white':'#666',borderWidth:'1px',borderStyle:'solid',borderColor:'#e5e7eb'}}>{cat}</button>)}</div>
                <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                  {marketProducts.filter((p:any)=>{ const cat = p.description?.match(/\[(.*?)\]/)?.[1] || 'Други'; const matchesCat = marketCat==='Всички' || cat===marketCat; const matchesSearch = !marketSearch || p.name.toLowerCase().includes(marketSearch.toLowerCase()) || p.description.toLowerCase().includes(marketSearch.toLowerCase()); return matchesCat && matchesSearch; }).map((p:any)=>{
                    const cat = p.description?.match(/\[(.*?)\]/)?.[1] || 'Други'; const desc = p.description?.replace(/\[.*?\]\s*/, '') || ''; const inCart = cart.find(c=> c.product.id===p.id);
                    return (<div key={p.id} style={{background:'white',borderRadius:'14px',padding:'12px',display:'flex',gap:'12px',border:'1px solid #e5e7eb',boxShadow:'0 1px 3px rgba(0,0,0,0.05)'}}>{p.image_url ? <img src={p.image_url} style={{width:'70px',height:'70px',objectFit:'cover',borderRadius:'10px'}}/> : <div style={{width:'70px',height:'70px',background:'#f3f4f6',borderRadius:'10px',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'24px'}}>🛒</div>}<div style={{flex:1}}><div style={{display:'flex',gap:'6px',alignItems:'center'}}><b style={{fontSize:'14px'}}>{p.name}</b><span style={{fontSize:'9px',background:'#f0fdf4',color:'#22c55e',padding:'2px 6px',borderRadius:'10px',border:'1px solid #22c55e',fontWeight:'bold'}}>{cat}</span></div><div style={{fontSize:'11px',color:'#666',marginTop:'2px'}}>{desc}</div><div style={{marginTop:'6px'}}><b style={{color:'#0F4C75',fontSize:'15px'}}>{p.price}€</b></div></div><div style={{display:'flex',flexDirection:'column',justifyContent:'center',alignItems:'center',gap:'6px'}}>{inCart ? <div style={{display:'flex',alignItems:'center',gap:'8px',background:'#f0fdf4',borderRadius:'20px',padding:'4px',border:'1px solid #22c55e'}}><button onClick={()=>decCart(p.id)} style={{width:'28px',height:'28px',borderRadius:'50%',border:'none',background:'white',fontWeight:'bold'}}>−</button><b style={{minWidth:'20px',textAlign:'center'}}>{inCart.qty}</b><button onClick={()=>incCart(p.id)} style={{width:'28px',height:'28px',borderRadius:'50%',border:'none',background:'#22c55e',color:'white',fontWeight:'bold'}}>+</button></div> : <button onClick={()=>addToCart(p)} style={{background:'#22c55e',color:'white',border:'none',padding:'10px 18px',borderRadius:'20px',fontWeight:'bold',fontSize:'12px'}}>Добави</button>}</div></div>)
                  })}
                </div>
                {cart.length>0 && (
                  <div style={{position:'sticky',bottom:'0',background:'white',borderTop:'2px solid #22c55e',padding:'12px',borderRadius:'16px 16px 0 0',boxShadow:'0 -4px 20px rgba(0,0,0,0.1)',marginTop:'10px'}}>
                    {!cartOpen ? (
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><b>🛒 {getCartCount()} продукта</b><br/><span style={{fontSize:'12px',color:'#666'}}>Общо {getCartTotal().toFixed(2)}€ с доставка</span></div><button onClick={()=>setCartOpen(true)} style={{background:'#0F4C75',color:'white',border:'none',padding:'12px 20px',borderRadius:'20px',fontWeight:'bold'}}>Количка →</button></div>
                    ) : (
                      <div>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}}><b>🛒 Количка - {getCartCount()}</b><button onClick={()=>setCartOpen(false)} style={{background:'#f3f4f6',border:'none',padding:'6px 12px',borderRadius:'8px'}}>− Скрий</button></div>
                        {cart.map(c=><div key={c.product.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:'1px solid #f3f4f6'}}><div style={{flex:1}}><b style={{fontSize:'12px'}}>{c.product.name}</b><br/><span style={{fontSize:'11px'}}>{c.product.price}€ x {c.qty} = {(parseFloat(c.product.price)*c.qty).toFixed(2)}€</span></div><div style={{display:'flex',gap:'6px',alignItems:'center'}}><button onClick={()=>decCart(c.product.id)} style={{width:'24px',height:'24px',borderRadius:'50%',border:'1px solid #ddd',background:'white'}}>−</button><b>{c.qty}</b><button onClick={()=>incCart(c.product.id)} style={{width:'24px',height:'24px',borderRadius:'50%',border:'none',background:'#22c55e',color:'white'}}>+</button><button onClick={()=>removeCartItem(c.product.id)} style={{background:'#ffeaea',border:'none',padding:'4px 8px',borderRadius:'6px',fontSize:'10px'}}>X</button></div></div>)}
                        <div style={{marginTop:'10px',background:'#f9fafb',padding:'10px',borderRadius:'10px',fontSize:'12px'}}><div style={{display:'flex',justifyContent:'space-between'}}><span>Продукти:</span><span>{getCartSubtotal().toFixed(2)}€</span></div><div style={{display:'flex',justifyContent:'space-between'}}><span>Доставка:</span><span>{parseFloat(selectedMarketShop.delivery_fee).toFixed(2)}€</span></div><div style={{display:'flex',justifyContent:'space-between',fontWeight:'800',borderTop:'1px solid #e5e7eb',marginTop:'6px',paddingTop:'6px'}}><span>Общо:</span><span>{getCartTotal().toFixed(2)}€</span></div></div>
                        <input value={customerAddr} onChange={e=>setCustomerAddr(e.target.value)} placeholder="📍 Адрес за доставка - улица, номер, град..." style={{width:'100%',padding:'12px',marginTop:'10px',borderRadius:'10px',border:'2px solid #22c55e',fontSize:'13px'}}/>
        ...
        {tab==='my' && <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
          {myShopProfile && myShopProfile.role==='shop_owner' && !isAdmin && (
            <div style={{background:'white',border:'3px solid #22c55e',borderRadius:'16px',padding:'14px'}}>
              <div style={{background:'#22c55e',color:'white',padding:'12px',borderRadius:'12px',margin:'-14px -14px 14px -14px'}}>
                <div style={{fontWeight:'800',fontSize:'15px'}}>🛒 Моят магазин - {shops.find(s=>s.id===myShopProfile.shop_id)?.name||'Магазин'}</div>
                <div style={{fontSize:'11px',opacity:0.9}}>Тук сменяш цени, категории, наличност - само ти. {myShopProducts.length} продукта • {myShopOrders.filter(o=> o.status==='new').length} нови поръчки</div>
              </div>
              <div style={{background:editingProduct?'#fef3c7':'#f0fdf4',border:`2px solid ${editingProduct?'#f59e0b':'#22c55e'}`,borderRadius:'14px',padding:'14px'}}>
                <div style={{fontWeight:'800',fontSize:'13px'}}>{editingProduct?`✏️ Редактираш: ${editingProduct.name}`:'➕ Добави нова стока'}</div>
                <input value={newProdName} onChange={e=>setNewProdName(e.target.value)} placeholder="Име *: Кайма Варна" style={{width:'100%',padding:'12px',borderRadius:'10px',border:'1px solid #22c55e',marginTop:'10px',background:'white',fontSize:'14px'}}/>
                <div style={{display:'flex',gap:'8px',marginTop:'8px'}}>
                  <input value={newProdPrice} onChange={e=>setNewProdPrice(e.target.value)} placeholder="Цена €*: 6.25" style={{flex:1,padding:'12px',borderRadius:'10px',border:'2px solid #22c55e',background:'white',fontSize:'14px',fontWeight:'bold'}}/>
                  <select value={newProdCat} onChange={e=>setNewProdCat(e.target.value)} style={{flex:1,padding:'12px',borderRadius:'10px',border:'1px solid #ddd',background:'white',fontSize:'12px',fontWeight:'bold'}}>{PRODUCT_CATS.map(c=><option key={c} value={c}>{c}</option>)}</select>
                </div>
                <input value={newProdDesc} onChange={e=>setNewProdDesc(e.target.value)} placeholder="Описание: прясно мляно 1кг" style={{width:'100%',padding:'10px',borderRadius:'10px',border:'1px solid #ddd',marginTop:'8px',fontSize:'12px'}}/>
                <div style={{display:'flex',gap:'8px',marginTop:'8px',alignItems:'center'}}><label style={{display:'flex',gap:'6px',fontSize:'12px',fontWeight:'bold'}}><input type="checkbox" checked={newProdActive} onChange={e=>setNewProdActive(e.target.checked)}/> Активен (видим в Пазар)</label></div>
                <div style={{marginTop:'10px',border:'1px dashed #22c55e',padding:'8px',borderRadius:'10px',background:'white'}}>
                  <input type="file" accept="image/*" onChange={e=>setNewProdFile(e.target.files?.[0]||null)} style={{width:'100%'}}/>
                  {editingProduct?.image_url && !newProdFile && <img src={editingProduct.image_url} style={{width:'60px',height:'60px',objectFit:'cover',borderRadius:'8px',marginTop:'6px'}}/>}
                </div>
                <div style={{display:'flex',gap:'8px',marginTop:'12px'}}>
                  <button onClick={addProductWithImage} style={{flex:1,padding:'14px',background:editingProduct?'#f59e0b':'#22c55e',color:'white',border:'none',borderRadius:'12px',fontWeight:'800',fontSize:'13px'}}>{editingProduct?'💾 Запази промените':'➕ Добави продукт'}</button>
              ...
              <div style={{marginTop:'16px'}}>
                <div style={{display:'flex',gap:'8px',marginBottom:'10px'}}><input placeholder="🔍 Търси мой продукт..." value={prodSearch} onChange={e=>setProdSearch(e.target.value)} style={{flex:1,padding:'10px',borderRadius:'10px',border:'1px solid #ddd',fontSize:'12px'}}/><select value={prodCatFilter} onChange={e=>setProdCatFilter(e.target.value)} style={{padding:'10px',borderRadius:'10px',border:'1px solid #ddd',fontSize:'11px'}}><option value="Всички">Всички кат.</option>{PRODUCT_CATS.map(c=><option key={c} value={c}>{c}</option>)}</select></div>
                <div style={{fontWeight:'800',fontSize:'14px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><span>📋 Моите стоки - {myShopProducts.filter(p=>{ const cat = p.description?.match(/\[(.*?)\]/)?.[1] || 'Други'; const matchesCat = prodCatFilter==='Всички' || cat===prodCatFilter; const matchesSearch = !prodSearch || p.name.toLowerCase().includes(prodSearch.toLowerCase()); return matchesCat && matchesSearch; }).length} / {myShopProducts.length}</span><button onClick={()=>loadMyShopProfile()} style={{background:'#0F4C75',color:'white',border:'none',padding:'8px 14px',borderRadius:'8px',fontSize:'11px',fontWeight:'bold'}}>🔄</button></div>
                <div style={{marginTop:'8px',display:'flex',flexDirection:'column',gap:'8px'}}>
                  {myShopProducts.filter(p=>{ const cat = p.description?.match(/\[(.*?)\]/)?.[1] || 'Други'; const matchesCat = prodCatFilter==='Всички' || cat===prodCatFilter; const matchesSearch = !prodSearch || p.name.toLowerCase().includes(prodSearch.toLowerCase()); return matchesCat && matchesSearch; }).map((p:any)=>{ const cat = p.description?.match(/\[(.*?)\]/)?.[1] || 'Други'; const desc = p.description?.replace(/\[.*?\]\s*/, '') || ''; return (<div key={p.id} style={{display:'flex',gap:'10px',alignItems:'center',border:'1px solid #e5e7eb',padding:'10px',borderRadius:'12px',background:p.active===false?'#ffeaea':p.id===editingProduct?.id?'#fef3c7':'white',opacity:p.active===false?0.6:1}}>{p.image_url ? <img src={p.image_url} style={{width:'56px',height:'56px',objectFit:'cover',borderRadius:'10px'}}/> : <div style={{width:'56px',height:'56px',background:'#f3f4f6',borderRadius:'10px',display:'flex',alignItems:'center',justifyContent:'center'}}>🛒</div>}<div style={{flex:1}}><div style={{display:'flex',gap:'6px'}}><b style={{fontSize:'13px'}}>{p.name}</b><span style={{fontSize:'9px',background:'#f0fdf4',padding:'2px 6px',borderRadius:'8px',border:'1px solid #22c55e'}}>{cat}</span>{p.active===false && <span style={{fontSize:'9px',background:'#ffeaea',padding:'2px 6px',borderRadius:'8px'}}>СКРИТ</span>}</div><div style={{fontSize:'11px',color:'#666'}}>{desc}</div><b style={{color:'#22c55e',fontSize:'13px'}}>{p.price}€ • {p.active!==false?'🟢':'🔴'}</b></div><div style={{display:'flex',flexDirection:'column',gap:'4px'}}><button onClick={()=>toggleProductActive(p)} style={{background:p.active===false?'#22c55e':'#f3f4f6',color:p.active===false?'white':'#666',border:'none',padding:'6px 10px',borderRadius:'8px',fontSize:'10px',fontWeight:'bold'}}>{p.active===false?'👁️ Пусни':'👁️‍🗨️ Скрий'}</button><button onClick={()=>startEditProduct(p)} style={{background:'#0F4C75',color:'white',border:'none',padding:'6px 10px',borderRadius:'8px',fontSize:'10px'}}>✏️</button><button onClick={()=>deleteProduct(p.id)} style={{background:'#FF3B30',color:'white',border:'none',padding:'4px 8px',borderRadius:'8px',fontSize:'10px'}}>🗑️</button></div></div>)})}
                </div>
              </div>
              <div style={{marginTop:'18px'}}>
                <div style={{fontWeight:'800',fontSize:'14px',marginBottom:'8px'}}>📦 Поръчки - {myShopOrders.length}</div>
                <div style={{display:'flex',gap:'6px',overflowX:'auto',marginBottom:'10px'}}>{[{k:'all',l:'Всички'},{k:'new',l:'🆕 Нови'},{k:'accepted',l:'✅ Приети'},{k:'preparing',l:'👨‍🍳 Приготвят'},{k:'on_the_way',l:'🚚 На път'},{k:'delivered',l:'📦 Доставени'}].map(f=><button key={f.k} onClick={()=>setOrderStatusFilter(f.k)} style={{whiteSpace:'nowrap',padding:'8px 12px',borderRadius:'20px',border:'none',fontSize:'11px',fontWeight:'bold',background:orderStatusFilter===f.k?'#0F4C75':'white',color:orderStatusFilter===f.k?'white':'#666',borderWidth:'1px',borderStyle:'solid',borderColor:'#e5e7eb'}}>{f.l} {f.k!=='all' ? `(${myShopOrders.filter(o=> o.status===f.k).length})` : ''}</button>)}</div>
                {myShopOrders.filter(o=> orderStatusFilter==='all' || o.status===orderStatusFilter).map((o:any)=><div key={o.id} style={{background:'white',border:'2px solid #22c55e',padding:'12px',borderRadius:'12px',marginBottom:'10px'}}><div style={{display:'flex',justifyContent:'space-between'}}><b style={{fontSize:'12px'}}>{o.customer_name} • {o.customer_phone}</b><span style={{fontSize:'10px',background:o.status==='new'?'#ffeaea':o.status==='delivered'?'#e6f9ed':'#fffbe6',padding:'4px 8px',borderRadius:'10px',fontWeight:'bold'}}>{o.status}</span></div><div style={{fontSize:'11px',marginTop:'4px'}}>📍 {o.address}<br/>🕒 {new Date(o.created_at).toLocaleString('bg-BG')}</div><div style={{background:'#f9fafb',padding:'8px',borderRadius:'8px',marginTop:'8px',fontSize:'11px'}}>{(o.items||[]).map((it:any,i:number)=><div key={i} style={{display:'flex',justifyContent:'space-between'}}><span>{it.name} x{it.qty||1}</span><span>{(parseFloat(it.price)*(it.qty||1)).toFixed(2)}€</span></div>)}<div style={{borderTop:'1px solid #e5e7eb',marginTop:'6px',paddingTop:'6px',fontWeight:'800',display:'flex',justifyContent:'space-between'}}><span>Общо:</span><span>{o.total}€</span></div></div><div style={{display:'flex',gap:'6px',marginTop:'10px'}}>{o.status==='new' && <><button onClick={()=>updateOrderStatus(o.id,'accepted')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'10px',borderRadius:'8px',fontWeight:'bold',fontSize:'11px'}}>✅ Приеми</button><a href={`tel:${o.customer_phone}`} style={{flex:1,background:'#0F4C75',color:'white',padding:'10px',borderRadius:'8px',textAlign:'center',textDecoration:'none',fontWeight:'bold',fontSize:'11px'}}>📞 Обади се</a></>}{o.status==='accepted' && <button onClick={()=>updateOrderStatus(o.id,'preparing')} style={{flex:1,background:'#FFD60A',border:'none',padding:'10px',borderRadius:'8px',fontWeight:'bold',fontSize:'11px'}}>👨‍🍳 Приготвям</button>}{o.status==='preparing' && <button onClick={()=>updateOrderStatus(o.id,'on_the_way')} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'10px',borderRadius:'8px',fontWeight:'bold',fontSize:'11px'}}>🚚 Изпрати</button>}{o.status==='on_the_way' && <button onClick={()=>updateOrderStatus(o.id,'delivered')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'10px',borderRadius:'8px',fontWeight:'bold',fontSize:'11px'}}>📦 Доставено</button>}</div></div>)}
                {myShopOrders.filter(o=> orderStatusFilter==='all' || o.status===orderStatusFilter).length===0 && <div style={{textAlign:'center',color:'#666',fontSize:'12px',padding:'10px'}}>Няма поръчки в тази категория</div>}
              </div>
            </div>
          )}
          {myShopProfile && myShopProfile.role==='driver' && !isAdmin && (
            <div style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}><b>🚚 Моите доставки - {myShopOrders.length}</b><div style={{display:'flex',gap:'6px',marginTop:'10px',marginBottom:'10px'}}>{['all','on_the_way','delivered'].map(s=><button key={s} onClick={()=>setOrderStatusFilter(s)} style={{padding:'6px 12px',borderRadius:'20px',border:'none',fontSize:'11px',background:orderStatusFilter===s?'#0F4C75':'white',color:orderStatusFilter===s?'white':'#666'}}>{s}</button>)}</div>{myShopOrders.filter(o=> orderStatusFilter==='all' || o.status===orderStatusFilter).map((o:any)=><div key={o.id} style={{background:'#f9fafb',border:'1px solid #0F4C75',padding:'12px',borderRadius:'12px',marginBottom:'10px',fontSize:'12px'}}><div style={{display:'flex',justifyContent:'space-between'}}><b>{o.id.slice(0,8)}</b><span style={{background:'#e0f2fe',padding:'4px 8px',borderRadius:'8px',fontSize:'10px'}}>{o.status}</span></div><div style={{marginTop:'6px'}}>{o.customer_name} - {o.customer_phone}<br/>📍 {o.address}<br/><b style={{fontSize:'14px'}}>{o.total}€</b></div><div style={{display:'flex',gap:'6px',marginTop:'10px'}}><a href={`tel:${o.customer_phone}`} style={{flex:1,background:'#0F4C75',color:'white',padding:'10px',borderRadius:'8px',textAlign:'center',textDecoration:'none',fontWeight:'bold',fontSize:'11px'}}>📞 Обади</a><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(o.address)}`} target="_blank" style={{flex:1,background:'#22c55e',color:'white',padding:'10px',borderRadius:'8px',textAlign:'center',textDecoration:'none',fontWeight:'bold',fontSize:'11px'}}>🗺️ Карта</a></div><div style={{display:'flex',gap:'6px',marginTop:'8px'}}>{o.status==='accepted' && <button onClick={()=>updateOrderStatus(o.id,'on_the_way')} style={{flex:1,background:'#FFD60A',border:'none',padding:'10px',borderRadius:'8px',fontWeight:'bold'}}>🚚 Тръгвам</button>}{o.status==='on_the_way' && <button onClick={()=>updateOrderStatus(o.id,'delivered')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'10px',borderRadius:'8px',fontWeight:'bold'}}>✅ Доставено</button>}</div></div>)}</div>
          )}
          {myShopProfile && !isAdmin && myShopProfile.role==='shop_owner' ? null : (
            <div style={{background:'white',border:'2px solid #22c55e',borderRadius:'14px',padding:'12px'}}>
              <b>🛒 Моите поръчки като клиент - {customerOrders.length}</b>
              {customerOrders.map((o:any)=><div key={o.id} style={{background:'#f9fafb',border:'1px solid #e5e7eb',padding:'10px',borderRadius:'10px',marginTop:'8px',fontSize:'11px'}}><div style={{display:'flex',justifyContent:'space-between'}}><b>{shops.find(s=>s.id===o.shop_id)?.name||'Магазин'} • {o.total}€</b><span style={{fontSize:'10px',background:o.status==='new'?'#ffeaea':'#e6f9ed',padding:'3px 8px',borderRadius:'10px'}}>{o.status}</span></div><div>{new Date(o.created_at).toLocaleString('bg-BG')} • {o.address}</div><div style={{marginTop:'4px'}}>{(o.items||[]).map((it:any,i:number)=><span key={i} style={{background:'white',border:'1px solid #e5e7eb',padding:'2px 6px',borderRadius:'6px',marginRight:'4px',fontSize:'10px'}}>{it.name} x{it.qty}</span>)}</div></div>)}
              {customerOrders.length===0 && <div style={{fontSize:'11px',color:'#666',marginTop:'6px'}}>Нямаш поръчки още - отиди в Пазар</div>}
            </div>
          )}
          {isAdmin && <div style={{background:'white',border:'2px solid #0F4C75',borderRadius:'12px',padding:'12px',textAlign:'center',fontSize:'12px'}}><b>👑 Ти си админ</b><br/>Не виждаш цени - цените са само за собственика.<br/>Отиди в <b>👑 Магазини</b> за СПРИ/ПУСНИ, телефони, такси.</div>}
          <div style={{background:isVerified?'#e6f9ed':'#fff3cd',border:`2px solid ${isVerified?'#2ECC71':'#FFD60A'}`,borderRadius:'12px',padding:'12px'}}><div style={{fontWeight:'bold',fontSize:'14px'}}>{isVerified? (lang==='bg'?'✅ Верифициран си! Можеш да пускаш обяви.':'✅ Verified!') : t.verifyTitle}</div><div style={{fontSize:'11px',marginTop:'4px'}}>{isVerified?`${t.verifiedOn} ${currentUser.verified_at? new Date(currentUser.verified_at).toLocaleDateString(lang==='bg'?'bg-BG':'en-GB'):''}`:t.verifyDesc}</div>{!isVerified && <div style={{marginTop:'10px'}}><div style={{background:'white',padding:'8px',borderRadius:'8px',fontSize:'11px',border:'1px solid #ddd',marginBottom:'8px'}}>{t.writeOnPaper}<br/>{t.step1}<br/>{t.step2}{new Date().toLocaleDateString(lang==='bg'?'bg-BG':'en-GB')}<br/>{t.step3}</div><input type="file" accept="image/*" capture="user" onChange={e=>setVerifyFile(e.target.files?.[0]||null)} style={{width:'100%',fontSize:'12px'}}/>{verifyFile && <button onClick={handleVerificationUpload} disabled={uploadingVerify} style={{width:'100%',marginTop:'8px',background:'#0F4C75',color:'white',padding:'12px',borderRadius:'8px',fontWeight:'bold',border:'none'}}>{uploadingVerify?t.uploading:t.sendForCheck}</button>}{currentUser.verification_photo_url &&!currentUser.is_verified && <div style={{fontSize:'11px',marginTop:'6px',color:'#b45309'}}>{t.pendingApproval}</div>}</div>}{isVerified && <button onClick={syncCurrentUser} style={{marginTop:'8px',background:'#0F4C75',color:'white',border:'none',padding:'8px 12px',borderRadius:'8px',fontSize:'11px'}}>{t.refresh}</button>}</div>
          <div style={{fontWeight:'bold',fontSize:'13px',marginTop:'4px'}}>🚗 Твоите обяви за пътуване</div>
          {rides.filter(r=> r.driverId===currentUser?.id || r.driverPhone===currentUser?.phone || clean(r.driverPhone)===clean(currentUser?.phone)).map((r:any)=><div key={r.id} style={{border:'2px solid #0F4C75',borderRadius:'12px',padding:'12px',background:'white'}}><b>{r.from} → {r.to} • {r.time}</b> <span style={{background:r.type==='request'?'#FFD60A':'#2ECC71',padding:'2px 6px',borderRadius:'6px',fontSize:'10px'}}>{r.type==='request'?t.youSeek:t.youOffer}</span><div style={{fontSize:'11px',marginTop:'4px'}}>{r.carInfo} • {r.seats} {t.seatsPlace}</div><div style={{display:'flex',gap:'6px',marginTop:'10px'}}><button onClick={()=>{setOfferForm({type:r.type,from:r.from,to:r.to,fromCountry:r.fromCountry,toCountry:r.toCountry,time:r.time||'09:30',returnTime:'',date:r.date,seats:r.seats.toString(),message:r.message,isDriver:r.isDriverVerified,carBrand:'',carColor:'',carReg:''}); setEditingRide(r.id); setTab('offer');}} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'10px',borderRadius:'8px',fontSize:'12px',fontWeight:'bold'}}>{t.edit}</button><button onClick={()=>deleteRide(r.id)} style={{background:'#FF3B30',color:'white',border:'none',padding:'10px 16px',borderRadius:'8px',fontSize:'12px',fontWeight:'bold'}}>{t.deleteRide}</button></div></div>)}
          {rides.filter(r=> r.driverId===currentUser?.id || r.driverPhone===currentUser?.phone || clean(r.driverPhone)===clean(currentUser?.phone)).length===0 && <div style={{textAlign:'center',padding:'10px',color:'#666',fontSize:'12px',background:'white',borderRadius:'10px'}}>{t.noRides} {isVerified?'':t.noRidesVerify}</div>}
          <div style={{marginTop:'10px'}}><button onClick={handleDeleteAccount} style={{width:'100%',background:'black',color:'white',padding:'14px',borderRadius:'10px',fontWeight:'bold',border:'none'}}>{t.deleteAccKeep}</button></div>
        </div>}
      </div>
      <div style={{flexShrink:0,background:'white',borderTop:'1px solid #e5e7eb',padding:'8px 12px'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:'8px'}}>
          <div style={{flex:1}}><div style={{fontSize:'12px',fontWeight:'800',color:'#0F4C75'}}>{t.footerTitle} • DropOffPay.co.uk</div><div style={{fontSize:'10px',color:'#333',marginTop:'2px'}}>{t.footerSub}<br/><span style={{color:'#b45309',fontWeight:'700'}}>{t.footerWarn}</span></div></div>
