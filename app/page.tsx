...
export default function FinalAllPanels(){
  ...
  const playSound = (type:'new_order'|'driver_coming'|'driver_arrived_shop'|'driver_at_door'|'order_accepted'|'order_ready')=>{
    ...
    try{
      ...
      // Звук през Web Audio API - работи без файлове!
  ...
  const toggleSound = ()=>{
    ...
    if(newVal){ playSound('order_accepted'); showToast('🔊 Звукът е ВКЛЮЧЕН'); }
  ...
  const subscribeToPush = async ()=>{
    try{
      ...
      // VAPID public key - сложи твоя в .env.local
      const VAPID_PUBLIC_KEY = (process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY as string) || 'BEl62iUYgUiwCo0F0DLdHhP1xGf9W5e8qZ3r2t1y0u9i8o7p6a5s4d3f2g1h0j9k8l7m6n5b4v3c2x1z';
  ...
  const updateOrderStatus = async (orderId:string,status:string)=>{
    ...
    // 🔔 Изпрати PUSH нотификация според статуса
    try{
      ...
      if(orderData){
        ...
        if(status==='on_the_way_to_shop' && shopPhone){
          await sendPushToUser(shopPhone, '🚚 Шофьор идва!', `Шофьор ${orderData.driver_phone||''} идва към магазина за поръчка #${orderId.slice(0,6)}`, orderId);
        }
        if(status==='arrived_at_shop' && shopPhone){
          await sendPushToUser(shopPhone, '📍 Шофьорът е в магазина!', `Шофьорът пристигна! Дай му поръчка #${orderId.slice(0,6)} - ${parseFloat(orderData.total).toFixed(2)}€`, orderId);
        }
        if(status==='picked_up' || status==='on_the_way'){
          await sendPushToUser(orderData.customer_phone, '📦 Поръчката е взета!', `Шофьорът идва към теб! Общо: ${parseFloat(orderData.total).toFixed(2)}€ кеш`, orderId);
        }
        if(status==='arrived_at_client'){
          await sendPushToUser(orderData.customer_phone, '🔔 ШОФЬОРЪТ Е НА ВРАТАТА!', `Излез! Шофьорът те чака с поръчка за ${parseFloat(orderData.total).toFixed(2)}€ кеш`, orderId);
        }
        if(status==='delivered' && shopPhone){
          await sendPushToUser(shopPhone, '✅ Доставена!', `Поръчка #${orderId.slice(0,6)} е доставена успешно!`, orderId);
        }
      }
...
.on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, (payload) => {
        ...
        try{
          ...
          const isMyShopOrder = myShopIds.includes(row.shop_id);
          const isMyClientOrder = row.customer_phone && clean(row.customer_phone) === myPhoneClean;
          const isDriver = myShopProfile?.role==='driver' || isAdmin;
          
          if(payload.eventType==='INSERT'){
            if(isMyShopOrder){
              // Нова поръчка за магазинера
              setTimeout(()=>playSound('new_order'), 300);
            }
            if(isDriver && row.driver_requested && !row.driver_phone){
              // Нова свободна поръчка за шофьор
              if(driverOnline) setTimeout(()=>playSound('new_order'), 300);
            }
          }
