...
export default function FinalAllPanels(){
  ...
  const sortClientsByNearest = async ()=>{
    ...
    try{
      ...
      if(!origin){ 
        // Fallback - ако няма GPS, сортирай по адрес текст (по пощенски код) за да не праща далечния пръв
...
.subscribe(); return ()=>{ supabase.removeChannel(channel); };
  ...
  // AUTO SORT - най-близкия клиент автоматично когато вземеш поръчките от магазина
  useEffect(()=>{
    if(pendingShopOrders.length===0 && pickedUpOrders.length>0){
      // Ако има взети но не са сортирани, или има нови взети - сортирай автоматично
