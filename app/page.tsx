...
export default function ShopsFinal(){
  ...
  // === АВТО НАВИГАЦИЯ - iPhone = Apple Maps, Android = geo: chooser (Google Maps, Waze и т.н.) ===
  const openNavigation = (address:string) => {
    ...
    if(isIOS){
      window.open(`http://maps.apple.com/?daddr=${q}`, '_blank');
    } else {
      window.location.href = `geo:0,0?q=${q}`;
      setTimeout(()=>{ window.open(`https://www.google.com/maps/dir/?api=1&destination=${q}`, '_blank'); }, 700);
    }
  };
  ...
  const handleAuth = async ()=>{
    ...
    try{
      ...
      if(j.codeForTesting) setCodeForTest(j.codeForTesting); else setCodeForTest(code);
    ...
    showToast(`Код изпратен на ${loginForm.email}`);
  };
  ...
  const loadMyShopProfile = async ()=>{
    ...
    if(myProfiles.length===0 && isAdmin){
      ...
      if(allDriverOrders && allDriverOrders.length>0){
        setMyShopOrders(allDriverOrders);
        setMyShopProfile({role:'driver', phone: currentUser.phone, shop_id: null, is_admin_view: true});
        setMyShopProducts([]);
        return;
      }
  ...
  const addShop = async ()=>{
    ...
    if(editShopData){
      ...
      if(driverPhone){
        await supabase.from('shop_profiles').delete().eq('shop_id',editShopData.id).eq('role','driver');
        await supabase.from('shop_profiles').insert([{phone:driverPhone,role:'driver',shop_id:editShopData.id}]);
      }
      showToast(`✅ Магазин ${shopName} променен!`);
  ...
  const placeOrder = async ()=>{ if(!selectedShop || cart.length===0 ||!customerAddr.trim()){ alert('Адрес и количка!'); return; } const items=cart.map(c=>({id:c.product.id,name:c.product.name,price:c.product.price,qty:c.qty,image_url:c.product.image_url})); const total=getCartTotal(); try{ const {error}=await supabase.from('orders').insert({shop_id:selectedShop.id,customer_phone:currentUser.phone,customer_name:`${currentUser.firstName} ${currentUser.lastName}`,address:customerAddr,items,total,status:'new',driver_requested:false}); if(error) throw error; alert('✅ Поръчка изпратена!'); setCart([]); setCustomerAddr(''); await loadCustomerOrders(); }catch(e:any){ alert(e.message); } };
  ...
  const refuseOrder = async (orderId:string)=>{ if(!confirm('Отказваш ли?')) return; await supabase.from('orders').update({driver_phone:null, driver_requested:false, status:'preparing'}).eq('id',orderId); setSortedOrders([]); };
  ...
  useEffect(()=>{
    if(!currentUser?.phone) return;
    const myPhoneClean = clean(currentUser.phone);
    const channelName = `realtime-${myPhoneClean}-${Date.now()}`;
  ...
  .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, (payload) => {
        ...
        setMyShopOrders(prev => {
          ...
          if(payload.eventType === 'DELETE'){ return prev.filter((o:any)=>o.id!==row.id); }
          return prev;
        });
        setSortedOrders([]);
      })
  .on('postgres_changes', { event: '*', schema: 'public', table: 'products' }, (payload) => {
        ...
        if(payload.eventType === 'INSERT' || payload.eventType === 'UPDATE'){
          ...
          if(selectedShop && newRow.shop_id === selectedShop.id){
            setMarketProducts(prev => { const exists = prev.some((p:any)=>p.id===newRow.id); if(newRow.active===false) return prev.filter((p:any)=>p.id!==newRow.id); if(exists) return prev.map((p:any)=> p.id===newRow.id? newRow : p); return [...prev, newRow]; });
          }
        }
      })
  .subscribe();
  ...
  const driverOrders = driverSource.filter((o:any)=> o.driver_requested === true);
  ...
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'#f9fafb',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      ...
      <div style={{display:'flex',gap:'4px',padding:'6px',background:'white',borderBottom:'1px solid #e5e7eb'}}>
        ...
        {myShopProfile?.role==='shop_owner' && <button onClick={()=>setTab('my_shop')} style={{flex:1,padding:'10px',borderRadius:'10px',border:'none',fontWeight:'bold',fontSize:'11px',background:tab==='my_shop'?'#f59e0b':'#f3f4f6',color:tab==='my_shop'?'white':'#666'}}>🏪 Моя ({myShopProducts.length})</button>}
      ...
      <div style={{flex:1,overflowY:'auto',padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
        {tab==='market' && (
          <>
            {!selectedShop? (
              <>
                <div style={{background:'#22c55e',color:'white',padding:'14px',borderRadius:'14px'}}><b>🛒 Пазар - Магазини</b><div style={{fontSize:'11px',marginTop:'4px'}}>LIVE ✅ без рефреш</div><input value={marketSearch} onChange={e=>setMarketSearch(e.target.value)} placeholder="🔍 Търси магазин..." style={{width:'100%',marginTop:'10px',padding:'10px',borderRadius:'10px',border:'none'}}/></div>
              ...
              <>
                ...
                <div style={{background:'#0F4C75',color:'white',padding:'14px',borderRadius:'14px'}}><b>{selectedShop.name}</b><div style={{fontSize:'11px'}}>Доставка {selectedShop.delivery_fee}€ • {selectedShop.phone}</div></div>
        ...
        {tab==='my_orders' && (
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}><b>📦 Моите поръчки LIVE - {customerOrders.length}</b><div style={{fontSize:'10px'}}>Обновява се автоматично - без рефреш</div></div>
            {customerOrders.map((o:any)=>(
              <div key={o.id} style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}>
                <div style={{display:'flex',justifyContent:'space-between'}}><b>#{o.id.slice(0,8)} • {parseFloat(o.total).toFixed(2)}€</b><span style={{fontSize:'10px',background:o.status==='delivered'?'#22c55e':'#f59e0b',color:'white',padding:'4px 8px',borderRadius:'10px'}}>{o.status}</span></div>
                ...
                {o.driver_phone && <div style={{fontSize:'11px',marginTop:'6px',color:'#0F4C75',fontWeight:'bold'}}>🚚 Шофьор: {o.driver_phone} • {o.status}</div>}
              </div>
            ))}
          </div>
        )}
        ...
        {tab==='my_shop' && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#22c55e',color:'white',padding:'14px',borderRadius:'14px'}}><b>🏪 Моят магазин LIVE - ПЪЛЕН ПАНЕЛ</b><div style={{fontSize:'11px'}}>{shops.find(s=>s.id===myShopProfile?.shop_id)?.name||''} • {myShopOrders.length} поръчки • {myShopProducts.length} продукта</div></div>

            <div style={{background:'white',border:'2px solid #22c55e',borderRadius:'14px',padding:'12px'}}>
              <div style={{display:'flex',justifyContent:'space-between'}}><b>⚙️ Настройки магазин</b><button onClick={startEditMyShop} style={{background:'#0F4C75',color:'white',border:'none',padding:'6px 10px',borderRadius:'8px',fontSize:'11px'}}>✏️ Зареди данните</button></div>
              <input placeholder="Име на магазин" value={shopName} onChange={e=>setShopName(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'8px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              ...
              <input placeholder="Такса доставка €" value={shopDeliveryFee} onChange={e=>setShopDeliveryFee(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input placeholder="Телефон магазин" value={ownerPhone} onChange={e=>setOwnerPhone(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <button onClick={updateMyShop} style={{width:'100%',marginTop:'8px',background:'#22c55e',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'bold'}}>💾 Запази магазина</button>
            </div>

            <div style={{background:'white',border:'2px solid #f59e0b',borderRadius:'14px',padding:'12px'}}>
              <b>{editingProduct? '✏️ Редактирай продукт' : '➕ Нов продукт - пълен'}</b>
              <input placeholder="Име продукт" value={newProdName} onChange={e=>setNewProdName(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'8px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <div style={{display:'flex',gap:'6px',marginTop:'6px'}}>
                <input placeholder="Цена €" value={newProdPrice} onChange={e=>setNewProdPrice(e.target.value)} style={{flex:1,padding:'10px',borderRadius:'8px',border:'1px solid #ddd'}}/>
                <select value={newProdCat} onChange={e=>setNewProdCat(e.target.value)} style={{flex:1,padding:'10px',borderRadius:'8px',border:'1px solid #ddd'}}>{PRODUCT_CATS.filter(c=>c!=='Всички').map(c=><option key={c} value={c}>{c}</option>)}</select>
              </div>
              ...
              <label style={{fontSize:'12px',marginTop:'6px',display:'block'}}><input type="checkbox" checked={newProdActive} onChange={e=>setNewProdActive(e.target.checked)}/> Активен в магазина</label>
              <div style={{display:'flex',gap:'6px',marginTop:'10px'}}>
                <button onClick={addProductWithImage} style={{flex:1,background:'#f59e0b',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>{editingProduct? '💾 Обнови продукта' : '✅ Добави продукт'}</button>
                {editingProduct && <button onClick={cancelEditProduct} style={{flex:1,background:'#f3f4f6',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'bold'}}>❌ Откажи</button>}
              </div>
            </div>

            <div style={{background:'white',borderRadius:'14px',padding:'12px'}}>
              <div style={{display:'flex',gap:'6px',marginBottom:'8px'}}>
                <input placeholder="🔍 Търси мой продукт..." value={prodSearch} onChange={e=>setProdSearch(e.target.value)} style={{flex:1,padding:'10px',borderRadius:'8px',border:'1px solid #ddd'}}/>
                <select value={prodCatFilter} onChange={e=>setProdCatFilter(e.target.value)} style={{padding:'10px',borderRadius:'8px',border:'1px solid #ddd'}}>{PRODUCT_CATS.map(c=><option key={c} value={c}>{c}</option>)}</select>
              </div>
              <b>📦 Мои продукти - {filteredMyProducts.length} / {myShopProducts.length}</b>
              {filteredMyProducts.map((p:any)=>(
                <div key={p.id} style={{display:'flex',gap:'8px',alignItems:'center',background:'#f9fafb',padding:'8px',borderRadius:'10px',marginTop:'8px',border:'1px solid #e5e7eb'}}>
                  {p.image_url && <img src={p.image_url} style={{width:'50px',height:'50px',objectFit:'cover',borderRadius:'8px'}}/>}
                  <div style={{flex:1}}><b style={{fontSize:'12px'}}>{p.name}</b><div style={{fontSize:'10px',color:'#666'}}>{p.price}€ • {(p.description||'').replace(/^\[.*?\]\s*/,'')} • {p.active===false?'🔴 Спрян':'🟢 Активен'}</div></div>
                  <button onClick={()=>startEditProduct(p)} style={{background:'#0F4C75',color:'white',border:'none',padding:'6px 8px',borderRadius:'6px',fontSize:'11px'}}>✏️</button>
                  <button onClick={()=>deleteProduct(p.id)} style={{background:'#FF3B30',color:'white',border:'none',padding:'6px 8px',borderRadius:'6px',fontSize:'11px'}}>🗑️</button>
                </div>
              ))}
            </div>
            ...
            <div style={{background:'white',border:'2px solid #22c55e',borderRadius:'14px',padding:'12px'}}>
              ...
              {myShopOrders.map((o:any)=>(
                <div key={o.id} style={{border:'2px solid #0F4C75',borderRadius:'12px',padding:'10px',marginTop:'10px'}}>
                  ...
                  <div style={{fontSize:'11px',marginTop:'4px'}}>{o.customer_name} • <a href={`tel:${o.customer_phone}`} style={{color:'#0F4C75',fontWeight:'bold'}}>{o.customer_phone}</a> • 📍 {o.address}</div>
                  <div style={{fontSize:'10px',background:'#f9fafb',padding:'6px',borderRadius:'6px',marginTop:'6px'}}>{(o.items||[]).map((it:any,i:number)=><div key={i}>{it.name} x{it.qty} - {it.price}€</div>)}</div>
                  <div style={{display:'flex',gap:'6px',marginTop:'8px',flexWrap:'wrap'}}>
                    ...
                    {o.driver_requested && <div style={{flex:1,background:'#e0f2fe',padding:'10px',borderRadius:'8px',textAlign:'center',fontSize:'11px',fontWeight:'bold'}}>🚚 Шофьор: {o.driver_phone||'чака...'}</div>}
                    <button onClick={()=>openNavigation(o.address)} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'10px',borderRadius:'8px',fontWeight:'bold'}}>🧭 До клиента</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {tab==='driver' && (
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}>
              <div style={{display:'flex',justifyContent:'space-between'}}><b>🚚 Доставки LIVE - {driverOrders.length} чакащи - ПЪЛЕН</b><span style={{fontSize:'10px',background:'#22c55e',padding:'4px 8px',borderRadius:'10px'}}>LIVE ✅</span></div>
              <div style={{background:activeDriverCount>=3?'#fee2e2':'#f0fdf4',border:'1px solid #fff',padding:'8px',borderRadius:'8px',marginTop:'8px',color:activeDriverCount>=3?'#991b1b':'#166534'}}><b style={{fontSize:'12px'}}>📦 Лимит: {activeDriverCount}/3 активни</b></div>
              {myShopOrders.length>1 && <button onClick={sortByDistance} disabled={isSorting} style={{width:'100%',marginTop:'8px',background:'#22c55e',color:'white',border:'none',padding:'10px',borderRadius:'10px',fontWeight:'bold'}}>{isSorting?'⏳ Сортирам...':'📍 Сортирай по близост'}</button>}
            </div>
            {availableDriverOrders.length===0 && <div style={{background:'white',padding:'20px',borderRadius:'12px',textAlign:'center',color:'#666'}}>Няма заявки<br/><span style={{fontSize:'11px'}}>Чакаме магазинер да пусне 🚚</span></div>}
              ...
              return (
                <div key={o.id} style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}>
                  ...
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#e0f2fe',padding:'8px',borderRadius:'8px'}}>
                    <b>🏪 {shop?.name || 'Магазин'}</b><br/>📍 {shop?.address || shop?.city}
                  ...
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#fff3cd',padding:'8px',borderRadius:'8px'}}>
                    ...
                    📍 {o.address}
                    <div style={{fontSize:'10px',background:'white',padding:'6px',borderRadius:'6px',marginTop:'6px'}}>{(o.items||[]).slice(0,2).map((it:any,i:number)=><div key={i}>{it.name} x{it.qty}</div>)}{o.items?.length>2?`+${o.items.length-2} още`:''}</div>
                    <button onClick={()=>openNavigation(o.address)} style={{width:'100%',marginTop:'8px',background:'#22c55e',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'900',fontSize:'14px'}}>🧭 До клиента - авто (Apple/Google/Waze)</button>
                  </div>
                  <div style={{display:'flex',gap:'8px',marginTop:'10px',flexWrap:'wrap'}}>
                    {!o.driver_phone && <button onClick={()=>assignDriverToOrder(o.id)} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>🚚 Вземи поръчката</button>}
                    {o.driver_phone && <button onClick={()=>refuseOrder(o.id)} style={{background:'#f3f4f6',color:'#666',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'bold'}}>❌ Откажи</button>}
                    ...
                    {o.driver_phone===currentUser.phone && <button onClick={()=>updateOrderStatus(o.id,'delivered')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>✅ Доставена</button>}
                  </div>
                </div>
              )
            })}
        ...
        {tab==='admin' && isAdmin && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#FFD60A',padding:'14px',borderRadius:'14px',border:'2px solid black'}}><b>👑 Админ LIVE - ПЪЛЕН ПАНЕЛ</b><div style={{fontSize:'11px'}}>Магазини: {shops.length} • Поръчки за шофьор: {myShopOrders.length} • Профили: {shopProfiles.length}</div></div>

            <div style={{background:'white',border:'2px solid black',borderRadius:'14px',padding:'12px'}}>
              <b>{editShopData? '✏️ Редактирай магазин' : '➕ Нов магазин - пълен'}</b>
              <input placeholder="Име на магазин" value={shopName} onChange={e=>setShopName(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'8px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input placeholder="Адрес" value={shopAddress} onChange={e=>setShopAddress(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input placeholder="Град" value={shopCity} onChange={e=>setShopCity(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input placeholder="Такса доставка €" value={shopDeliveryFee} onChange={e=>setShopDeliveryFee(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input placeholder="Телефон собственик" value={ownerPhone} onChange={e=>setOwnerPhone(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'2px solid #22c55e'}}/>
              <input placeholder="Телефон шофьор (по избор)" value={driverPhone} onChange={e=>setDriverPhone(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <div style={{display:'flex',gap:'6px',marginTop:'10px'}}>
                <button onClick={addShop} style={{flex:1,background:'black',color:'#FFD60A',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>{editShopData? '💾 Обнови магазин' : '✅ Създай магазин'}</button>
                {editShopData && <button onClick={cancelEditShop} style={{flex:1,background:'#f3f4f6',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'bold'}}>❌ Откажи</button>}
              </div>
            </div>

            <div style={{background:'white',borderRadius:'14px',padding:'12px'}}>
              <b>🏪 Всички магазини - {shops.length} - пълен контрол</b>
              {shops.map((s:any)=>{ const driverP = shopProfiles.find((p:any)=> p.shop_id===s.id && p.role==='driver'); return (
                <div key={s.id} style={{background:'#f9fafb',padding:'12px',borderRadius:'12px',border:'1px solid #e5e7eb',marginTop:'10px'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                    <div style={{flex:1}}><b>{s.name}</b><div style={{fontSize:'11px',color:'#666'}}>📍 {s.address} • {s.city} • 💶 {s.delivery_fee}€ • {s.vip_active===false?'🔴 Спрян':'🟢 Активен'}</div><div style={{fontSize:'10px',color:'#666'}}>Собственик: {s.phone} {driverP? `• Шофьор: ${driverP.phone}` : '• Няма шофьор'}</div></div>
                    <div style={{display:'flex',gap:'4px',flexDirection:'column'}}>
                      <button onClick={()=>startEditShop(s)} style={{background:'#0F4C75',color:'white',border:'none',padding:'6px 10px',borderRadius:'8px',fontSize:'11px'}}>✏️</button>
                      <button onClick={()=>toggleShopActive(s)} style={{background:s.vip_active===false?'#22c55e':'#f59e0b',color:'white',border:'none',padding:'6px 10px',borderRadius:'8px',fontSize:'11px'}}>{s.vip_active===false?'▶️ Актив':'⏸️ Спри'}</button>
                      <button onClick={()=>deleteShop(s.id)} style={{background:'#FF3B30',color:'white',border:'none',padding:'6px 10px',borderRadius:'8px',fontSize:'11px'}}>🗑️</button>
                    </div>
                  </div>
                  <button onClick={()=>openNavigation(s.address||s.city||'')} style={{width:'100%',marginTop:'6px',background:'#f3f4f6',border:'none',padding:'6px',borderRadius:'6px',fontSize:'11px'}}>🗺️ Виж на карта - авто</button>
                </div>
              )})}
            </div>
          </div>
