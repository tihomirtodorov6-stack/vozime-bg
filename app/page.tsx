...
export default function ShopsFinal(){
  ...
  const handleAuth = async ()=>{
    ...
    setVerificationStep('verify'); setCodeInput(''); setIsSendingCode(false); showToast(`Код изпратен на ${loginForm.email}`);
  };
  const handleVerifyCode = async ()=>{
    ...
    localStorage.setItem('vozime_current',JSON.stringify(u)); setCurrentUser(u); setVerificationStep('form'); setPendingData(null); setCodeInput(''); setCodeForTest(null); setLoginForm({firstName:'',lastName:'',phone:'',email:''}); showToast(`Успешна регистрация, ${newUser.first_name}!`);
  };
  ...
  const loadMarketProducts = async (shopId:string)=>{ const shop = shops.find(s=>s.id===shopId); if(shop && shop.vip_active===false){ showToast('🔴 Магазинът е затворен в момента'); return; } const {data}=await supabase.from('products').select('*').eq('shop_id', shopId); if(data) setMarketProducts(data.filter((p:any)=>p.active!==false)); setSelectedShop(shop); setCart([]); };
  const addShop = async ()=>{
    if(!shopName ||!ownerPhone){ alert('Име и телефон на собственик!'); return; }
    if(editShopData){
      const {error}=await supabase.from('shops').update({ name:shopName, city:shopCity||shopAddress||'', address:shopAddress||'', phone:ownerPhone, delivery_fee: parseFloat(shopDeliveryFee)||4.99, }).eq('id',editShopData.id);
      if(error){ alert(error.message); return; }
      await supabase.from('shop_profiles').delete().eq('shop_id',editShopData.id).eq('role','shop_owner');
      await supabase.from('shop_profiles').insert([{phone:ownerPhone,role:'shop_owner',shop_id:editShopData.id}]);
      if(driverPhone){ await supabase.from('shop_profiles').delete().eq('shop_id',editShopData.id).eq('role','driver'); await supabase.from('shop_profiles').insert([{phone:driverPhone,role:'driver',shop_id:editShopData.id}]); }
      showToast(`✅ Магазин ${shopName} променен!`); setEditShopData(null);
    }else{
      const {data,error}=await supabase.from('shops').insert({ name:shopName, slug:shopName.toLowerCase().replace(/\s+/g,'-')+'-'+Date.now(), city:shopCity||shopAddress||'', address:shopAddress||'', phone:ownerPhone, delivery_fee: parseFloat(shopDeliveryFee)||4.99, vip_active:true }).select().single();
      if(error){ alert(error.message); return; }
      ...
      showToast(`✅ Магазин ${shopName} създаден!`);
    }
    setShopName(''); setShopAddress(''); setShopCity(''); setShopDeliveryFee('4.99'); setOwnerPhone(''); setDriverPhone(''); await loadShops();
  };
  const startEditShop = (shop:any)=>{ setEditShopData(shop); setShopName(shop.name||''); setShopAddress(shop.address||''); setShopCity(shop.city||''); setShopDeliveryFee(String(shop.delivery_fee||'4.99')); setOwnerPhone(shop.phone||''); const driverProfile = shopProfiles.find((p:any)=> p.shop_id===shop.id && p.role==='driver'); setDriverPhone(driverProfile?.phone||''); window.scrollTo({top:0,behavior:'smooth'}); showToast(`✏️ Редактираш ${shop.name}`); };
  ...
  const toggleShopActive = async (shop:any)=>{ await supabase.from('shops').update({vip_active:!shop.vip_active}).eq('id',shop.id); await loadShops(); showToast(shop.vip_active? '🔴 Магазинът е затворен' : '🟢 Магазинът е отворен'); };
  const toggleMyShopOnline = async ()=>{ const myShop = shops.find(s=> s.id===myShopProfile?.shop_id); if(!myShop) return; const newStatus =!myShop.vip_active; await supabase.from('shops').update({vip_active:newStatus}).eq('id',myShop.id); await loadShops(); showToast(newStatus? '🟢 Магазинът е отворен - клиентите могат да поръчват' : '🔴 Магазинът е затворен - поръчките са спрени'); };
  const deleteShop = async (id:string)=>{ if(!confirm('Изтрий магазина?')) return; await supabase.from('shop_profiles').delete().eq('shop_id',id); await supabase.from('products').delete().eq('shop_id',id); await supabase.from('shops').delete().eq('id',id); await loadShops(); if(editShopData?.id===id) cancelEditShop(); };
  const updateMyShop = async ()=>{ if(!myShopProfile?.shop_id){ alert('Нямаш магазин'); return; } if(!shopName ||!shopAddress){ alert('Име и адрес!'); return; } const {error}=await supabase.from('shops').update({ name: shopName, address: shopAddress, city: shopCity||shopAddress, delivery_fee: parseFloat(shopDeliveryFee)||4.99, phone: ownerPhone||myShopProfile.phone }).eq('id',myShopProfile.shop_id); if(error){ alert(error.message); return; } showToast('✅ Моят магазин е променен!'); setShopName(''); setShopAddress(''); setShopCity(''); await loadShops(); await loadMyShopProfile(); };
  const startEditMyShop = ()=>{ const myShop = shops.find(s=> s.id===myShopProfile?.shop_id); if(!myShop) return; setShopName(myShop.name); setShopAddress(myShop.address||''); setShopCity(myShop.city||''); setShopDeliveryFee(String(myShop.delivery_fee||4.99)); setOwnerPhone(myShop.phone||''); };
  const addProductWithImage = async ()=>{ const targetShopId = myShopProfile?.shop_id || shops[0]?.id; if(!targetShopId){ alert('Нямаш магазин!'); return; } if(!newProdName.trim() ||!newProdPrice.trim()){ alert('Име и цена!'); return; } const priceNum=parseFloat(newProdPrice.replace(',','.').replace(/[^\d.]/g,'')); if(isNaN(priceNum)||priceNum<=0){ alert('Грешна цена!'); return; } let imageUrl=editingProduct?.image_url||null; if(newProdFile){ const fname=`${targetShopId}_${Date.now()}_${newProdFile.name}`; const {error}=await supabase.storage.from('product-images').upload(fname,newProdFile); if(error){ alert(error.message); return; } const {data:urlData}=supabase.storage.from('product-images').getPublicUrl(fname); imageUrl=urlData.publicUrl; } const desc=`[${newProdCat}] ${newProdDesc.trim()}`; try{ if(editingProduct){ await supabase.from('products').update({name:newProdName.trim(),price:priceNum,description:desc,image_url:imageUrl,active:newProdActive}).eq('id',editingProduct.id); }else{ await supabase.from('products').insert({shop_id:targetShopId,name:newProdName.trim(),price:priceNum,description:desc,image_url:imageUrl,active:newProdActive}); } }catch(e:any){ alert(e.message); return; } setNewProdName(''); setNewProdPrice(''); setNewProdDesc(''); setNewProdFile(null); setEditingProduct(null); await loadMyShopProfile(); };
  ...
  const addToCart = (product:any)=>{ if(selectedShop?.vip_active===false){ showToast('🔴 Магазинът е затворен'); return; } setCart(prev=>{ const ex=prev.find(c=>c.product.id===product.id); if(ex) return prev.map(c=> c.product.id===product.id? {...c,qty:c.qty+1}:c); return [...prev,{product,qty:1}]; }); };
  ...
  const placeOrder = async ()=>{ if(selectedShop?.vip_active===false){ alert('🔴 Магазинът е затворен в момента и не приема поръчки'); return; } if(!selectedShop || cart.length===0 ||!customerAddr.trim()){ alert('Адрес и количка!'); return; } const items=cart.map(c=>({id:c.product.id,name:c.product.name,price:c.product.price,qty:c.qty,image_url:c.product.image_url})); const total=getCartTotal(); try{ const {error}=await supabase.from('orders').insert({shop_id:selectedShop.id,customer_phone:currentUser.phone,customer_name:`${currentUser.firstName} ${currentUser.lastName}`,address:customerAddr,items,total,status:'new',driver_requested:false}); if(error) throw error; alert('✅ Поръчка изпратена!'); setCart([]); setCustomerAddr(''); await loadCustomerOrders(); }catch(e:any){ alert(e.message); } };
  ...
  const requestDriverForOrder = async (orderId:string)=>{ setSortedOrders([]); await supabase.from('orders').update({driver_requested:true, status:'ready_for_driver'}).eq('id',orderId); setMyShopOrders(prev=> prev.map(o=> o.id===orderId? {...o, driver_requested:true, status:'ready_for_driver'} : o)); showToast('✅ Поиска шофьор!'); };
  ...
  const refuseOrder = async (orderId:string)=>{ if(!confirm('Отказваш ли поръчката?')) return; await supabase.from('orders').update({driver_phone:null, driver_requested:false, status:'preparing'}).eq('id',orderId); setSortedOrders([]); };
  ...
  const sortByDistance = async ()=>{ if(myShopOrders.length<=1) return; setIsSorting(true); try{ const byShop: any = {}; myShopOrders.forEach((o:any)=>{ if(!byShop[o.shop_id]) byShop[o.shop_id]=[]; byShop[o.shop_id].push(o); }); let allSorted:any[]=[]; for(const shopId in byShop){ const shop = shops.find(s=>s.id===shopId); const shopAddr = shop?.address || shop?.city || ''; const shopCoords = await geocodeAddress(shopAddr); if(!shopCoords){ allSorted=[...allSorted,...byShop[shopId]]; continue; } const withDist = await Promise.all(byShop[shopId].map(async (o:any)=>{ const custCoords = await geocodeAddress(o.address); const dist = custCoords? calcDistance(shopCoords,custCoords) : 9999; return {...o,_dist:dist}; })); withDist.sort((a,b)=>a._dist-b._dist); allSorted=[...allSorted,...withDist]; } setSortedOrders(allSorted); }catch{ setSortedOrders(myShopOrders); } setIsSorting(false); };
  const toggleDriverOnline = ()=>{ const newVal=!driverOnline; setDriverOnline(newVal); if(currentUser?.phone){ localStorage.setItem(`vozime_driver_online_${clean(currentUser.phone)}`, String(newVal)); } showToast(newVal? '🟢 Онлайн си - получаваш поръчки' : '🔴 Офлайн си - приключи деня'); };
  ...
  useEffect(()=>{
    if(!currentUser?.phone) return; const myPhoneClean = clean(currentUser.phone); const channelName = `realtime-${myPhoneClean}-${Date.now()}`;
...
.on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, (payload) => {
        ...
        if(row.customer_phone && clean(row.customer_phone) === myPhoneClean){
          ...
          else if(payload.eventType === 'UPDATE'){ setCustomerOrders(prev => prev.map((o:any)=> o.id===row.id? {...o,...newRow} : o)); showToast(`📦 Поръчка #${row.id.slice(0,4)} е ${newRow.status}`); }
          else if(payload.eventType === 'DELETE'){ setCustomerOrders(prev => prev.filter((o:any)=>o.id!==row.id)); }
        }
        setMyShopOrders(prev => {
          const myShopIds = [...new Set(shopProfiles.filter((p:any)=>{ const p10=clean(p.phone); const pFull=cleanFull(p.phone); return p10===myPhoneClean || pFull===cleanFull(currentUser.phone) || p.phone===currentUser.phone; }).map((p:any)=>p.shop_id))];
          ...
          if(payload.eventType === 'INSERT'){ if(!prev.some((o:any)=>o.id===row.id)){ showToast(`🔔 Нова поръчка ${row.id.slice(0,4)}!`); return [row,...prev]; } return prev; }
          if(payload.eventType === 'UPDATE'){ if(!prev.some((o:any)=>o.id===row.id) && row.driver_requested){ showToast(`🚚 Нова заявка за шофьор!`); return [row,...prev]; } return prev.map((o:any)=> o.id===row.id? {...o,...newRow} : o); }
...
.on('postgres_changes', { event: '*', schema: 'public', table: 'products' }, (payload) => {
        ...
        if(payload.eventType === 'INSERT' || payload.eventType === 'UPDATE'){
          setMyShopProducts(prev => { const exists = prev.some((p:any)=>p.id===newRow.id); if(exists) return prev.map((p:any)=> p.id===newRow.id? newRow : p); return [newRow,...prev]; });
          if(selectedShop && newRow.shop_id === selectedShop.id){ setMarketProducts(prev => { const exists = prev.some((p:any)=>p.id===newRow.id); if(newRow.active===false) return prev.filter((p:any)=>p.id!==newRow.id); if(exists) return prev.map((p:any)=> p.id===newRow.id? newRow : p); return [...prev, newRow]; }); }
        }
      })
.on('postgres_changes', { event: '*', schema: 'public', table: 'shops' }, (payload) => {
        const newRow = payload.new as any; if(newRow){ setShops(prev=> prev.map(s=> s.id===newRow.id? newRow : s)); if(selectedShop && selectedShop.id===newRow.id){ setSelectedShop(newRow); if(newRow.vip_active===false){ showToast('🔴 Магазинът затвори'); } } }
      })
.subscribe(); return ()=>{ supabase.removeChannel(channel); };
        ...
        <div style={{background:'white',padding:'20px',borderRadius:'24px',width:'100%',maxWidth:'380px'}}>
          ...
          {verificationStep==='form'? (
            <>
              ...
              <button onClick={handleAuth} disabled={isSendingCode} style={{width:'100%',padding:'16px',background: isSendingCode?'#9ca3af':'#22c55e',border:'none',borderRadius:'14px',fontWeight:'bold',color:'white'}}>{isSendingCode? '⏳ Пращам код...' : loginTab==='register'? '📧 Изпрати код →' : 'Влез →'}</button>
            </>
          ) : (
            <>
              ...
              {codeForTest && (<div style={{background:'#fef3c7',border:'2px solid #f59e0b',borderRadius:'12px',padding:'12px',textAlign:'center',marginBottom:'12px'}}><div style={{fontSize:'10px'}}>ТЕСТ КОД:</div><div style={{fontSize:'24px',fontWeight:'900'}}>{codeForTest}</div></div>)}
  ...
  const filteredMarketProducts = marketProducts.filter(p=>{
    ...
    if(marketCat!=='Всички' &&!(p.description||'').includes(`[${marketCat}]`)) return false; return true;
  });
  const filteredMyProducts = myShopProducts.filter(p=>{
    ...
    if(prodCatFilter!=='Всички' &&!(p.description||'').includes(`[${prodCatFilter}]`)) return false; return true;
  });
  ...
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'#f9fafb',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      <header style={{background:'#0F4C75',color:'white',padding:'10px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div><b>🛒 Магазини {isAdmin?'👑 АДМИН':''}</b><div style={{fontSize:'10px',opacity:0.8}}>{currentUser.firstName} • {currentUser.phone} • LIVE ✅</div></div>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
        {tab==='market' && (
          <>
            {!selectedShop? (
              <>
                <div style={{background:'#22c55e',color:'white',padding:'14px',borderRadius:'14px'}}><b>🛒 Пазар - Магазини LIVE</b><div style={{fontSize:'11px',marginTop:'4px'}}>🟢 Отворени приемат поръчки • 🔴 Затворени се виждат но без поръчки</div><input value={marketSearch} onChange={e=>setMarketSearch(e.target.value)} placeholder="🔍 Търси магазин..." style={{width:'100%',marginTop:'10px',padding:'10px',borderRadius:'10px',border:'none'}}/></div>
                {filteredMarketShops.map((s:any)=>(
                  <div key={s.id} onClick={()=> s.vip_active===false? showToast('🔴 Магазинът е затворен в момента') : loadMarketProducts(s.id)} style={{background:'white',border: s.vip_active===false? '2px solid #FF3B30' : '2px solid #22c55e',borderRadius:'14px',padding:'14px',cursor: s.vip_active===false? 'not-allowed' : 'pointer', opacity: s.vip_active===false? 0.7 : 1}}>
                    <div style={{display:'flex',justifyContent:'space-between'}}><b>{s.name}</b><span style={{fontSize:'10px',background: s.vip_active===false? '#FF3B30' : '#22c55e',color:'white',padding:'4px 8px',borderRadius:'10px'}}>{s.vip_active===false? '🔴 Затворен' : '🟢 Отворен'}</span></div>
                    <div style={{fontSize:'12px',color:'#666',marginTop:'4px'}}>📍 {s.address||s.city||'Без адрес'} • Доставка {s.delivery_fee}€</div>
                    <div style={{fontSize:'12px',fontWeight:'800',marginTop:'8px',color: s.vip_active===false? '#FF3B30' : '#22c55e'}}>{s.vip_active===false? '❌ Затворен - няма поръчки' : 'Виж продукти →'}</div>
                  </div>
                ))}
              </>
            ) : (
              <>
                <button onClick={()=>{setSelectedShop(null); setMarketProducts([]); setCart([]);}} style={{padding:'10px',borderRadius:'10px',border:'1px solid #ddd',background:'white',fontWeight:'bold'}}>← Назад към магазини</button>
                <div style={{background: selectedShop.vip_active===false? '#FF3B30' : '#0F4C75',color:'white',padding:'14px',borderRadius:'14px'}}><b>{selectedShop.name} {selectedShop.vip_active===false? '🔴 ЗАТВОРЕН' : '🟢 ОТВОРЕН'}</b><div style={{fontSize:'11px'}}>Доставка {selectedShop.delivery_fee}€ • {selectedShop.phone} • {selectedShop.vip_active===false? 'Не приема поръчки' : 'Приема поръчки'}</div></div>
                {selectedShop.vip_active===false && <div style={{background:'#fee2e2',border:'2px solid #FF3B30',padding:'14px',borderRadius:'12px',textAlign:'center'}}><b>🔴 Магазинът е затворен в момента</b><div style={{fontSize:'11px',marginTop:'4px'}}>Собственикът е офлайн. Върни се по-късно - магазинът се вижда но поръчки са спрени.</div></div>}
                ...
                {filteredMarketProducts.map((p:any)=>(
                  <div key={p.id} style={{background:'white',borderRadius:'14px',padding:'12px',display:'flex',gap:'12px',border:'1px solid #e5e7eb',opacity: selectedShop.vip_active===false? 0.5 : 1}}>
        ...
        {tab==='my_orders' && (<div style={{display:'flex',flexDirection:'column',gap:'10px'}}><div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}><b>📦 Моите поръчки LIVE - {customerOrders.length}</b></div>{customerOrders.map((o:any)=>(<div key={o.id} style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}><div style={{display:'flex',justifyContent:'space-between'}}><b>#{o.id.slice(0,8)} • {parseFloat(o.total).toFixed(2)}€</b><span style={{fontSize:'10px',background:o.status==='delivered'?'#22c55e':'#f59e0b',color:'white',padding:'4px 8px',borderRadius:'10px'}}>{o.status}</span></div><div style={{fontSize:'11px',color:'#666',marginTop:'4px'}}>{new Date(o.created_at).toLocaleString('bg-BG')} • 📍 {o.address}</div><div style={{background:'#f9fafb',padding:'8px',borderRadius:'8px',marginTop:'8px',fontSize:'12px'}}>{(o.items||[]).map((it:any,i:number)=><div key={i} style={{display:'flex',justifyContent:'space-between'}}><span>{it.name} x{it.qty}</span><span>{(parseFloat(it.price)*(it.qty||1)).toFixed(2)}€</span></div>)}</div>{o.driver_phone && <div style={{fontSize:'11px',marginTop:'6px',color:'#0F4C75',fontWeight:'bold'}}>🚚 Шофьор: {o.driver_phone} • {o.status}</div>}</div>))}</div>)}
        {tab==='my_shop' && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#22c55e',color:'white',padding:'14px',borderRadius:'14px'}}><b>🏪 Моят магазин LIVE - ПЪЛЕН</b><div style={{fontSize:'11px'}}>{myShopData?.name||''} • {myShopData?.vip_active===false? '🔴 Затворен' : '🟢 Отворен'} • {myShopOrders.length} поръчки</div></div>
            <div style={{background: myShopData?.vip_active===false? '#fee2e2' : '#f0fdf4',border: myShopData?.vip_active===false? '2px solid #FF3B30' : '2px solid #22c55e',borderRadius:'14px',padding:'12px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <div><b style={{fontSize:'13px'}}>{myShopData?.vip_active===false? '🔴 Магазинът е ЗАТВОРЕН' : '🟢 Магазинът е ОТВОРЕН'}</b><div style={{fontSize:'10px',marginTop:'2px',color: myShopData?.vip_active===false? '#991b1b' : '#166534'}}>{myShopData?.vip_active===false? 'Клиентите виждат Затворен и НЕ могат да поръчват' : 'Клиентите виждат Отворен и могат да поръчват'}</div></div>
                <button onClick={toggleMyShopOnline} style={{background: myShopData?.vip_active===false? '#22c55e' : '#FF3B30',color:'white',border:'none',padding:'12px 16px',borderRadius:'10px',fontWeight:'800',fontSize:'12px'}}>{myShopData?.vip_active===false? '▶️ Отвори' : '⏸️ Затвори'}</button>
              </div>
            </div>
            <div style={{background:'white',border:'2px solid #22c55e',borderRadius:'14px',padding:'12px'}}>
              <div style={{display:'flex',justifyContent:'space-between'}}><b>⚙️ Настройки</b><button onClick={startEditMyShop} style={{background:'#0F4C75',color:'white',border:'none',padding:'6px 10px',borderRadius:'8px',fontSize:'11px'}}>✏️ Зареди</button></div>
              <input placeholder="Име" value={shopName} onChange={e=>setShopName(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'8px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input placeholder="Адрес" value={shopAddress} onChange={e=>setShopAddress(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input placeholder="Град" value={shopCity} onChange={e=>setShopCity(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input placeholder="Такса" value={shopDeliveryFee} onChange={e=>setShopDeliveryFee(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <button onClick={updateMyShop} style={{width:'100%',marginTop:'8px',background:'#22c55e',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'bold'}}>💾 Запази</button>
            </div>
            <div style={{background:'white',border:'2px solid #f59e0b',borderRadius:'14px',padding:'12px'}}>
              <b>{editingProduct? '✏️ Редактирай' : '➕ Нов продукт'}</b>
              <input placeholder="Име" value={newProdName} onChange={e=>setNewProdName(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'8px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <div style={{display:'flex',gap:'6px',marginTop:'6px'}}><input placeholder="Цена €" value={newProdPrice} onChange={e=>setNewProdPrice(e.target.value)} style={{flex:1,padding:'10px',borderRadius:'8px',border:'1px solid #ddd'}}/><select value={newProdCat} onChange={e=>setNewProdCat(e.target.value)} style={{flex:1,padding:'10px',borderRadius:'8px',border:'1px solid #ddd'}}>{PRODUCT_CATS.filter(c=>c!=='Всички').map(c=><option key={c} value={c}>{c}</option>)}</select></div>
              <input placeholder="Описание" value={newProdDesc} onChange={e=>setNewProdDesc(e.target.value)} style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input type="file" accept="image/*" onChange={e=>setNewProdFile(e.target.files?.[0]||null)} style={{width:'100%',marginTop:'6px'}}/>
              <div style={{display:'flex',gap:'6px',marginTop:'10px'}}><button onClick={addProductWithImage} style={{flex:1,background:'#f59e0b',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>{editingProduct? '💾 Обнови' : '✅ Добави'}</button>{editingProduct && <button onClick={cancelEditProduct} style={{flex:1,background:'#f3f4f6',border:'none',padding:'12px',borderRadius:'10px'}}>❌</button>}</div>
            </div>
            <div style={{background:'white',borderRadius:'14px',padding:'12px'}}>
              <div style={{display:'flex',gap:'6px',marginBottom:'8px'}}><input placeholder="🔍 Търси..." value={prodSearch} onChange={e=>setProdSearch(e.target.value)} style={{flex:1,padding:'10px',borderRadius:'8px',border:'1px solid #ddd'}}/><select value={prodCatFilter} onChange={e=>setProdCatFilter(e.target.value)} style={{padding:'10px',borderRadius:'8px'}}>{PRODUCT_CATS.map(c=><option key={c} value={c}>{c}</option>)}</select></div>
              <b>📦 Продукти - {filteredMyProducts.length}</b>
              {filteredMyProducts.map((p:any)=>(<div key={p.id} style={{display:'flex',gap:'8px',alignItems:'center',background:'#f9fafb',padding:'8px',borderRadius:'10px',marginTop:'8px'}}>{p.image_url && <img src={p.image_url} style={{width:'50px',height:'50px',objectFit:'cover',borderRadius:'8px'}}/>}<div style={{flex:1}}><b style={{fontSize:'12px'}}>{p.name}</b><div style={{fontSize:'10px'}}>{p.price}€ • {p.active===false?'🔴':'🟢'}</div></div><button onClick={()=>startEditProduct(p)} style={{background:'#0F4C75',color:'white',border:'none',padding:'6px 8px',borderRadius:'6px'}}>✏️</button><button onClick={()=>deleteProduct(p.id)} style={{background:'#FF3B30',color:'white',border:'none',padding:'6px 8px',borderRadius:'6px'}}>🗑️</button></div>))}
            </div>
            <div style={{background:'white',border:'2px solid #22c55e',borderRadius:'14px',padding:'12px'}}>
              <b>📦 Поръчки LIVE - {myShopOrders.length}</b>
              {myShopOrders.map((o:any)=>(<div key={o.id} style={{border:'2px solid #0F4C75',borderRadius:'12px',padding:'10px',marginTop:'10px'}}><div style={{display:'flex',justifyContent:'space-between'}}><b>#{o.id.slice(0,8)} • {parseFloat(o.total).toFixed(2)}€</b><span style={{fontSize:'10px',background:'#f59e0b',color:'white',padding:'4px 8px',borderRadius:'8px'}}>{o.status}</span></div><div style={{fontSize:'11px',marginTop:'4px'}}>{o.customer_name} • {o.customer_phone} • 📍 {o.address}</div><div style={{display:'flex',gap:'6px',marginTop:'8px',flexWrap:'wrap'}}>{o.status==='new' && <button onClick={()=>updateOrderStatus(o.id,'accepted')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'10px',borderRadius:'8px'}}>✅ Приеми</button>}{o.status==='accepted' && <button onClick={()=>updateOrderStatus(o.id,'preparing')} style={{flex:1,background:'#f59e0b',color:'white',border:'none',padding:'10px',borderRadius:'8px'}}>👨‍🍳 Готвя</button>}{(o.status==='preparing' || o.status==='accepted') &&!o.driver_requested && <button onClick={()=>requestDriverForOrder(o.id)} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'10px',borderRadius:'8px'}}>🚚 Поискай шофьор</button>}{o.driver_requested && <div style={{flex:1,background:'#e0f2fe',padding:'10px',borderRadius:'8px',textAlign:'center',fontSize:'11px'}}>🚚 {o.driver_phone||'чака...'}</div>}<button onClick={()=>openNavigation(o.address)} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'10px',borderRadius:'8px'}}>🧭 До клиента</button></div></div>))}
            </div>
          </div>
        )}
        {tab==='driver' && (
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <div><b>🚚 Доставки LIVE</b><div style={{fontSize:'10px'}}>{driverOnline? '🟢 Получаваш поръчки' : '🔴 Офлайн'} • {availableDriverOrders.length} активни</div></div>
                <button onClick={toggleDriverOnline} style={{background:driverOnline?'#22c55e':'#FF3B30',color:'white',border:'none',padding:'10px 14px',borderRadius:'12px',fontWeight:'800',fontSize:'12px'}}>{driverOnline? '🟢 ОНЛАЙН' : '🔴 ОФЛАЙН'}</button>
              </div>
              <div style={{background:'white',color:'black',padding:'10px',borderRadius:'10px',marginTop:'10px'}}>
                <div style={{display:'flex',justifyContent:'space-between'}}><b>💰 Днес: {todayDeliveredOrders.length} доставки</b><b style={{color:'#22c55e'}}>{todayEarnings.toFixed(2)}€</b></div>
                <button onClick={()=>{ if(confirm(`Приключваш деня? Днес: ${todayDeliveredOrders.length} - ${todayEarnings.toFixed(2)}€`)){ toggleDriverOnline(); } }} style={{width:'100%',marginTop:'8px',background:'#0F4C75',color:'white',border:'none',padding:'10px',borderRadius:'8px',fontWeight:'bold',fontSize:'12px'}}>🏁 Приключи деня - Офлайн + отчет</button>
              </div>
            </div>
            {!driverOnline && <div style={{background:'#fee2e2',border:'2px solid #FF3B30',padding:'14px',borderRadius:'12px',textAlign:'center'}}><b>🔴 Офлайн си</b><div style={{fontSize:'11px',marginTop:'4px'}}>Не получаваш нови поръчки.</div><div style={{fontSize:'11px',marginTop:'6px',background:'white',padding:'8px',borderRadius:'8px'}}>💰 Днес: <b>{todayEarnings.toFixed(2)}€</b> от {todayDeliveredOrders.length}</div></div>}
            {driverOnline && availableDriverOrders.length===0 && <div style={{background:'white',padding:'20px',borderRadius:'12px',textAlign:'center',color:'#666'}}>Няма активни заявки<br/><span style={{fontSize:'11px'}}>Днес: {todayEarnings.toFixed(2)}€</span></div>}
            {availableDriverOrders.map((o:any)=>{
              ...
              return (
                <div key={o.id} style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}>
                  <div style={{display:'flex',justifyContent:'space-between'}}><b>#{o.id.slice(0,8)} • {parseFloat(o.total).toFixed(2)}€</b><span style={{fontSize:'10px',background:'#0F4C75',color:'white',padding:'4px 8px',borderRadius:'10px'}}>{o.status}{o._dist? ` • ${o._dist.toFixed(1)}км` : ''}</span></div>
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#e0f2fe',padding:'8px',borderRadius:'8px'}}>
                    <b>🏪 {shop?.name || 'Магазин'} {shop?.vip_active===false? '🔴 Затворен' : '🟢'}</b><br/>📍 {shop?.address || shop?.city}
                    <button onClick={()=>openNavigation(shop?.address || shop?.city || '')} style={{width:'100%',marginTop:'6px',background:'#0F4C75',color:'white',border:'none',padding:'10px',borderRadius:'8px',fontWeight:'bold'}}>🗺️ До магазина - авто</button>
                  </div>
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#fff3cd',padding:'8px',borderRadius:'8px'}}>
                    <div style={{display:'flex',justifyContent:'space-between'}}><b>👤 {o.customer_name}</b><a href={`tel:${o.customer_phone}`} style={{color:'#0F4C75',fontWeight:'bold',textDecoration:'none'}}>📞 {o.customer_phone}</a></div>
                    📍 {o.address}
                    <button onClick={()=>openNavigation(o.address)} style={{width:'100%',marginTop:'8px',background:'#22c55e',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'900',fontSize:'14px'}}>🧭 До клиента - авто</button>
                  </div>
                  <div style={{display:'flex',gap:'8px',marginTop:'10px',flexWrap:'wrap'}}>
                    {!o.driver_phone && driverOnline && <button onClick={()=>assignDriverToOrder(o.id)} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>🚚 Вземи поръчката</button>}
                    {(o.status==='ready_for_driver' || o.status==='on_the_way_to_shop') && o.driver_phone===currentUser.phone && <button onClick={()=>refuseOrder(o.id)} style={{background:'#f3f4f6',color:'#666',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'bold'}}>❌ Откажи</button>}
                    {o.driver_phone===currentUser.phone && o.status==='on_the_way_to_shop' && <button onClick={()=>updateOrderStatus(o.id,'picked_up')} style={{flex:1,background:'#8b5cf6',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>📦 Взех от магазина</button>}
                    {o.driver_phone===currentUser.phone && o.status==='picked_up' && <button onClick={()=>updateOrderStatus(o.id,'on_the_way')} style={{flex:1,background:'#f59e0b',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>🚚 Пътувам към клиент</button>}
                    {o.driver_phone===currentUser.phone && o.status==='on_the_way' && <button onClick={()=>updateOrderStatus(o.id,'delivered')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>✅ Доставена - +{(shops.find(s=>s.id===o.shop_id)?.delivery_fee||4.99)}€ - изчезва</button>}
                  </div>
                </div>
              )
            })}
