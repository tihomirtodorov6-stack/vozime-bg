...
export default function FinalAllPanels(){
  ...
  // ПРОСТО - БЕЗ API-та, САМО 1 ФАЙЛ - РАБОТИ В PWA
  const handleAuth = async ()=>{
    ...
    // Регистрация > код > въвеждане > вход
    ...
    try{
      await supabase.from('email_codes').delete().eq('email', emailLow);
      ...
      alert('Грешка: '+e.message+' - пусни SQL-а за email_codes от стъпка 2');
    }
  ...
  const handleVerifyCode = async ()=>{
    ...
    try{
      const {data, error} = await supabase.from('email_codes')
        .select('*')
      ...
      if(!data){ alert('Грешен или изтекъл код'); return; }
                ...
                {availableFree.map((o:any)=>{
                  ...
                  return (
                    <div key={o.id} style={{border: sameShop? '3px solid #22c55e' : '2px solid #0F4C75',borderRadius:'12px',padding:'10px',marginTop:'10px',background: sameShop? '#f0fdf4' : 'white'}}>
                      ...
                      <button onClick={()=>assignDriverToOrder(o.id)} style={{width:'100%',marginTop:'8px',background: sameShop? '#22c55e' : '#0F4C75',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'900'}}>✅ Приеми</button>
                    </div>
                  )
                })}
              </div>
                        ...
                        <div style={{marginTop:'10px'}}>
                          ...
                          {shopOrders.map((o:any)=>(
                            <div key={o.id} style={{border:'3px solid black',borderRadius:'12px',padding:'12px',marginTop:'10px',background:'#f9fafb'}}>
                              <div style={{display:'flex',justifyContent:'space-between'}}><b>#{o.id.slice(0,8)} • {parseFloat(o.total).toFixed(2)}€</b><span style={{fontSize:'10px',background:'#fff3cd',padding:'4px 8px',borderRadius:'6px'}}>{o.address}</span></div>
                              <div style={{display:'flex',gap:'6px',marginTop:'10px'}}>
                                <button onClick={()=>updateOrderStatus(o.id,'picked_up')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'14px',borderRadius:'10px',fontWeight:'900'}}>✅ ВЗЕХ</button>
                  ...
                  return (
                    <>
                      <div style={{background:'white',border:'4px solid #22c55e',borderRadius:'16px',padding:'14px'}}>
                        <div style={{display:'flex',justifyContent:'space-between'}}><b>#{activeClient.id.slice(0,8)} • {parseFloat(activeClient.total).toFixed(2)}€</b><span style={{fontSize:'11px',background:'#22c55e',color:'white',padding:'6px 10px',borderRadius:'20px'}}>{activeClient._dist!==undefined? `${activeClient._dist.toFixed(1)}км` : activeClient.status}</span></div>
                        <div style={{marginTop:'10px',background:'white',border:'3px solid #22c55e',padding:'12px',borderRadius:'10px'}}>
                          <div style={{fontSize:'10px',fontWeight:'900'}}>📍 КЛИЕНТ:</div>
                        ...
                        <button onClick={()=>openNavigation(activeClient.address)} style={{width:'100%',marginTop:'12px',background:'black',color:'#FFD60A',border:'none',padding:'16px',borderRadius:'12px',fontWeight:'900',fontSize:'16px'}}>🧭 НАВИГИРАЙ<br/><span style={{fontSize:'13px',background:'white',color:'black',padding:'6px 10px',borderRadius:'8px',marginTop:'6px',display:'inline-block'}}>{activeClient.address}</span></button>
