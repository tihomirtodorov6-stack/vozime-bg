...
export default function FinalAllPanels(){
  ...
  const sortClientsByNearest = async ()=>{
    ...
    try{
      ...
      if(!origin){ const firstShop = shops.find(s=>s.id===ordersToSort[0].shop_id); origin = await geocodeAddress(firstShop?.address || firstShop?.city || ''); }
      if(!origin){ setSortedClientOrders(ordersToSort); return; }
      const withDist = await Promise.all(ordersToSort.map(async (o:any)=>{ const custCoords = await geocodeAddress(o.address); const dist = custCoords? calcDistance(origin, custCoords) : 9999; return {...o, _dist: dist}; }));
      ...
      <div style={{flex:1,overflowY:'auto',padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
        ...
        {tab==='driver' && (
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                ...
                {availableFree.map((o:any)=>{
                  ...
                  return (
                    <div key={o.id} style={{border: sameShop? '3px solid #22c55e' : '2px solid #0F4C75',borderRadius:'12px',padding:'10px',marginTop:'10px',background: sameShop? '#f0fdf4' : 'white'}}>
                      {sameShop && <div style={{fontSize:'10px',background:'#22c55e',color:'white',padding:'4px 8px',borderRadius:'6px',fontWeight:'900',marginBottom:'6px'}}>⭐ СЪЩИЯ МАГАЗИН КАТО ВЗЕТИТЕ - ВЗЕМИ И ТАЗИ!</div>}
                      <div style={{display:'flex',justifyContent:'space-between'}}><b>#{o.id.slice(0,8)} • {parseFloat(o.total).toFixed(2)}€ • {o.customer_name}</b><span style={{fontSize:'10px',background:'#f59e0b',padding:'4px 8px',borderRadius:'8px'}}>{o.status}</span></div>
                      ...
                      <div style={{fontSize:'11px',marginTop:'6px',background:'#fff3cd',padding:'8px',borderRadius:'8px',border:'2px solid #f59e0b',fontWeight:'800'}}>📍 ПЪЛЕН АДРЕС С НОМЕР - КЛИЕНТ: {o.address}</div>
                      <button onClick={()=>assignDriverToOrder(o.id)} style={{width:'100%',marginTop:'8px',background: sameShop? '#22c55e' : '#0F4C75',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'900'}}>✅ Приеми #{o.id.slice(0,6)} {sameShop? '- от същия магазин!' : ''}</button>
                    </div>
                  )
                })}
              </div>
            ...
            {pendingShopOrders.length>0 && (
              <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                <div style={{background:'black',color:'#FFD60A',padding:'12px',borderRadius:'12px',border:'2px solid #FFD60A'}}><b>🏪 ФАЗА 1: КЪМ МАГАЗИНА - {pendingShopOrders.length} за взимане от {shopIdsForPickup.length} магазина</b></div>
                  ...
                  return (
                    <div key={shopId} style={{background:'white',border:'4px solid black',borderRadius:'16px',padding:'14px'}}>
                      ...
                      <div style={{marginTop:'8px',background:'#e0f2fe',border:'3px solid #0F4C75',padding:'12px',borderRadius:'10px'}}>
                        <div style={{fontSize:'10px',fontWeight:'900'}}>📍 АДРЕС НА МАГАЗИНА - УЛИЦА + НОМЕР:</div>
                      ...
                      {!isArrived ? (
                        <>
                          <button onClick={()=>openNavigation(shop?.address||'')} style={{width:'100%',marginTop:'12px',background:'black',color:'#FFD60A',border:'none',padding:'16px',borderRadius:'12px',fontWeight:'900',fontSize:'16px'}}>
                            🧭 КЪМ МАГАЗИНА {shop?.name} - {shopOrders.length} поръчки<br/><span style={{fontSize:'11px'}}>{shop?.address} - {shopOrders.map((o:any)=>`#${o.id.slice(0,6)}`).join(', ')}</span>
                          </button>
                          <button onClick={()=>setArrivedShops(prev=>({...prev,[shopId]:true}))} style={{width:'100%',marginTop:'8px',background:'white',border:'3px solid black',padding:'14px',borderRadius:'12px',fontWeight:'900'}}>
                            📍 ПРИСТИГНАХ ДО МАГАЗИНА - показвам номерата
                          </button>
                        </>
                      ) : (
                        <div style={{marginTop:'10px'}}>
                          <div style={{background:'#FFD60A',color:'black',padding:'10px',borderRadius:'10px',border:'2px solid black',fontWeight:'900',textAlign:'center'}}>📱 ПОКАЖИ НОМЕРАТА #{shopOrders.map((o:any)=>o.id.slice(0,6)).join(', #')} НА МАГАЗИНЕРА</div>
                          {shopOrders.map((o:any)=>(
                            <div key={o.id} style={{border:'3px solid black',borderRadius:'12px',padding:'12px',marginTop:'10px',background:'#f9fafb'}}>
                              <div style={{display:'flex',justifyContent:'space-between'}}><b style={{fontSize:'16px'}}>#{o.id.slice(0,8)} • {parseFloat(o.total).toFixed(2)}€</b><span style={{fontSize:'10px',background:'#fff3cd',padding:'4px 8px',borderRadius:'6px'}}>📍 {o.address}</span></div>
                              <div style={{fontSize:'11px',marginTop:'6px'}}>👤 {o.customer_name} • {o.customer_phone}<br/>📍 ПЪЛЕН АДРЕС С НОМЕР: {o.address}</div>
                              <div style={{display:'flex',gap:'6px',marginTop:'10px'}}>
                                ...
                                <button onClick={()=>refuseOrder(o.id)} style={{background:'#fee2e2',border:'2px solid #FF3B30',padding:'14px',borderRadius:'10px',fontWeight:'800'}}>❌</button>
                              </div>
                            </div>
                          ))}
                        </div>
            ...
            {pendingShopOrders.length===0 && pickedUpOrders.length>0 && (
              <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                <div style={{background:'black',color:'#FFD60A',padding:'12px',borderRadius:'12px',border:'2px solid #FFD60A',display:'flex',justifyContent:'space-between'}}>
                  ...
                  <button onClick={sortClientsByNearest} style={{background:'#FFD60A',color:'black',border:'none',padding:'6px 10px',borderRadius:'8px',fontWeight:'900',fontSize:'10px'}}>🤖 Подреди по близост</button>
                </div>
                {(() => {
                  ...
                  return (
                    <>
                      <div style={{background:'white',border:'4px solid #22c55e',borderRadius:'16px',padding:'14px'}}>
                        ...
                        <button onClick={()=>openNavigation(activeClient.address)} style={{width:'100%',marginTop:'12px',background:'black',color:'#FFD60A',border:'none',padding:'16px',borderRadius:'12px',fontWeight:'900',fontSize:'16px'}}>
                          🧭 НАВИГИРАЙ КЪМ КЛИЕНТА - най-близкия автоматично<br/><span style={{fontSize:'13px',background:'white',color:'black',padding:'6px 10px',borderRadius:'8px',marginTop:'6px',display:'inline-block'}}>{activeClient.address} - #{activeClient.id.slice(0,6)}</span>
                        </button>
                        <button onClick={()=>showToast('📍 Пристигнал')} style={{width:'100%',marginTop:'8px',background:'white',border:'3px solid black',padding:'12px',borderRadius:'10px',fontWeight:'900'}}>
                          📍 ПРИСТИГНАХ ПРИ КЛИЕНТА - {activeClient.address}
                        </button>
                        <button onClick={async()=>{
                          await updateOrderStatus(activeClient.id,'delivered');
                          await upsertDriverStatus(driverOnline, todayEarnings + parseFloat(shop?.delivery_fee||'4.99'), todayDeliveredOrders.length+1);
                          showToast(queue.length>0? `✅ Завършена! Продължи към #${queue[0].id.slice(0,6)}` : `🏁 Всички доставени! ${(todayEarnings + parseFloat(shop?.delivery_fee||'4.99')).toFixed(2)}€`);
                          if(queue.length>0){ setTimeout(()=>openNavigation(queue[0].address), 600); } else setSortedClientOrders([]);
                        }} style={{width:'100%',marginTop:'8px',background:'#22c55e',color:'white',border:'none',padding:'18px',borderRadius:'14px',fontWeight:'900',fontSize:'18px',border:'3px solid black'}}>
                          ✅ ЗАВЪРШИ ДОСТАВКАТА #{activeClient.id.slice(0,6)} - ВИНАГИ ВИДИМ<br/>
                          <span style={{fontSize:'11px',background:'black',color:'#FFD60A',padding:'4px 8px',borderRadius:'6px',marginTop:'4px',display:'inline-block'}}>{parseFloat(shop?.delivery_fee||'4.99').toFixed(2)}€ отчетени - Днес: {(todayEarnings + parseFloat(shop?.delivery_fee||'4.99')).toFixed(2)}€</span>
                        </button>
                        <div style={{display:'flex',gap:'6px',marginTop:'8px'}}>
                          <button onClick={()=>openNavigation(activeClient.address)} style={{flex:1,background:'white',border:'2px solid black',padding:'10px',borderRadius:'10px',fontWeight:'800'}}>🧭 До клиента с номер</button>
                          <a href={`tel:${activeClient.customer_phone}`} style={{background:'black',color:'#FFD60A',padding:'10px 14px',borderRadius:'10px',textDecoration:'none',fontWeight:'900'}}>📞</a>
                        </div>
                      </div>
                      {queue.length>0 && (
                        <div style={{background:'#f9fafb',border:'2px dashed #666',borderRadius:'12px',padding:'10px'}}>
                          <b>📋 Опашка - остават {queue.length} - Продължи към следваща</b>
            ...
            {myTakenOrders.length===0 && availableFree.length===0 && (
              <div style={{background:'white',padding:'20px',borderRadius:'12px',textAlign:'center'}}>
                <b>Няма активни доставки</b><div style={{fontSize:'11px',marginTop:'6px'}}>Днес: {todayEarnings.toFixed(2)}€ от {todayDeliveredOrders.length} доставки</div>
              </div>
            )}
          </div>
        )}
        ...
        {tab==='admin' && isAdmin && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#FFD60A',padding:'14px',borderRadius:'14px',border:'2px solid black'}}><b>👑 Админ панел - пълен и работещ</b><div style={{fontSize:'11px'}}>Магазини: {shops.length} • Шофьори LIVE: {driverStatuses.filter((d:any)=>d.is_online).length} • Онлайн: {driverStatuses.filter((d:any)=>d.is_online).map((d:any)=>d.phone).join(', ')} • Днес: {todayEarnings.toFixed(2)}€</div></div>
            <div style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}>
              <b>{editShopData? '✏️ Редактирай магазин' : '➕ Нов магазин - админ панел пълен'}</b>
              <input value={shopName} onChange={e=>setShopName(e.target.value)} placeholder="Име магазин HALASTRA" style={{width:'100%',padding:'10px',marginTop:'8px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input value={shopAddress} onChange={e=>setShopAddress(e.target.value)} placeholder="Адрес с номер PO20BQ LONDON TOAD 21-23" style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input value={shopCity} onChange={e=>setShopCity(e.target.value)} placeholder="Град Portsmouth" style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input value={shopDeliveryFee} onChange={e=>setShopDeliveryFee(e.target.value)} placeholder="Доставка € 4.99" style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input value={ownerPhone} onChange={e=>setOwnerPhone(e.target.value)} placeholder="Собственик тел +44..." style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <input value={driverPhone} onChange={e=>setDriverPhone(e.target.value)} placeholder="Шофьор тел +44..." style={{width:'100%',padding:'10px',marginTop:'6px',borderRadius:'8px',border:'1px solid #ddd'}}/>
              <div style={{display:'flex',gap:'6px',marginTop:'8px'}}><button onClick={addShop} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>{editShopData? '💾 Запази' : '➕ Създай магазин'}</button>{editShopData && <button onClick={cancelEditShop} style={{background:'#fee2e2',border:'none',padding:'12px',borderRadius:'10px'}}>Отказ</button>}</div>
            </div>
            <div style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}><b>🚚 Шофьори LIVE - пълен</b>{driverStatuses.map((d:any)=>(<div key={d.clean_phone} style={{display:'flex',justifyContent:'space-between',padding:'10px',borderRadius:'10px',marginTop:'8px',background: d.is_online? '#f0fdf4' : '#fee2e2',border: d.is_online? '2px solid #22c55e' : '1px solid #ddd'}}><div><b>{d.phone}</b><div style={{fontSize:'10px'}}>{d.is_online? '🟢 Онлайн LIVE' : '🔴 Офлайн'} • {d.deliveries_today||0} доставки • {parseFloat(d.earnings_today||0).toFixed(2)}€ днес</div></div><span style={{fontSize:'10px',background: d.is_online? '#22c55e' : '#FF3B30',color:'white',padding:'4px 8px',borderRadius:'10px',fontWeight:'800'}}>{d.is_online? 'ОНЛАЙН' : 'ОФЛАЙН'}</span></div>))}</div>
            <div style={{background:'white',borderRadius:'14px',padding:'12px'}}><b>🏪 Всички магазини - админ пълен - {shops.length}</b>{shops.map((s:any)=>(<div key={s.id} style={{padding:'10px',border:'1px solid #e5e7eb',borderRadius:'10px',marginTop:'8px',background: s.vip_active===false? '#fee2e2' : 'white'}}><div style={{display:'flex',justifyContent:'space-between'}}><b>{s.name} {s.vip_active===false? '🔴 Затворен' : '🟢 Отворен LIVE'}</b><span style={{fontSize:'10px'}}>{s.delivery_fee}€</span></div><div style={{fontSize:'11px',color:'#666'}}>📍 {s.address} • {s.city} - с номер • {s.phone}</div><div style={{display:'flex',gap:'4px',marginTop:'6px'}}><button onClick={()=>startEditShop(s)} style={{background:'#0F4C75',color:'white',border:'none',padding:'6px 10px',borderRadius:'8px'}}>✏️ Редактирай</button><button onClick={()=>toggleShopActive(s)} style={{background:s.vip_active===false?'#22c55e':'#FF3B30',color:'white',border:'none',padding:'6px 10px',borderRadius:'8px',fontWeight:'800'}}>{s.vip_active===false?'▶️ Отвори':'⏸️ Затвори'} LIVE</button><button onClick={()=>deleteShop(s.id)} style={{background:'#FF3B30',color:'white',border:'none',padding:'6px 10px',borderRadius:'8px'}}>🗑️ Изтрий</button></div></div>))}</div>
          </div>
        )}
      </div>
