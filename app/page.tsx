...
export default function ShopsFinal(){
  ...
  const [editShopData,setEditShopData]=useState<any>(null); // СЕГА СЕ ПОЛЗВА!
  ...
  const loadMyShopProfile = async ()=>{
    ...
    if(myProfiles.length===0 && isAdmin){
      // АДМИН НЯМА ДА ПОЛЗВА МОЯ ПАНЕЛ - ВСИЧКО Е В АДМИН ТАБА
  ...
  // === НОВО: АДМИН СЪЗДАВА ИЛИ РЕДАКТИРА МАГАЗИН ===
  const addShop = async ()=>{
  ...
  // МАГАЗИНЕР МОЖЕ ДА СИ РЕДАКТИРА СВОЯ МАГАЗИН
  ...
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'#f9fafb',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      ...
      <div style={{display:'flex',gap:'4px',padding:'6px',background:'white',borderBottom:'1px solid #e5e7eb'}}>
        ...
        {/* МАГАЗИНЕР - САМО АКО Е МАГАЗИНЕР, НЕ АДМИН */}
        ...
        {tab==='driver' && (
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}>
              <div style={{display:'flex',justifyContent:'space-between'}}><b>🚚 Доставки LIVE - {driverOrders.length}</b><span style={{fontSize:'10px',background:'#22c55e',padding:'4px 8px',borderRadius:'10px'}}>LIVE без треперене</span></div>
              <div style={{background:activeDriverCount>=3?'#fee2e2':'#f0fdf4',border:'1px solid #fff',padding:'8px',borderRadius:'8px',marginTop:'8px',color:activeDriverCount>=3?'#991b1b':'#166534'}}>
                <b style={{fontSize:'12px'}}>📦 Лимит: {activeDriverCount}/3 активни</b>
              </div>
            ...
            {availableDriverOrders.map((o:any)=>{
              ...
              return (
                <div key={o.id} style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}>
                  ...
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#e0f2fe',padding:'8px',borderRadius:'8px'}}><b>🏪 {shop?.name}</b><br/>📍 {shop?.address||shop?.city}<br/><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(shop?.address||'')}`} target="_blank" style={{color:'#0F4C75',fontWeight:'bold'}}>🗺️ До магазина</a></div>
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#fff3cd',padding:'8px',borderRadius:'8px'}}><b>👤 {o.customer_name}</b><br/>📍 {o.address}<br/><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(o.address)}`} target="_blank" style={{color:'#b45309',fontWeight:'bold'}}>🗺️ До клиента</a></div>
                  <div style={{display:'flex',gap:'8px',marginTop:'10px'}}>
                    {!o.driver_phone && <button onClick={()=>assignDriverToOrder(o.id)} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>🚚 Вземи</button>}
                    ...
                    {o.driver_phone===currentUser.phone && o.status==='picked_up' && <button onClick={()=>updateOrderStatus(o.id,'on_the_way')} style={{flex:1,background:'#f59e0b',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>🚚 Пътувам</button>}
                    {o.driver_phone===currentUser.phone && (o.status==='on_the_way' || o.status==='picked_up') && <button onClick={()=>updateOrderStatus(o.id,'delivered')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>✅ Завършена</button>}
                  </div>
                </div>
              )
            })}
        ...
        {tab==='admin' && isAdmin && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#FFD60A',padding:'14px',borderRadius:'14px',border:'2px solid black'}}>
              ...
              <div style={{fontSize:'11px',marginTop:'4px'}}>✅ Редактирай адрес, телефон, доставка • ✅ Без Моя панел • ✅ Без треперене • ✅ Само LIVE</div>
            </div>
            ...
            <div style={{background: editShopData ? '#fef3c7' : 'white',border:`3px solid ${editShopData ? '#f59e0b' : '#22c55e'}`,borderRadius:'16px',padding:'14px'}}>
              ...
              <div style={{fontSize:'10px',color:'#666',marginTop:'8px'}}>💡 Сега можеш да променяш адрес, телефон и доставка на всеки магазин - натисни ✏️ до магазина</div>
            </div>

            <div style={{background:'white',borderRadius:'14px',padding:'12px'}}>
              <b>🏪 Всички магазини - {shops.length} - с Редактирай</b>
              {shops.map((s:any)=>{
            ...
            <div style={{background:'white',borderRadius:'14px',padding:'12px',border:'2px solid #0F4C75'}}>
              <b>📊 Статистика - веригата</b>
              <div style={{display:'flex',gap:'8px',marginTop:'10px'}}>
                <div style={{flex:1,background:'#f0fdf4',padding:'10px',borderRadius:'10px',textAlign:'center'}}><b style={{fontSize:'18px',color:'#22c55e'}}>{shops.length}</b><div style={{fontSize:'10px'}}>Магазина</div></div>
                <div style={{flex:1,background:'#e0f2fe',padding:'10px',borderRadius:'10px',textAlign:'center'}}><b style={{fontSize:'18px',color:'#0F4C75'}}>{shopProfiles.filter((p:any)=>p.role==='shop_owner').length}</b><div style={{fontSize:'10px'}}>Собственици</div></div>
                <div style={{flex:1,background:'#fef3c7',padding:'10px',borderRadius:'10px',textAlign:'center'}}><b style={{fontSize:'18px',color:'#f59e0b'}}>{shopProfiles.filter((p:any)=>p.role==='driver').length}</b><div style={{fontSize:'10px'}}>Шофьори</div></div>
              </div>
            </div>
          </div>
        )}
