...
export default function ShopsFinal(){
  ...
  const placeOrder = async ()=>{
    ...
    setCart([]); setCustomerAddr(''); await loadCustomerOrders(); showToast('✅ Поръчка с пълен адрес - улица + номер');
  };
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            ...
            {availableDriverOrders.length>1 && sortedOrders.length===0 && (
              <button onClick={()=>sortAndNavigateToNearest(false)} disabled={isLocating||isSorting} style={{width:'100%',background:'#FFD60A',color:'black',border:'3px solid black',padding:'14px',borderRadius:'14px',fontWeight:'900',fontSize:'14px'}}>
                {isLocating? '📍 Взимам GPS...' : isSorting? '⏳ Сортирам по близост...' : `🤖 Автоматично подреди ${availableDriverOrders.length} поръчки по близост (като Uber) - с улица + номер`}
              </button>
            )}
                  ...
                  return (
                    <>
                      <div style={{background:'black',color:'#FFD60A',padding:'12px',borderRadius:'12px',border:'2px solid #FFD60A'}}>
                        <div style={{display:'flex',justifyContent:'space-between'}}><b>🚀 АКТИВНА ДОСТАВКА - ЮБЕР РЕЖИМ + ПЪЛЕН АДРЕС</b><span style={{fontSize:'11px',background:'#FFD60A',color:'black',padding:'4px 8px',borderRadius:'8px',fontWeight:'900'}}>{availableDriverOrders.length} взети</span></div>
                        {activeOrder._dist!==undefined && <div style={{fontSize:'11px',marginTop:'4px'}}>📍 Най-близък автоматично избран - {activeOrder._dist.toFixed(1)}км • Улица + номер като при магазина</div>}
                        {queue.length>0 && <div style={{fontSize:'10px',marginTop:'4px',opacity:0.8}}>Следва: {queue.map((o:any)=>`#${o.id.slice(0,6)} (${o._dist!==undefined? o._dist.toFixed(1)+'км' : ''}) - ${o.address.slice(0,20)}`).join(' → ')}</div>}
                      </div>
                      <div style={{background:'white',border:'4px solid black',borderRadius:'16px',padding:'14px'}}>
                        ...
                        <div style={{background:'#e0f2fe',padding:'10px',borderRadius:'10px',marginTop:'10px',border:'2px solid #0F4C75'}}>
                          <div style={{fontSize:'10px',color:'#0F4C75',fontWeight:'900'}}>🏪 МАГАЗИН - УЛИЦА + НОМЕР КАТО ПРИМЕР:</div>
                          <div style={{fontSize:'14px',fontWeight:'900',marginTop:'4px'}}>{shop?.name} {shop?.vip_active===false? '🔴 Затворен' : '🟢 Отворен'}</div>
                          <div style={{fontSize:'13px',fontWeight:'800',marginTop:'2px',background:'white',padding:'8px',borderRadius:'8px',border:'2px solid #0F4C75'}}>📍 {shop?.address} • {shop?.city} - С НОМЕР КАТО ИСКАШ</div>
                          <button onClick={()=>openNavigation(shop?.address||'')} style={{width:'100%',marginTop:'8px',background:'#0F4C75',color:'white',border:'none',padding:'10px',borderRadius:'8px'}}>🗺️ До магазина - авто</button>
                        </div>
                        <div style={{background:'#fff3cd',padding:'10px',borderRadius:'10px',marginTop:'10px',border:'3px solid #f59e0b'}}>
                          ...
                          <div style={{marginTop:'8px',background:'white',border:'3px solid black',padding:'12px',borderRadius:'10px'}}>
                            <div style={{fontSize:'10px',color:'#666',fontWeight:'800'}}>📍 ПЪЛЕН АДРЕС НА КЛИЕНТА С НОМЕР - КАТО ПРИ МАГАЗИНА:</div>
                            <div style={{fontSize:'16px',fontWeight:'900',marginTop:'4px',lineHeight:'1.3',color:'black'}}>{activeOrder.address}</div>
                            <div style={{fontSize:'10px',color:'#666',marginTop:'4px'}}>Точно както при магазина: улица + номер + град + пощенски код</div>
                            {!/\d/.test(activeOrder.address) && <div style={{marginTop:'8px',background:'#FF3B30',color:'white',padding:'8px',borderRadius:'8px',fontSize:'12px',fontWeight:'900',textAlign:'center'}}>⚠️ ЛИПСВА НОМЕР! Обади се на клиента за точен номер - трябва да е като при магазина: улица + номер</div>}
                          </div>
                        </div>
                        <div style={{marginTop:'12px'}}>
                          {(activeOrder.status==='ready_for_driver' || activeOrder.status==='on_the_way_to_shop') && (
                            <>
                              ...
                              <button onClick={()=>updateOrderStatus(activeOrder.id,'picked_up')} style={{width:'100%',marginTop:'8px',background:'#8b5cf6',color:'white',border:'none',padding:'14px',borderRadius:'12px',fontWeight:'900'}}>📦 Взех от магазина - Тръгни към клиента #{activeOrder.id.slice(0,6)}<br/><span style={{fontSize:'11px'}}>{activeOrder.address}</span></button>
                            </>
                          )}
                          {activeOrder.status==='picked_up' && (
                            <button onClick={async()=>{ await updateOrderStatus(activeOrder.id,'on_the_way'); openNavigation(activeOrder.address); }} style={{width:'100%',background:'black',color:'#FFD60A',border:'none',padding:'18px',borderRadius:'14px',fontWeight:'900',fontSize:'18px'}}>
                              🧭 Започни пътуване до клиента #{activeOrder.id.slice(0,6)}<br/><span style={{fontSize:'13px',background:'white',color:'black',padding:'4px 8px',borderRadius:'6px',marginTop:'4px',display:'inline-block'}}>{activeOrder.address}</span>
                            </button>
                          )}
                          {activeOrder.status==='on_the_way' && (
                            <button onClick={async()=>{
                              ...
                              if(queue.length>0){
                                showToast(`✅ Доставена! Продължи към следващ клиент #${queue[0].id.slice(0,6)} - ${queue[0].address}`);
                              ...
                              ✅ Доставена - {queue.length>0? `Продължи към следващ клиент #${queue[0].id.slice(0,6)}` : 'Край на доставките'}<br/><span style={{fontSize:'11px'}}>{queue.length>0? queue[0].address : ''}</span>
                            </button>
                          )}
                          <div style={{display:'flex',gap:'6px',marginTop:'8px'}}>
                            <button onClick={()=>openNavigation(activeOrder.address)} style={{flex:1,background:'white',border:'2px solid black',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>🧭 До клиента - {activeOrder.address.slice(0,35)}</button>
                      ...
                      {queue.length>0 && (
                        <div style={{background:'#f9fafb',border:'2px dashed #666',borderRadius:'12px',padding:'10px'}}>
                          <b style={{fontSize:'12px'}}>📋 Опашка - остават {queue.length} (автоматично по близост с пълен адрес)</b>
                          {queue.map((o:any,i:number)=>(
                            <div key={o.id} style={{background:'white',padding:'8px',borderRadius:'8px',marginTop:'6px',border:'1px solid #e5e7eb'}}>
                              ...
                              <div style={{fontSize:'11px',marginTop:'4px',background:'#fff3cd',padding:'6px',borderRadius:'6px',border:'2px solid #f59e0b',fontWeight:'800'}}>📍 {o.address} - С НОМЕР КАТО ПРИ МАГАЗИНА</div>
                            </div>
                          ))}
                        </div>
                      )}
            ...
            {availableDriverOrders.length===0 && <div style={{background:'white',padding:'20px',borderRadius:'12px',textAlign:'center',color:'#666'}}>Няма активни поръчки • Днес: {todayEarnings.toFixed(2)}€ от {todayDeliveredOrders.length} доставки • Адреса вече е с номер като при магазина</div>}
          </div>
        )}
        {tab==='admin' && isAdmin && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#FFD60A',padding:'14px',borderRadius:'14px',border:'2px solid black'}}><b>👑 Админ LIVE - ВСИЧКИ ЕКСТРИ</b><div style={{fontSize:'11px'}}>Магазини: {shops.length} • Отворени LIVE: {shops.filter(s=>s.vip_active!==false).length} • Шофьори LIVE: {driverStatuses.filter((d:any)=>d.is_online).length} / {driverStatuses.length} • Продукти: {myShopProducts.length} • Поръчки: {myShopOrders.length}</div></div>
            <div style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}>
              <b>🚚 Шофьори LIVE - без рефреш {driverStatuses.length} - с пълен адрес с номер</b>
