...
export default function ShopsFinal(){
  ...
  const loadMyShopProfile = async ()=>{
    ...
    if(!allProfiles){ setMyShopProfile(null); setShopProfiles([]); return; }
    ...
    // АДМИН НЕ Е МАГАЗИН - НЕ ЗАРЕЖДАМЕ ПРОДУКТИ КАТО "МОЯ"
    ...
    if(!ownerProfile){
      // Ако е само шофьор, не е собственик
      setMyShopProfile(null);
      setMyShopProducts([]);
      // Поръчките за шофьор ги зареждаме отделно
    ...
    const shopIds = [...new Set(myProfiles.filter((p:any)=>p.role==='shop_owner').map((p:any)=>p.shop_id))];
  ...
  const addProductWithImage = async ()=>{
    if(!isShopOwner){ alert('Само собственик на магазин може да добавя стоки! Ти си админ.'); return; }
    const targetShopId = myShopProfile?.shop_id;
    if(!targetShopId){ alert('Нямаш магазин! Ти си админ, не собственик.'); return; }
  ...
  const startEditProduct = (p:any)=>{ 
    if(!isShopOwner){ alert('Админа не може да редактира цени!'); return; }
    setEditingProduct(p); setNewProdName(p.name); setNewProdPrice(p.price.toString()); setNewProdDesc((p.description||'').replace(/^\[.*?\]\s*/,'').trim()); const m=p.description?.match(/^\[(.*?)\]/); if(m) setNewProdCat(m[1]); setNewProdActive(p.active!==false); window.scrollTo({top:0,behavior:'smooth'}); 
  };
  const cancelEditProduct = ()=>{ setEditingProduct(null); setNewProdName(''); setNewProdPrice(''); setNewProdDesc(''); setNewProdFile(null); };
  const deleteProduct = async (id:string)=>{ 
    if(!isShopOwner){ alert('Админа не може да трие стоки!'); return; }
    if(!confirm('Изтрий?')) return; await supabase.from('products').delete().eq('id',id); setMyShopProducts(myShopProducts.filter(p=>p.id!==id)); 
  };
    ...
    return (
      <main style={{position:'fixed',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'#0F4C75',padding:'16px'}}>
        <div style={{background:'white',padding:'20px',borderRadius:'24px',width:'100%',maxWidth:'380px'}}>
          ...
          <div style={{textAlign:'center',fontSize:'12px',color:'#666',marginTop:'6px'}}>Админ - без редакция на цени</div>
          ...
          {verificationStep==='form' ? (
            <>
              ...
              <button onClick={handleAuth} disabled={isSendingCode} style={{width:'100%',padding:'16px',background: isSendingCode?'#9ca3af':'#22c55e',border:'none',borderRadius:'14px',fontWeight:'bold',color:'white'}}>
                {isSendingCode ? '⏳ Пращам код...' : loginTab==='register' ? '📧 Изпрати код →' : 'Влез →'}
              </button>
            </>
          ) : (
            <>
              ...
              {codeForTest && (
                <div style={{background:'#fef3c7',border:'2px solid #f59e0b',borderRadius:'12px',padding:'12px',textAlign:'center',marginBottom:'12px'}}>
                  <div style={{fontSize:'10px'}}>ТЕСТ КОД:</div>
  ...
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'#f9fafb',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      <header style={{background:'#0F4C75',color:'white',padding:'10px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div><b>🛒 Магазини {isAdmin?'👑 АДМИН':''}</b><div style={{fontSize:'10px',opacity:0.8}}>{currentUser.firstName} • {currentUser.phone} {isAdmin?'👑 Админ - не пипа цени':''}</div></div>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
        ...
        {tab==='market' && (
          <>
            {!selectedShop ? (
              <>
                <div style={{background:'#22c55e',color:'white',padding:'14px',borderRadius:'14px'}}><b>🛒 Пазар - {isAdmin?'Всички магазини (само гледаш)':'Магазини'}</b><div style={{fontSize:'11px',marginTop:'4px'}}>{isAdmin?'Админа не пипа цени - само пуска/спира магазини от Админ таба':''}</div><input value={marketSearch} onChange={e=>setMarketSearch(e.target.value)} placeholder="🔍 Търси магазин..." style={{width:'100%',marginTop:'10px',padding:'10px',borderRadius:'10px',border:'none'}}/></div>
        ...
        {tab==='my_shop' && !isShopOwner && (
          <div style={{background:'white',padding:'20px',borderRadius:'14px',textAlign:'center'}}>
            <b>⛔ Ти си админ, не собственик на магазин</b><div style={{fontSize:'12px',color:'#666',marginTop:'8px'}}>Админа не може да пипа цени и стоки. Отиди в 👑 Админ таба за да управляваш магазините (пускаш/спираш).</div>
          </div>
        )}
        ...
        {tab==='admin' && isAdmin && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#FFD60A',padding:'12px',borderRadius:'12px'}}><b>👑 Админ панел - Ти си админ, не собственик</b><div style={{fontSize:'11px'}}>Тук само създаваш магазини и пускаш/спираш. НЕ пипаш цени - това го прави собственика от "Моя" таба.</div></div>
            ...
            {shops.map((s:any)=>(
              <div key={s.id} style={{background:'white',padding:'12px',borderRadius:'12px',border:'1px solid #ddd'}}>
                <div style={{display:'flex',justifyContent:'space-between'}}><div><b>{s.name}</b><div style={{fontSize:'11px'}}>{s.address||s.city} • {s.phone}</div></div><div style={{display:'flex',gap:'4px'}}><button onClick={()=>toggleShopActive(s)} style={{background:s.vip_active===false?'#22c55e':'#FF3B30',color:'white',border:'none',padding:'6px 10px',borderRadius:'6px',fontSize:'10px'}}>{s.vip_active===false?'ПУСНИ':'СПРИ'}</button><button onClick={()=>deleteShop(s.id)} style={{background:'black',color:'white',border:'none',padding:'6px 10px',borderRadius:'6px',fontSize:'10px'}}>🗑️</button></div></div>
              </div>
            ))}
          </div>
        )}
