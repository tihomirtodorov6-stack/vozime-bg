...
export default function ShopsOnly(){
  ...
  const addShop = async ()=>{
    ...
    const {data,error}=await supabase.from('shops').insert({name:shopName,slug:shopName.toLowerCase().replace(/\s+/g,'-')+'-'+Date.now(),city:shopAddress||'',phone:ownerPhone,delivery_fee:4.99,vip_active:true,address:shopAddress||''}).select().single();
    if(error){ alert(error.message); return; }
    ...
    setShopName(''); setOwnerPhone(''); setDriverPhone(''); setShopAddress(''); await loadShops();
  };
  ...
  const updateShopInfo = async (shop:any)=>{
    ...
    await supabase.from('shops').update({name:edit.name||shop.name,delivery_fee:fee,phone:edit.ownerPhone||shop.phone}).eq('id',shop.id);:4.99,vip_active:true}).select().single();
    if(error){ alert(error.message); return; }
    await supabase.from('shop_profiles').insert([{phone:ownerPhone,role:'shop_owner',shop_id:data.id}]);
    if(driverPhone) await supabase.from('shop_profiles').insert([{phone:driverPhone,role:'driver',shop_id:data.id}]);
    setShopName(''); setOwnerPhone(''); setDriverPhone(''); setShopAddress(''); await loadShops();
  ...
  const requestDriverForOrder = async (orderId:string)=>{
    // Магазинара иска шофьор за тази поръчка
    await supabase.from('orders').update({driver_requested:true, status:'ready_for_driver'}).eq('id',orderId);
    alert('✅ Поиска шофьор! Поръчката отиде при шофьорите.');
    await loadMyShopProfile();
  };

  const assignDriverToOrder = async (orderId:string)=>{
    // Шофьора взема поръчката - проверка за лимит 3
    const activeOrders = myShopOrders.filter((o:any)=> o.driver_phone===currentUser.phone && ['on_the_way_to_shop','picked_up','on_the_way'].includes(o.status));
    if(activeOrders.length>=3){
      alert('⛔ Лимит 3 поръчки! Завърши поне една от текущите 3 поръчки, за да вземеш нова.\n\nТекущи активни: '+activeOrders.length);
      return;
    }
  ...
  const refuseOrder = async (orderId:string)=>{
    if(!confirm('Отказваш ли тази поръчка? Тя ще се върне при магазина и друг шофьор може да я вземе.')) return;
    await supabase.from('orders').update({driver_phone:null, driver_requested:false, status:'preparing'}).eq('id',orderId);
    await loadMyShopProfile();
  };

  const canTakeMoreOrders = ()=>{
    const active = myShopOrders.filter((o:any)=> o.driver_phone===currentUser.phone && ['on_the_way_to_shop','picked_up','on_the_way'].includes(o.status));
    return active.length < 3;
  };

  // Distance sorting for driver - closest to shop first
  const [sortedByDistance, setSortedByDistance] = useState<any[]>([]);
  ...
  const geocodeAddress = async (addr:string): Promise<{lat:number,lon:number}|null>=>{
    try{
      const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(addr)}&limit=1&countrycodes=bg,gb`);
  ...
  const calcDistance = (a:{lat:number,lon:number}, b:{lat:number,lon:number})=>{
  ...
  const sortOrdersByDistanceFromShop = async ()=>{
    if(myShopOrders.length<=1){ setSortedByDistance(myShopOrders); return; }
    setIsSortingByDistance(true);
    try{
      // Group by shop
      const byShop: {[key:string]: any[]} = {};
      myShopOrders.forEach(o=>{ const sid=o.shop_id; if(!byShop[sid]) byShop[sid]=[]; byShop[sid].push(o); });
      ...
      for(const shopId in byShop){
        ...
        if(!shopCoords){
          // If can't geocode shop, keep original order
          allSorted = [...allSorted, ...byShop[shopId]];
          continue;
        }

        // Geocode each customer address and calculate distance from shop
        const withDist = await Promise.all(byShop[shopId].map(async (o:any)=>{
          ...
          return {...o, _distFromShop:dist, _shopName:shop?.name, _shopAddr:shopAddr};
        }));
        
        // Sort by distance - closest first
        withDist.sort((a,b)=> a._distFromShop - b._distFromShop);
        allSorted = [...allSorted, ...withDist];
      }
      
      setSortedByDistance(allSorted);
    }catch(e){
      console.error(e);
      setSortedByDistance(myShopOrders);
    }
    setIsSortingByDistance(false);
  };

  // Auto-sort when orders change if driver has multiple orders from same shop
  useEffect(()=>{
    if(myShopOrders.length>1){
      // Check if driver has 2+ orders from same shop
      const counts: {[k:string]: number} = {};
      myShopOrders.forEach(o=>{ counts[o.shop_id]=(counts[o.shop_id]||0)+1; });
      const hasMultipleFromSameShop = Object.values(counts).some(c=>c>=2);
      if(hasMultipleFromSameShop){
        // Don't auto-sort, let driver click button
      }
    }
  },[myShopOrders]);
            ...
            {customerOrders.map((o:any)=>{
              ...
              return (
                <div key={o.id} style={{background:'white',border:`2px solid ${color}`,borderRadius:'14px',padding:'12px'}}>
                  <div style={{display:'flex',justifyContent:'space-between'}}><b style={{fontSize:'13px'}}>{shops.find(s=>s.id===o.shop_id)?.name||'Магазин'} • {parseFloat(o.total).toFixed(2)}€</b><span style={{fontSize:'10px',background:color,color:'white',padding:'4px 8px',borderRadius:'10px',fontWeight:'bold'}}>{text}</span></div>
                ...
                return (
                  <div key={o.id} style={{background:'white',border:`2px solid ${col}`,borderRadius:'14px',padding:'12px',marginBottom:'12px'}}>
                    ...
                    <div style={{background:'#f9fafb',padding:'10px',borderRadius:'10px',marginTop:'10px'}}>
                      ...
                      <div style={{display:'flex',justifyContent:'space-between',fontWeight:'800',marginTop:'8px',borderTop:'2px solid #e5e7eb',paddingTop:'8px'}}><span>Общо:</span><span style={{color:'#0F4C75',fontSize:'14px'}}>{parseFloat(o.total).toFixed(2)}€</span></div>
                    </div>
                    <div style={{display:'flex',gap:'8px',marginTop:'12px'}}>
                      ...
                      {(o.status==='preparing' || o.status==='accepted') && !o.driver_requested && (
                        <button onClick={()=>requestDriverForOrder(o.id)} style={{width:'100%',background:'#0F4C75',color:'white',border:'2px dashed #22c55e',padding:'12px',borderRadius:'10px',fontWeight:'800',fontSize:'12px',marginTop:'8px'}}>
                          🚚 Поискай шофьор за доставка<br/><span style={{fontSize:'10px',opacity:0.8}}>Само тогава отива при шофьорите</span>
                        </button>
                      )}
                      ...
                      {o.status==='on_the_way' && <button onClick={()=>updateOrderStatus(o.id,'delivered')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>✅ Завършена поръчка</button>}
                    </div>
                  </div>
                )
              })}
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <b>🚚 Моите доставки LIVE - {myShopOrders.length}</b>
                <span style={{fontSize:'10px',background:'#22c55e',padding:'4px 8px',borderRadius:'10px'}}>🔴 LIVE</span>
              </div>
              {myShopOrders.length>1 && (
                <div style={{marginTop:'10px',display:'flex',gap:'8px'}}>
                  <button onClick={sortOrdersByDistanceFromShop} disabled={isSortingByDistance} style={{flex:1,background:isSortingByDistance?'#666':'#22c55e',color:'white',border:'none',padding:'10px',borderRadius:'10px',fontWeight:'800',fontSize:'12px'}}>
                    {isSortingByDistance?'⏳ Сортирам по разстояние...':'📍 Сортирай по близост до магазина'}
                  </button>
                  {sortedByDistance.length>0 && <button onClick={()=>setSortedByDistance([])} style={{background:'white',color:'#0F4C75',border:'none',padding:'10px 14px',borderRadius:'10px',fontWeight:'800',fontSize:'11px'}}>✕ Оригинален ред</button>}
                </div>
              )}
              <div style={{fontSize:'10px',marginTop:'8px',opacity:0.8}}>
                Ако вземеш 2 поръчки от един магазин, цъкаш бутона и ще ти подреди коя е най-близо до магазина първо.
              </div>
            </div>
            <div style={{background:'#e0f2fe',padding:'10px',borderRadius:'10px',border:'1px solid #90caf9',fontSize:'11px'}}>
              <b>🔄 Workflow:</b> Нова → Приета от магазина → Пътувам към магазина → Взех поръчката → Пътувам към клиента → Доставена
            </div>
            {(()=>{
              const allDriverOrders = (sortedByDistance.length>0 ? sortedByDistance : myShopOrders).filter((o:any)=> o.driver_requested);
              const activeCount = myShopOrders.filter((o:any)=> o.driver_phone===currentUser.phone && ['on_the_way_to_shop','picked_up','on_the_way','ready_for_driver'].includes(o.status)).length;
              const availableOrders = allDriverOrders.filter((o:any)=> !o.driver_phone || o.driver_phone===currentUser.phone);
              const myActiveOrders = allDriverOrders.filter((o:any)=> o.driver_phone===currentUser.phone);
              return (
                <>
                  <div style={{background:activeCount>=3?'#fee2e2':'#f0fdf4',border:`2px solid ${activeCount>=3?'#FF3B30':'#22c55e'}`,padding:'12px',borderRadius:'12px',marginBottom:'12px'}}>
                    <div style={{display:'flex',justifyContent:'space-between'}}>
                      <b style={{fontSize:'13px'}}>📦 Лимит: {activeCount}/3 активни</b>
                      <span style={{fontSize:'11px',background:activeCount>=3?'#FF3B30':'#22c55e',color:'white',padding:'4px 8px',borderRadius:'10px',fontWeight:'800'}}>{activeCount>=3?'⛔ ЛИМИТ':'✅ Можеш'}</span>
                    </div>
                    <div style={{fontSize:'11px',marginTop:'6px',color:'#666'}}>
                      {activeCount>=3 ? '⛔ Лимит 3 достигнат! Завърши поръчка за да вземеш нова.' : `Можеш още ${3-activeCount} поръчки. Само поръчки където магазинара е поискал шофьор.`}
                    </div>
                    {myActiveOrders.length>0 && <div style={{marginTop:'8px',background:'white',padding:'8px',borderRadius:'8px',fontSize:'11px'}}><b>Твои активни ({myActiveOrders.length}):</b> {myActiveOrders.map((oo:any)=>`#${oo.id.slice(0,4)}`).join(', ')}</div>}
                  </div>
                  {availableOrders.length===0 && <div style={{textAlign:'center',padding:'20px',background:'white',borderRadius:'12px',color:'#666'}}>Няма поръчки с поискан шофьор. Чакай магазинара да натисне "Поискай шофьор".</div>}
                  {availableOrders.map((o:any)=>{
            
              const shop = shops.find(s=>s.id===o.shop_id);
              const shopAddr = shop?.address || shop?.city || (shop as any)?.address || 'Адрес не е въведен - добави в Админ панела';
              let col='#f59e0b'; let statusText='🆕 Нова';
              if(o.status==='accepted'){ col='#22c55e'; statusText='✅ Приета от магазина'; }
              else if(o.status==='preparing'){ col='#f59e0b'; statusText='👨‍🍳 Приготвя се'; }
              else if(o.status==='on_the_way_to_shop'){ col='#0F4C75'; statusText='🚚 Пътувам към магазина'; }
              else if(o.status==='picked_up'){ col='#8b5cf6'; statusText='📦 Взех поръчката'; }
              else if(o.status==='on_the_way'){ col='#0F4C75'; statusText='🚚 Пътувам към клиента'; }
              else if(o.status==='delivered'){ col='#22c55e'; statusText='✅ Доставена'; }
              else if(o.status==='new'){ col='#FF3B30'; statusText='🆕 Нова поръчка'; }
              return (
                <div key={o.id} style={{background:'white',border:`2px solid ${col}`,borderRadius:'14px',padding:'12px'}}>
                  <div style={{display:'flex',justifyContent:'space-between'}}><b>#{o.id.slice(0,8)} • {parseFloat(o.total).toFixed(2)}€</b><span style={{fontSize:'10px',background:col,color:'white',padding:'4px 8px',borderRadius:'10px',fontWeight:'800'}}>{statusText}</span></div>
                  <div style={{fontSize:'12px',marginTop:'6px'}}><b>{o.customer_name}</b> • {o.customer_phone}</div>
                  
                  <div style={{fontSize:'11px',background:'#e0f2fe',padding:'10px',borderRadius:'10px',marginTop:'10px',border:'2px solid #0F4C75'}}>
                    <div style={{fontWeight:'800',fontSize:'11px',color:'#0F4C75',marginBottom:'4px'}}>🏪 ВЗЕМИ ОТ МАГАЗИН:</div>
                    <div style={{fontWeight:'bold',fontSize:'13px'}}>{shop?.name||'Магазин'}</div>
                    <div style={{marginTop:'4px'}}>📍 <b>Адрес:</b> {shopAddr}</div>
                    <div>📞 <b>Тел:</b> {shop?.phone||''}</div>
                    <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(shopAddr)}`} target="_blank" style={{display:'block',marginTop:'8px',background:'#0F4C75',color:'white',padding:'10px',borderRadius:'10px',textAlign:'center',textDecoration:'none',fontWeight:'800',fontSize:'12px'}}>🗺️ Навигация до магазина</a>
                  </div>

                  <div style={{fontSize:'11px',background:'#fff3cd',padding:'10px',borderRadius:'10px',marginTop:'10px',border:'2px solid #f59e0b'}}>
                    <div style={{fontWeight:'800',fontSize:'11px',color:'#b45309',marginBottom:'4px'}}>👤 ДОСТАВИ НА КЛИЕНТ:</div>
                    <div>📍 <b>Адрес:</b> {o.address}</div>
                    <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(o.address)}`} target="_blank" style={{display:'block',marginTop:'8px',background:'#b45309',color:'white',padding:'10px',borderRadius:'10px',textAlign:'center',textDecoration:'none',fontWeight:'800',fontSize:'12px'}}>🗺️ Навигация до клиента</a>
                  </div>

                  <div style={{background:'#f9fafb',padding:'10px',borderRadius:'10px',marginTop:'10px',fontSize:'11px'}}>
                    <div style={{fontWeight:'800',fontSize:'11px',marginBottom:'6px'}}>🛒 Продукти за вземане:</div>
                    {(o.items||[]).map((it:any,i:number)=><div key={i} style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:i<(o.items.length-1)?'1px solid #e5e7eb':'none'}}><span><b>{it.name}</b> x{it.qty||1}</span><span>{(parseFloat(it.price)*(it.qty||1)).toFixed(2)}€</span></div>)}
                    <div style={{display:'flex',justifyContent:'space-between',fontWeight:'800',marginTop:'8px',borderTop:'2px solid #e5e7eb',paddingTop:'8px'}}><span>Общо за клиента:</span><span style={{color:'#0F4C75'}}>{parseFloat(o.total).toFixed(2)}€</span></div>
                  </div>

                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginTop:'12px'}}>
                    <a href={`tel:${o.customer_phone}`} style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'10px',textAlign:'center',textDecoration:'none',fontWeight:'800',fontSize:'12px'}}>📞 Клиент</a>
                    <a href={`tel:${shop?.phone||''}`} style={{background:'white',border:'2px solid #0F4C75',color:'#0F4C75',padding:'12px',borderRadius:'10px',textAlign:'center',textDecoration:'none',fontWeight:'800',fontSize:'12px'}}>🏪 Магазин</a>
                  </div>

                  <div style={{display:'flex',flexDirection:'column',gap:'8px',marginTop:'12px'}}>
                    {(o.status==='ready_for_driver' || o.status==='new' || o.status==='accepted' || o.status==='preparing') && !o.driver_phone && (
                      <div style={{display:'flex',gap:'8px'}}>
                        <button onClick={()=>assignDriverToOrder(o.id)} disabled={myShopOrders.filter((oo:any)=> oo.driver_phone===currentUser.phone && ['on_the_way_to_shop','picked_up','on_the_way'].includes(oo.status)).length>=3} style={{flex:1,background:myShopOrders.filter((oo:any)=> oo.driver_phone===currentUser.phone && ['on_the_way_to_shop','picked_up','on_the_way'].includes(oo.status)).length>=3?'#9ca3af':'#0F4C75',color:'white',border:'none',padding:'14px',borderRadius:'12px',fontWeight:'800',fontSize:'13px'}}>
                          🚚 Вземи поръчката<br/><span style={{fontSize:'10px',opacity:0.8}}>Ще пътуваш към магазина {myShopOrders.filter((oo:any)=> oo.driver_phone===currentUser.phone && ['on_the_way_to_shop','picked_up','on_the_way'].includes(oo.status)).length>=3?'⛔ Лимит 3':''}</span>
                        </button>
                        <button onClick={()=>refuseOrder(o.id)} style={{background:'#f3f4f6',border:'1px solid #ddd',padding:'14px',borderRadius:'12px',fontWeight:'800',fontSize:'12px'}}>❌ Откажи</button>
                      </div>
                    )}
                    {(o.status==='new' || o.status==='accepted' || o.status==='preparing') && o.driver_phone && (
                      <button onClick={()=>updateOrderStatus(o.id,'on_the_way_to_shop')} style={{width:'100%',background:'#0F4C75',color:'white',border:'none',padding:'14px',borderRadius:'12px',fontWeight:'800',fontSize:'13px'}}>
                        🚚 Пътувам към магазина<br/><span style={{fontSize:'10px',opacity:0.8}}>Отивам да взема поръчката от {shop?.name||'магазина'}</span>
                      </button>
                    )}
                    {o.status==='on_the_way_to_shop' && (
                      <button onClick={()=>updateOrderStatus(o.id,'picked_up')} style={{width:'100%',background:'#8b5cf6',color:'white',border:'none',padding:'14px',borderRadius:'12px',fontWeight:'800',fontSize:'13px'}}>
                        📦 Взех поръчката от магазина<br/><span style={{fontSize:'10px',opacity:0.8}}>Взето от {shop?.name}, пътувам към клиента</span>
                      </button>
                    )}
                    {o.status==='picked_up' && (
                      <button onClick={()=>updateOrderStatus(o.id,'on_the_way')} style={{width:'100%',background:'#f59e0b',color:'white',border:'none',padding:'14px',borderRadius:'12px',fontWeight:'800',fontSize:'13px'}}>
                        🚚 Пътувам към клиента<br/><span style={{fontSize:'10px',opacity:0.8}}>Доставям на {o.address}</span>
                      </button>
                    )}
                    {(o.status==='on_the_way' || o.status==='picked_up') && (
                      <button onClick={()=>updateOrderStatus(o.id,'delivered')} style={{width:'100%',background:'#22c55e',color:'white',border:'none',padding:'14px',borderRadius:'12px',fontWeight:'800',fontSize:'14px'}}>
                        ✅ Завършена поръчка - Доставено<br/><span style={{fontSize:'10px',opacity:0.8}}>Клиентът получи поръчката</span>
                      </button>
                    )}
                    {o.status==='delivered' && (
                      <div>
                        <div style={{background:'#dcfce7',color:'#166534',padding:'12px',borderRadius:'10px',textAlign:'center',fontWeight:'800',marginBottom:'8px'}}>✅ Поръчката е завършена и доставена успешно</div>
                        {(()=>{
                          const remaining = (sortedByDistance.length>0 ? sortedByDistance : myShopOrders).filter((rem:any)=> rem.id!==o.id && (rem.status==='on_the_way' || rem.status==='picked_up' || rem.status==='on_the_way_to_shop'));
                          if(remaining.length>0){
                            const next = remaining[0];
                            const nextDist = (next as any)._distFromShop ? `${(next as any)._distFromShop.toFixed(1)}km` : '';
                            return (
                              <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}>
                                <div style={{fontSize:'12px',fontWeight:'800'}}>➡️ Продължи към следваща спирка ({remaining.length} остават):</div>
                                <div style={{fontSize:'11px',marginTop:'6px',background:'rgba(255,255,255,0.15)',padding:'8px',borderRadius:'8px'}}>
                                  <b>#{next.id.slice(0,8)} • {next.customer_name}</b><br/>
                                  📍 {next.address} {nextDist && `• ${nextDist} от магазина`}<br/>
                                  🛒 {(next.items||[]).map((it:any)=>`${it.name} x${it.qty}`).join(', ')}<br/>
                                  💰 {parseFloat(next.total).toFixed(2)}€
                                </div>
                                <div style={{display:'flex',gap:'8px',marginTop:'8px'}}>
                                  <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(next.address)}`} target="_blank" style={{flex:1,background:'#22c55e',color:'white',padding:'10px',borderRadius:'8px',textAlign:'center',textDecoration:'none',fontWeight:'800',fontSize:'12px'}}>🗺️ Навигирай до следващия клиент</a>
                                  <a href={`tel:${next.customer_phone}`} style={{background:'white',color:'#0F4C75',padding:'10px 14px',borderRadius:'8px',textAlign:'center',textDecoration:'none',fontWeight:'800'}}>📞 Обади се</a>
                                </div>
                              </div>
                            )
                          }else{
                            return <div style={{background:'#f0fdf4',padding:'10px',borderRadius:'10px',textAlign:'center',fontSize:'11px',color:'#166534',border:'1px solid #22c55e'}}>🎉 Всички поръчки доставени! Няма повече спирки.</div>
                          }
                        })()}
                      </div>
                    )}
            ...
            {(sortedByDistance.length>0 ? sortedByDistance : myShopOrders).length===0 && <div style={{textAlign:'center',padding:'20px',background:'white',borderRadius:'12px',color:'#666'}}>Нямаш доставки още. Като магазинара приеме поръчка, ще се появи тук в реално време.</div>}
          </div>
        )}
        ...
        {tab==='admin' && isAdmin && (
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            ...
            <div style={{background:'white',border:'2px solid #22c55e',borderRadius:'12px',padding:'12px'}}>
              ...
              <input value={shopName} onChange={e=>setShopName(e.target.value)} placeholder="Име: HALASTRA Две Могили" style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #ddd',marginTop:'8px'}}/>\n              <input value={shopAddress} onChange={e=>setShopAddress(e.target.value)} placeholder="📍 Адрес на магазина: ул. Вапцаров 2, Две Могили (за шофьора)" style={{width:'100%',padding:'10px',borderRadius:'8px',border:'2px solid #0F4C75',marginTop:'8px'}}/>\n              <input value={ownerPhone} onChange={e=>setOwnerPhone(e.target.value)} placeholder="Собственик тел: +447511230086" style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #ddd',marginTop:'8px'}}/>\n              <input value={driverPhone} onChange={e=>setDriverPhone(e.target.value)} placeholder="Шофьор тел (по желание)" Две Могили" style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #ddd',marginTop:'8px'}}/>
              <input value={ownerPhone} onChange={e=>setOwnerPhone(e.target.value)} placeholder="Собственик тел: +447511230086" style={{width:'100%',padding:'10px',borderRadius:'8px',border:'1px solid #ddd',marginTop:'8px'}}/>
