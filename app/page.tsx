        ...
        {tab==='driver' && (
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}>
              <div style={{display:'flex',justifyContent:'space-between'}}><div><b>🚚 Доставки - Точния флоу + ФИКС 2-ра поръчка</b><div style={{fontSize:'10px'}}>{driverOnline? '🟢 Онлайн' : '🔴 Офлайн'} • Взети: {myTakenOrders.length}/3 • Свободни: {availableFree.length} • Днес: {todayEarnings.toFixed(2)}€ от {todayDeliveredOrders.length}</div></div><button onClick={toggleDriverOnline} style={{background:driverOnline?'#22c55e':'#FF3B30',color:'white',border:'none',padding:'10px 14px',borderRadius:'12px',fontWeight:'800'}}>{driverOnline? '🟢' : '🔴'}</button></div>
            </div>

            {/* ВИНАГИ ВИДИМИ СВОБОДНИ - ЗА ДА ВЗЕМЕШ ВТОРАТА ОТ СЪЩИЯ МАГАЗИН */}
            {availableFree.length>0 && (
              <div style={{background:'white',border:'3px solid #FFD60A',borderRadius:'14px',padding:'12px'}}>
                <b>📥 Свободни заявки - вземи и 2-ра от същия магазин - {availableFree.length}</b>
                <div style={{fontSize:'10px',color:'#666'}}>Взети: {myTakenOrders.length}/3 - ако 2-те са от HALASTRA вземи и двете преди магазина</div>
                {availableFree.map((o:any)=>{
                  const shop = shops.find(s=>s.id===o.shop_id);
                  const sameShopAsTaken = myTakenOrders.some((t:any)=>t.shop_id===o.shop_id);
                  return (
                    <div key={o.id} style={{border: sameShopAsTaken? '3px solid #22c55e' : '2px solid #0F4C75',borderRadius:'12px',padding:'10px',marginTop:'10px',background: sameShopAsTaken? '#f0fdf4' : 'white'}}>
                      {sameShopAsTaken && <div style={{fontSize:'10px',background:'#22c55e',color:'white',padding:'4px 8px',borderRadius:'6px',fontWeight:'900',marginBottom:'6px'}}>⭐ СЪЩИЯ МАГАЗИН - ВЗЕМИ И ТАЗИ!</div>}
                      <div style={{display:'flex',justifyContent:'space-between'}}><b>#{o.id.slice(0,8)} • {parseFloat(o.total).toFixed(2)}€ • {o.customer_name}</b></div>
                      <div style={{fontSize:'11px',marginTop:'6px',background:'#e0f2fe',padding:'8px',borderRadius:'8px'}}>🏪 {shop?.name}<br/>📍 {shop?.address} - С НОМЕР • {shop?.city}</div>
                      <div style={{fontSize:'11px',marginTop:'6px',background:'#fff3cd',padding:'8px',borderRadius:'8px',border:'2px solid black',fontWeight:'800'}}>📍 КЛИЕНТ С НОМЕР: {o.address}</div>
                      <button onClick={()=>assignDriverToOrder(o.id)} style={{width:'100%',marginTop:'8px',background: sameShopAsTaken? '#22c55e' : '#0F4C75',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'900'}}>
                        ✅ Приеми #{o.id.slice(0,6)} {sameShopAsTaken? '- от същия магазин!' : ''} - {o.address.slice(0,25)}
                      </button>
                    </div>
                  )
                })}
