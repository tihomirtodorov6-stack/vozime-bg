...
export default function FinalAllPanels(){
  ...
  // ОПРАВЕНО ЗА PWA - ВЛИЗА СЛЕД РЕГИСТРАЦИЯ НА ВТОРИ ТЕЛ
  const handleAuth = async ()=>{
    const cPhone = clean(loginForm.phone);
    if(!loginForm.phone || cPhone.length<9){ alert('Телефон!'); return; }

    if(loginTab==='login'){
      // търсим САМО по clean_phone с limit(1) - не гърми при + и при дублиран
    ...
    try{
      const res = await fetch('/api/send-email',{
        ...
        body:JSON.stringify({email:loginForm.email.trim().toLowerCase(),code,firstName:loginForm.firstName,clean_phone:cPhone})
      });
      const j = await res.json().catch(()=>({}));
      if(!res.ok) throw new Error(j.error||'Грешка при пращане');
      setCodeForTest(j.codeForTesting||code);
    }catch(e:any){
      alert('Грешка при пращане на код: '+e.message);
  ...
  const handleVerifyCode = async ()=>{
    ...
    try{
      const res = await fetch('/api/verify-code',{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({email:pendingData.email,code:codeInput.trim()})
      });
      const result = await res.json();
      if(!result.valid){ alert(result.error||'Грешен код'); return; }
    }catch{
      alert('Грешка при проверка');
      return;
    }
...
.subscribe(); return ()=>{ supabase.removeChannel(channel); };
  ...
  if(!currentUser){
    if(verificationStep==='verify'){
      return (<main style={{position:'fixed',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'#0F4C75',padding:'16px'}}><div style={{background:'white',padding:'24px',borderRadius:'24px',width:'100%',maxWidth:'380px',textAlign:'center'}}><div style={{fontSize:'32px'}}>📧</div><b style={{fontSize:'18px'}}>Въведи кода</b><div style={{fontSize:'13px', marginTop:'8px', color:'#444', lineHeight:'1.4'}}>Изпратихме код на<br/><b>{pendingData?.email}</b>{codeForTest && <><br/><div style={{marginTop:'10px', background:'#f0fdf4', border:'2px dashed #22c55e', padding:'12px', borderRadius:'12px'}}><div style={{fontSize:'11px', color:'#666'}}>КОД ЗА ТЕСТ (докато оправим имейла):</div><div style={{fontSize:'28px', fontWeight:'900', letterSpacing:'6px', color:'#0F4C75', marginTop:'4px'}}>{codeForTest}</div></div></>}<br/><span style={{fontSize:'11px', color:'#888'}}>Валиден 10 мин. - Провери и SPAM.</span></div><input value={codeInput} onChange={e=>setCodeInput(e.target.value)} placeholder="000000" maxLength={6} style={{width:'100%',padding:'16px',borderRadius:'14px',border:'2px solid #0F4C75',marginTop:'16px',textAlign:'center',fontSize:'24px',letterSpacing:'6px',fontWeight:'800'}}/><button onClick={handleVerifyCode} style={{width:'100%',padding:'16px',background:'#22c55e',color:'white',border:'none',borderRadius:'14px',marginTop:'12px',fontWeight:'900',fontSize:'16px'}}>✅ Потвърди кода</button><button onClick={()=>{setVerificationStep('form'); setPendingData(null); setCodeInput('');}} style={{width:'100%',padding:'12px',background:'white',color:'#666',border:'1px solid #ddd',borderRadius:'12px',marginTop:'8px',fontSize:'13px'}}>← Назад</button></div></main>);
    }
        ...
        {tab==='driver' && (
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                ...
                {availableFree.map((o:any)=>{
                  ...
                  return (
                    <div key={o.id} style={{border: sameShop? '3px solid #22c55e' : '2px solid #0F4C75',borderRadius:'12px',padding:'10px',marginTop:'10px',background: sameShop? '#f0fdf4' : 'white'}}>
                      {sameShop && <div style={{fontSize:'10px',background:'#22c55e',color:'white',padding:'4px 8px',borderRadius:'6px',fontWeight:'900',marginBottom:'6px'}}>⭐ СЪЩИЯ МАГАЗИН - ВЗЕМИ И ТАЗИ!</div>}
                      ...
                      <div style={{fontSize:'11px',marginTop:'6px',background:'#e0f2fe',padding:'8px',borderRadius:'8px',border:'2px solid #0F4C75'}}><b>🏪 {shop?.name}</b><br/><span style={{fontWeight:'800',background:'white',padding:'4px',borderRadius:'4px',border:'1px solid black'}}>📍 {shop?.address} • {shop?.city}</span></div>
                      <div style={{fontSize:'11px',marginTop:'6px',background:'#fff3cd',padding:'8px',borderRadius:'8px',border:'2px solid #f59e0b',fontWeight:'800'}}>📍 КЛИЕНТ: {o.address}</div>
            ...
            {pendingShopOrders.length>0 && (
              <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                <div style={{background:'black',color:'#FFD60A',padding:'12px',borderRadius:'12px',border:'2px solid #FFD60A'}}><b>🏪 ФАЗА 1: КЪМ МАГАЗИНА - {pendingShopOrders.length} от {shopIdsForPickup.length} магазина</b></div>
                  ...
                  return (
                    <div key={shopId} style={{background:'white',border:'4px solid black',borderRadius:'16px',padding:'14px'}}>
                      ...
                      <div style={{marginTop:'8px',background:'#e0f2fe',border:'3px solid #0F4C75',padding:'12px',borderRadius:'10px'}}>
                        <div style={{fontSize:'10px',fontWeight:'900'}}>📍 АДРЕС НА МАГАЗИНА:</div>
                      ...
                      {!isArrived? (
                        <>
                          <button onClick={()=>openNavigation(shop?.address||'')} style={{width:'100%',marginTop:'12px',background:'black',color:'#FFD60A',border:'none',padding:'16px',borderRadius:'12px',fontWeight:'900',fontSize:'16px'}}>🧭 КЪМ МАГАЗИНА {shop?.name}</button>
                          <button onClick={()=>setArrivedShops(prev=>({...prev,[shopId]:true}))} style={{width:'100%',marginTop:'8px',background:'white',border:'3px solid black',padding:'14px',borderRadius:'12px',fontWeight:'900'}}>📍 ПРИСТИГНАХ ДО МАГАЗИНА</button>
                        </>
                      ) : (
                        <div style={{marginTop:'10px'}}>
                          <div style={{background:'#FFD60A',color:'black',padding:'10px',borderRadius:'10px',border:'2px solid black',fontWeight:'900',textAlign:'center'}}>📱 ПОКАЖИ #{shopOrders.map((o:any)=>o.id.slice(0,6)).join(', #')} НА МАГАЗИНЕРА</div>
                          {shopOrders.map((o:any)=>(
                            <div key={o.id} style={{border:'3px solid black',borderRadius:'12px',padding:'12px',marginTop:'10px',background:'#f9fafb'}}>
                              ...
                              <div style={{fontSize:'11px',marginTop:'6px'}}>👤 {o.customer_name} • {o.customer_phone}</div>
                              <div style={{display:'flex',gap:'6px',marginTop:'10px'}}>
                                <button onClick={()=>updateOrderStatus(o.id,'picked_up')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'14px',borderRadius:'10px',fontWeight:'900'}}>✅ ВЗЕХ #{o.id.slice(0,6)}</button>
            ...
            {pendingShopOrders.length===0 && pickedUpOrders.length>0 && (
              <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                <div style={{background:'black',color:'#FFD60A',padding:'12px',borderRadius:'12px',border:'2px solid #FFD60A',display:'flex',justifyContent:'space-between'}}>
                  <b>👤 ФАЗА 2: КЪМ КЛИЕНТИ - {pickedUpOrders.length}</b>
                    ...
                    <>
                      <div style={{background:'white',border:'4px solid #22c55e',borderRadius:'16px',padding:'14px'}}>
                        ...
                        <div style={{marginTop:'10px',background:'white',border:'3px solid #22c55e',padding:'12px',borderRadius:'10px'}}>
                          <div style={{fontSize:'10px',fontWeight:'900'}}>📍 АДРЕС КЛИЕНТА:</div>
                        ...
                        <button onClick={()=>openNavigation(activeClient.address)} style={{width:'100%',marginTop:'12px',background:'black',color:'#FFD60A',border:'none',padding:'16px',borderRadius:'12px',fontWeight:'900',fontSize:'16px'}}>🧭 НАВИГИРАЙ КЪМ КЛИЕНТА<br/><span style={{fontSize:'13px',background:'white',color:'black',padding:'6px 10px',borderRadius:'8px',marginTop:'6px',display:'inline-block'}}>{activeClient.address}</span></button>
                        ...
                        }} style={{width:'100%',marginTop:'8px',background:'#22c55e',color:'white',border:'none',padding:'18px',borderRadius:'14px',fontWeight:'900',fontSize:'18px'}}>✅ ЗАВЪРШИ #{activeClient.id.slice(0,6)}</button>
                      </div>
                    </>
                  );
                })()}
            ...
            {myTakenOrders.length===0 && availableFree.length===0 && (
              <div style={{background:'white',padding:'20px',borderRadius:'12px',textAlign:'center'}}><b>Няма активни доставки</b></div>
            )}
          </div>
        )}
        {tab==='admin' && isAdmin && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#FFD60A',padding:'14px',borderRadius:'14px',border:'2px solid black'}}><b>👑 Админ панел - ПЪЛЕН</b><div style={{fontSize:'11px'}}>Магазини: {shops.length} • Шофьори LIVE: {driverStatuses.filter((d:any)=>d.is_online).length} • Вход PWA оправен ✅</div></div>
            ...
            <div style={{background:'white',borderRadius:'14px',padding:'12px',border:'2px solid black'}}>
              <b>📋 Всички магазини - {shops.length} - с всички екстри</b>
              {shops.map((s:any)=>{
                ...
                return (
                  <div key={s.id} style={{border: s.vip_active===false? '3px solid #FF3B30' : '2px solid #22c55e', borderRadius:'12px', padding:'12px', marginTop:'10px', background: s.vip_active===false? '#fee2e2' : 'white'}}>
                    <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                      <b style={{fontSize:'14px'}}>🏪 {s.name} {s.vip_active===false? '🔴 Затворен' : '🟢 Отворен'}</b>
                      <span style={{fontSize:'10px', background: s.vip_active===false? '#FF3B30' : '#22c55e', color:'white', padding:'4px 8px', borderRadius:'8px', fontWeight:'900'}}>{s.delivery_fee}€</span>
                    </div>
                    <div style={{fontSize:'11px', marginTop:'6px', background:'#f3f4f6', padding:'8px', borderRadius:'8px'}}>
                      📍 {s.address} • {s.city}<br/>
                      👤 Собственик: {owner?.phone || s.phone || 'няма'}<br/>
                      🚚 Шофьор: {driver?.phone || 'няма'}
                    </div>
                    <div style={{display:'flex', gap:'6px', marginTop:'10px', flexWrap:'wrap'}}>
                      ...
                      <button onClick={()=>deleteShop(s.id)} style={{background:'#FF3B30', color:'white', border:'none', padding:'10px 12px', borderRadius:'8px', fontWeight:'800', fontSize:'11px'}}>🗑️ Изтрий</button>
                    </div>
                  </div>
                )
              })}
              {shops.length===0 && <div style={{textAlign:'center', padding:'20px', color:'#666'}}>Няма магазини - създай първия горе</div>}
            </div>

            <div style={{background:'white',borderRadius:'14px',padding:'12px',border:'1px solid #e5e7eb'}}>
              <b>🚚 Шофьори LIVE - {driverStatuses.length}</b>
              {driverStatuses.map((d:any)=>(
                <div key={d.clean_phone} style={{display:'flex', justifyContent:'space-between', padding:'8px', borderBottom:'1px solid #eee', fontSize:'11px'}}>
                  <span>📱 {d.phone} {d.is_online? '🟢 Онлайн' : '🔴 Офлайн'}</span>
                  <span>{d.earnings_today?.toFixed(2)}€ / {d.deliveries_today} доставки</span>
                </div>
              ))}
            </div>
          </div>
