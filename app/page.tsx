...
export default function Home(){
  ...
  const isAdmin = currentUser && currentUser.phone && isAdminPhone(currentUser.phone);
  ...
  const loadRides = async ()=>{
    try{
      // Махнат 48ч филтър - показва всички обяви от последните 30 дни
      const cutoff = Date.now() - 30*24*60*60*1000;
      const {data, error} = await supabase.from('rides').select('*').order('created_at',{ascending:false}).limit(200);
      if(error){ console.error('loadRides error', error); return; }
      if(data){
        // Филтрираме в паметта само ако има много стари
        const filtered = data.filter((d:any)=> !d.created_at || Number(d.created_at) > cutoff || new Date(d.created_at).getTime() > cutoff);
        const toShow = filtered.length>0 ? filtered : data;
        setRides(toShow.map((d:any)=>({id:d.id, driverName:d.driver_name, driverPhone:d.driver_phone, driverId:d.driver_id, from:d.from_city, to:d.to_city, fromCountry:d.from_country||'BG', toCountry:d.to_country||'BG', time:d.time, date:d.date, seats:d.seats, message:d.message, createdAt:d.created_at, type:d.type||'offer', carInfo:d.car_info, isDriverVerified:d.is_driver})));
      }
  ...
  const loadMyShopProfile = async ()=>{
    ...
    if(profErr){ setDebugLog('shop_profiles error: '+profErr.message); return; }
    if(!allProfiles || allProfiles.length===0){ setMyShopProfile(null); setDebugLog('Няма shop_profiles'); return; }
    let myProfiles:any[] = [];
    if(isAdminPhone(currentUser.phone)){
      myProfiles = allProfiles.filter((p:any)=> clean(p.phone)===myClean10 && cleanFull(p.phone)===myCleanFull && p.phone===currentUser.phone);
    } else {
      myProfiles = allProfiles.filter((p:any)=>{
        const pClean10 = clean(p.phone); const pCleanFull = cleanFull(p.phone);
    ...
    if(myProfiles.length===0){ setMyShopProfile(null); setMyShopProducts([]); setMyShopOrders([]); setDebugLog(`Няма профил за ${currentUser.phone} clean10=${myClean10}`); return; }
    const ownerProfile = myProfiles.find((p:any)=>p.role==='shop_owner') || myProfiles[0];
    setMyShopProfile(ownerProfile);
    setDebugLog(`Намерен профил shop_id=${ownerProfile.shop_id} role=${ownerProfile.role} от ${myProfiles.length} профила`);
    const shopIds = [...new Set(myProfiles.map((p:any)=>p.shop_id))];
    let allProds:any[] = []; let allOrders:any[] = [];
    for(const sid of shopIds){
      const {data:prods, error:pErr}=await supabase.from('products').select('*').eq('shop_id', sid);
      if(pErr){ setDebugLog(prev=> prev+` | products error ${sid}: ${pErr.message}`); } else if(prods) allProds = [...allProds, ...prods];
  ...
  const addProductWithImage = async ()=>{
    setDebugLog('Започвам добавяне...');
    if(!myShopProfile?.shop_id){ alert('Нямаш shop_id! Профила ти е: '+JSON.stringify(myShopProfile)); setDebugLog('Няма shop_id - myShopProfile е null или празен'); return; }
    if(myShopProfile.role!=='shop_owner'){ alert('Само собственика може! Твоя role е: '+myShopProfile.role); return; }
    ...
    if(isNaN(priceNum) || priceNum<=0){ alert('Грешна цена! Получих: '+cleanedPrice); return; }
    let imageUrl = editingProduct?.image_url || null;
    if(newProdFile){
      setDebugLog('Качвам снимка...');
      ...
      if(upErr){ alert('Грешка качване снимка: '+upErr.message); setDebugLog('Upload error: '+upErr.message); return; }
      const {data:urlData}=supabase.storage.from('product-images').getPublicUrl(fileName);
      imageUrl = urlData.publicUrl;
      setDebugLog('Снимка качена: '+imageUrl);
    }
    const baseData:any = {name:newProdName.trim(), price:priceNum, description:`[${newProdCat}] ${newProdDesc.trim()}`, image_url:imageUrl, active:newProdActive, shop_id:myShopProfile.shop_id};
    setDebugLog(`Опитвам insert: shop_id=${baseData.shop_id} name=${baseData.name} price=${baseData.price} active=${baseData.active}`);
    if(editingProduct){
      const {data, error}=await supabase.from('products').update({name:baseData.name, price:baseData.price, description:baseData.description, image_url:baseData.image_url, active:baseData.active}).eq('id', editingProduct.id).select().single();
      if(error){ alert('Грешка при UPDATE: '+error.message+'\nДетайли: '+JSON.stringify(error)); setDebugLog('UPDATE error: '+error.message); return; }
      setDebugLog('UPDATE успех: '+JSON.stringify(data));
    } else {
      const {data, error}=await supabase.from('products').insert(baseData).select().single();
      if(error){ 
        alert('ГРЕШКА при запис на продукт!\n'+error.message+'\n\nТова е заради RLS в Supabase. Пусни SQL-а който съм ти дал!\n\nДетайли: '+JSON.stringify(error)); 
        setDebugLog('INSERT error: '+error.message+' | '+JSON.stringify(error));
        console.error('INSERT products error', error);
        return; 
      }
      setDebugLog('INSERT успех! Продукт ID: '+data.id+' shop_id: '+data.shop_id);
    }
    ...
    if(selectedMarketShop && selectedMarketShop.id===myShopProfile.shop_id){ await loadMarketProducts(selectedMarketShop.id); }
    alert('✅ Продукт запазен успешно!');
  };
  ...
  const deleteProduct = async (id:string)=>{ if(myShopProfile?.role!=='shop_owner'){ alert('Само собственика!'); return; } if(!confirm('Изтрий?')) return; const {error}=await supabase.from('products').delete().eq('id',id); if(error){ alert('Грешка триене: '+error.message); return; } setMyShopProducts(myShopProducts.filter(p=>p.id!==id)); setMarketProducts(marketProducts.filter(p=>p.id!==id)); };
  const toggleProductActive = async (p:any)=>{ const newActive = !p.active; const {error}=await supabase.from('products').update({active:newActive}).eq('id', p.id); if(error){ alert('Грешка active: '+error.message); return; } setMyShopProducts(myShopProducts.map(x=> x.id===p.id ? {...x, active:newActive}: x)); setMarketProducts(marketProducts.map(x=> x.id===p.id ? {...x, active:newActive}: x)); };
  const loadMarketProducts = async (shopId:string)=>{ const {data, error}=await supabase.from('products').select('*').eq('shop_id', shopId); if(error){ alert('Грешка зареждане продукти: '+error.message); return; } if(data) setMarketProducts(data.filter((x:any)=> x.active!==false)); setSelectedMarketShop(marketShops.find(s=>s.id===shopId)); setMarketSearch(''); setMarketCat('Всички'); setCart([]); setCartOpen(false); };
  const addToCart = (product:any)=>{ setCart(prev=>{ const existing = prev.find(c=> c.product.id===product.id); if(existing) return prev.map(c=> c.product.id===product.id ? {...c, qty: c.qty+1} : c); return [...prev, {product, qty:1}]; }); };
  ...
  const placeOrder = async ()=>{
    ...
    if(error){ alert('Грешка поръчка: '+error.message); return; }
    alert('Поръчка изпратена!'); setCart([]); setCustomerAddr(''); setCartOpen(false); await loadCustomerOrders(); await loadMyShopProfile();
  };
  const updateOrderStatus = async (orderId:string, status:string)=>{ const {error}=await supabase.from('orders').update({status}).eq('id',orderId); if(error){ alert(error.message); return; } await loadMyShopProfile(); await loadCustomerOrders(); };
  ...
  // REALTIME - поръчки и продукти в реално време
  useEffect(()=>{
    if(!currentUser) return;
    const ordersChannel = supabase.channel('orders-realtime-'+currentUser.phone)
      ...
      .on('postgres_changes', {event:'*', schema:'public', table:'products'}, (payload)=>{
        ...
        loadMyShopProfile();
        if(selectedMarketShop) loadMarketProducts(selectedMarketShop.id);
        loadShops();
      })
      .subscribe();
    return ()=> { supabase.removeChannel(ordersChannel); };
  },[currentUser, selectedMarketShop?.id]);
  ...
  const filteredRides = (rides||[]).filter(r=>{ try{ if(filterText && !( (r.from||'').toLowerCase().includes(filterText.toLowerCase()) || (r.to||'').toLowerCase().includes(filterText.toLowerCase()) )) return false; if(filterType && r.type!==filterType) return false; return true; }catch{ return true; } });
  ...
  if(staticPage){
    ...
    return (
      <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'white',display:'flex',flexDirection:'column',fontFamily:'-apple-system, sans-serif'}}>
        ...
        <div style={{padding:'10px',borderTop:'1px solid #eee',textAlign:'center',fontSize:'11px'}}>Contact: dropoffpay.co.uk@gmail.com</div>
      </main>
    )
  }
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'white',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      <div style={{flexShrink:0}}>
        ...
        <div style={{height:'52px',display:'flex',gap:'4px',padding:'6px',background:'#f1f3f4'}}>
          ...
          {isAdmin && <button onClick={()=>setTab('admin')} style={{flex:1,borderRadius:'10px',border:'none',fontWeight:'bold',background:tab==='admin'?'#FFD60A':'#FF3B30',color:tab==='admin'?'black':'white',fontSize:'11px'}}>{t.admin}</button>}
        </div>
      </div>
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f9fafb'}} ref={ridesContainerRef}>
        ...
        {tab==='shops' && isAdmin && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
            ...
            {shops.map((s:any)=><div key={s.id} style={{background:'white',padding:'12px',borderRadius:'12px',border:'2px solid #22c55e'}}><b>{s.name}</b> - {s.delivery_fee}€ • {s.vip_active===false?'🔴':'🟢'} <button onClick={()=>toggleShopActive(s)} style={{marginLeft:'8px',background:s.vip_active===false?'#22c55e':'#FF3B30',color:'white',border:'none',padding:'4px 8px',borderRadius:'6px',fontSize:'10px'}}>{s.vip_active===false?'Пусни':'Спри'}</button></div>)}
          </div>
        )}
        {tab==='market' && (
          <div style={{padding:'10px',display:'flex',flexDirection:'column',gap:'10px'}}>
            {!selectedMarketShop? (
              <>
                <div style={{background:'#22c55e',color:'white',padding:'12px',borderRadius:'12px'}}><b>🛒 Пазар - Магазини (без Sofia)</b><div style={{fontSize:'11px'}}>VoziMe 0% - доставката е при магазина.</div></div>
              ...
              <>
                ...
                <input placeholder="Търси продукт..." value={marketSearch} onChange={e=>setMarketSearch(e.target.value)} style={{padding:'10px',borderRadius:'10px',border:'1px solid #22c55e'}}/>
                <div style={{display:'flex',gap:'6px',overflowX:'auto'}}>{['Всички',...PRODUCT_CATS].map(cat=><button key={cat} onClick={()=>setMarketCat(cat)} style={{whiteSpace:'nowrap',padding:'6px 12px',borderRadius:'20px',border:'none',fontSize:'11px',background:marketCat===cat?'#22c55e':'white',color:marketCat===cat?'white':'#666'}}>{cat}</button>)}</div>
                {marketProducts.filter((p:any)=>{ const cat = p.description?.match(/\[(.*?)\]/)?.[1] || 'Други'; const mCat = marketCat==='Всички' || cat===marketCat; const mSearch = !marketSearch || p.name.toLowerCase().includes(marketSearch.toLowerCase()); return mCat && mSearch; }).map((p:any)=>{
                  const inCart = cart.find(c=> c.product.id===p.id);
                  const cat = p.description?.match(/\[(.*?)\]/)?.[1] || 'Други';
                  return (<div key={p.id} style={{background:'white',borderRadius:'12px',padding:'10px',display:'flex',gap:'10px',border:'1px solid #eee'}}>{p.image_url && <img src={p.image_url} style={{width:'60px',height:'60px',objectFit:'cover',borderRadius:'8px'}}/>}<div style={{flex:1}}><b>{p.name}</b> <span style={{fontSize:'9px',background:'#f0fdf4',padding:'2px 6px',borderRadius:'8px'}}>{cat}</span><br/><span style={{fontSize:'11px'}}>{p.description?.replace(/\[.*?\]\s*/,'')}</span><br/><b>{p.price}€</b></div>{inCart ? <div style={{display:'flex',alignItems:'center',gap:'6px'}}><button onClick={()=>decCart(p.id)} style={{width:'28px',height:'28px',borderRadius:'50%',border:'1px solid #ddd'}}>−</button><b>{inCart.qty}</b><button onClick={()=>incCart(p.id)} style={{width:'28px',height:'28px',borderRadius:'50%',border:'none',background:'#22c55e',color:'white'}}>+</button></div> : <button onClick={()=>addToCart(p)} style={{background:'#22c55e',color:'white',border:'none',padding:'8px 12px',borderRadius:'8px'}}>Добави</button>}</div>)
                })}
                {cart.length>0 && <div style={{background:'white',border:'2px solid #22c55e',borderRadius:'12px',padding:'12px',position:'sticky',bottom:'0'}}><b>🛒 Количка {getCartCount()} - {getCartTotal().toFixed(2)}€</b>{cart.map((c:any)=><div key={c.product.id} style={{fontSize:'12px',display:'flex',justifyContent:'space-between'}}><span>{c.product.name} x{c.qty}</span><span>{(c.product.price*c.qty).toFixed(2)}€ <button onClick={()=>removeCartItem(c.product.id)} style={{border:'none',background:'#ffeaea',borderRadius:'4px'}}>X</button></span></div>)}<input value={customerAddr} onChange={e=>setCustomerAddr(e.target.value)} placeholder="Адрес" style={{width:'100%',padding:'10px',marginTop:'8px',borderRadius:'8px',border:'1px solid #ddd'}}/><button onClick={placeOrder} style={{width:'100%',marginTop:'8px',padding:'12px',background:'#22c55e',color:'white',border:'none',borderRadius:'8px',fontWeight:'bold'}}>Поръчай {getCartTotal().toFixed(2)}€</button></div>}
              </>
            )}
          </div>
        )}
          ...
          {myShopProfile && myShopProfile.role==='shop_owner' && !isAdmin && (
            <div style={{background:'white',border:'3px solid #22c55e',borderRadius:'14px',padding:'14px'}}>
              <div style={{background:'#22c55e',color:'white',padding:'10px',borderRadius:'10px',margin:'-14px -14px 12px -14px'}}>
                <div style={{fontWeight:'800',fontSize:'14px'}}>🛒 Моят магазин - {shops.find(s=>s.id===myShopProfile.shop_id)?.name||'Магазин'}</div>
                <div style={{fontSize:'10px'}}>ID: {myShopProfile.shop_id?.slice(0,8)} • Твоя тел: {currentUser.phone}</div>
                {debugLog && <div style={{fontSize:'9px',background:'rgba(0,0,0,0.2)',padding:'6px',borderRadius:'6px',marginTop:'6px',whiteSpace:'pre-wrap'}}>{debugLog}</div>}
              </div>
              <div style={{background:editingProduct?'#fef3c7':'#f0fdf4',border:`2px solid ${editingProduct?'#f59e0b':'#22c55e'}`,borderRadius:'12px',padding:'12px'}}>
                <div style={{fontWeight:'bold',fontSize:'12px'}}>{editingProduct?`✏️ Редактираш: ${editingProduct.name}`:'➕ Добави нова стока (ФИКСНАТО)'}</div>
                ...
                <div style={{display:'flex',gap:'8px',marginTop:'8px'}}>
                  <input value={newProdPrice} onChange={e=>setNewProdPrice(e.target.value)} placeholder="Цена €*: 6.25" style={{flex:1,padding:'10px',borderRadius:'8px',border:'2px solid #22c55e',background:'white',fontWeight:'bold'}}/>
                  <select value={newProdCat} onChange={e=>setNewProdCat(e.target.value)} style={{flex:1,padding:'10px',borderRadius:'8px',border:'1px solid #ddd'}}>{PRODUCT_CATS.map(c=><option key={c} value={c}>{c}</option>)}</select>
                </div>
                <input value={newProdDesc} onChange={e=>setNewProdDesc(e.target.value)} placeholder="Описание: 1кг прясно" style={{width:'100%',padding:'8px',borderRadius:'8px',border:'1px solid #ddd',marginTop:'8px'}}/>
                <label style={{display:'flex',gap:'6px',fontSize:'11px',marginTop:'8px'}}><input type="checkbox" checked={newProdActive} onChange={e=>setNewProdActive(e.target.checked)}/> Активен (видим за клиенти)</label>
                <input type="file" accept="image/*" onChange={e=>setNewProdFile(e.target.files?.[0]||null)} style={{width:'100%',marginTop:'8px'}}/>
                <div style={{display:'flex',gap:'8px',marginTop:'10px'}}>
                  <button onClick={addProductWithImage} style={{flex:1,padding:'12px',background:editingProduct?'#f59e0b':'#22c55e',color:'white',border:'none',borderRadius:'10px',fontWeight:'bold'}}>{editingProduct?'💾 Запази':'➕ Добави продукт'}</button>
              ...
              <div style={{marginTop:'14px'}}>
                ...
                {myShopProducts.length===0 && <div style={{fontSize:'12px',color:'#666',marginTop:'8px',background:'#ffeaea',padding:'10px',borderRadius:'8px'}}>⚠️ Няма продукти - това значи INSERT-а се блокира от RLS! Пусни SQL-а долу!</div>}
                {myShopProducts.map((p:any)=><div key={p.id} style={{display:'flex',gap:'10px',alignItems:'center',border:'1px solid #e5e7eb',padding:'10px',borderRadius:'10px',marginTop:'8px',background:p.active===false?'#ffeaea':'white'}}>{p.image_url && <img src={p.image_url} style={{width:'50px',height:'50px',objectFit:'cover',borderRadius:'8px'}}/>}<div style={{flex:1}}><b>{p.name}</b><br/><span style={{fontSize:'11px'}}>{p.description||''}</span><br/><b style={{color:'#22c55e'}}>{p.price}€ {p.active===false?'🔴 СКРИТ':'🟢'}</b></div><div style={{display:'flex',flexDirection:'column',gap:'4px'}}><button onClick={()=>toggleProductActive(p)} style={{background:p.active===false?'#22c55e':'#f3f4f6',border:'none',padding:'6px 10px',borderRadius:'6px',fontSize:'10px'}}>{p.active===false?'Пусни':'Скрий'}</button><button onClick={()=>startEditProduct(p)} style={{background:'#0F4C75',color:'white',border:'none',padding:'6px 10px',borderRadius:'6px',fontSize:'10px'}}>✏️</button><button onClick={()=>deleteProduct(p.id)} style={{background:'#FF3B30',color:'white',border:'none',padding:'4px 8px',borderRadius:'6px',fontSize:'10px'}}>🗑️</button></div></div>)}
              </div>
              <div style={{marginTop:'16px'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}}>
                  <b style={{fontSize:'14px'}}>📦 Поръчки в реално време - {myShopOrders.length}</b>
                  <span style={{fontSize:'10px',background:'#22c55e',color:'white',padding:'4px 8px',borderRadius:'12px'}}>🔴 LIVE</span>
                </div>
                <div style={{display:'flex',gap:'6px',overflowX:'auto',marginBottom:'10px'}}>
                  {[{k:'all',l:'Всички'},{k:'new',l:'🆕 Нови'},{k:'accepted',l:'✅ Приети'},{k:'preparing',l:'👨‍🍳 Готвят'},{k:'on_the_way',l:'🚚 На път'},{k:'delivered',l:'✅ Доставени'}].map(f=>
                    <button key={f.k} onClick={()=>setOrderStatusFilter(f.k)} style={{whiteSpace:'nowrap',padding:'8px 12px',borderRadius:'20px',border:'none',fontSize:'11px',fontWeight:'bold',background:orderStatusFilter===f.k?'#0F4C75':'white',color:orderStatusFilter===f.k?'white':'#666',borderWidth:'1px',borderStyle:'solid',borderColor:'#e5e7eb'}}>
                      {f.l} {f.k!=='all' ? `(${myShopOrders.filter(o=> o.status===f.k).length})` : ''}
                    </button>
                  )}
                </div>
                {myShopOrders.filter(o=> orderStatusFilter==='all' || o.status===orderStatusFilter).map((o:any)=>{
                  const statusColor = o.status==='new' ? '#FF3B30' : o.status==='accepted' ? '#22c55e' : o.status==='preparing' ? '#f59e0b' : o.status==='on_the_way' ? '#0F4C75' : '#22c55e';
                  const statusText = o.status==='new' ? '🆕 НОВА' : o.status==='accepted' ? '✅ ПРИЕТА' : o.status==='preparing' ? '👨‍🍳 ГОТВИ СЕ' : o.status==='on_the_way' ? '🚚 НА ПЪТ' : '✅ ДОСТАВЕНА';
                  return (
                    <div key={o.id} style={{background:'white',border:`2px solid ${statusColor}`,padding:'12px',borderRadius:'14px',marginBottom:'12px',boxShadow:'0 2px 8px rgba(0,0,0,0.06)'}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                        <b style={{fontSize:'13px'}}>{o.customer_name}</b>
                        <span style={{fontSize:'10px',background:statusColor,color:'white',padding:'4px 10px',borderRadius:'12px',fontWeight:'800'}}>{statusText}</span>
                      </div>
                      ...
                      <div style={{background:'#f9fafb',padding:'10px',borderRadius:'10px',marginTop:'10px'}}>
                        <div style={{fontSize:'11px',fontWeight:'800',marginBottom:'6px'}}>🛒 Поръчано:</div>
                        {(o.items||[]).map((it:any,i:number)=>(
                          <div key={i} style={{display:'flex',justifyContent:'space-between',fontSize:'12px',padding:'6px 0',borderBottom:i<(o.items.length-1)?'1px solid #e5e7eb':'none'}}>
                            <span><b>{it.name}</b> x{it.qty||1}</span>
                            <span>{(parseFloat(it.price)*(it.qty||1)).toFixed(2)}€</span>
                          </div>
                        ))}
                        <div style={{display:'flex',justifyContent:'space-between',fontSize:'12px',marginTop:'8px',paddingTop:'8px',borderTop:'2px solid #e5e7eb',fontWeight:'800'}}>
                          <span>Общо с доставка:</span>
                          <span style={{color:'#0F4C75',fontSize:'14px'}}>{o.total}€</span>
                        </div>
                      </div>
                      <div style={{display:'flex',gap:'8px',marginTop:'12px'}}>
                        ...
                        {o.status==='new' && <button onClick={()=>updateOrderStatus(o.id,'accepted')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800',fontSize:'12px'}}>✅ Приеми</button>}
                        {o.status==='accepted' && <button onClick={()=>updateOrderStatus(o.id,'preparing')} style={{flex:1,background:'#f59e0b',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800',fontSize:'12px'}}>👨‍🍳 Готвя</button>}
                        {o.status==='preparing' && <button onClick={()=>updateOrderStatus(o.id,'on_the_way')} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800',fontSize:'12px'}}>🚚 На път</button>}
                        {o.status==='on_the_way' && <button onClick={()=>updateOrderStatus(o.id,'delivered')} style={{flex:1,background:'#22c55e',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800',fontSize:'12px'}}>✅ Доставено</button>}
                      </div>
                    </div>
                  )
                })}
                {myShopOrders.filter(o=> orderStatusFilter==='all' || o.status===orderStatusFilter).length===0 && <div style={{textAlign:'center',padding:'20px',color:'#666',fontSize:'12px',background:'white',borderRadius:'12px'}}>Няма поръчки в тази категория</div>}
              </div>
            </div>
          )}
          {isAdmin && <div style={{background:'white',border:'2px solid #0F4C75',borderRadius:'12px',padding:'12px',textAlign:'center',fontSize:'12px'}}><b>👑 Ти си админ</b><br/>Не виждаш цени. Отиди в Магазини.</div>}
          <div style={{background:'white',border:'2px solid #22c55e',borderRadius:'14px',padding:'14px'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <b style={{fontSize:'14px'}}>🛒 Моите поръчки като клиент - {customerOrders.length}</b>
              <span style={{fontSize:'9px',background:'#22c55e',color:'white',padding:'3px 8px',borderRadius:'10px'}}>🔴 LIVE</span>
            </div>
            {customerOrders.map((o:any)=>{
              const statusColor = o.status==='new' ? '#FF3B30' : o.status==='accepted' ? '#22c55e' : o.status==='preparing' ? '#f59e0b' : o.status==='on_the_way' ? '#0F4C75' : '#22c55e';
              const statusText = o.status==='new' ? '🆕 Изпратена' : o.status==='accepted' ? '✅ Приета от магазина' : o.status==='preparing' ? '👨‍🍳 Приготвя се' : o.status==='on_the_way' ? '🚚 На път към теб' : '✅ Доставена';
              return (
                <div key={o.id} style={{background:'#f9fafb',border:`2px solid ${statusColor}`,padding:'12px',borderRadius:'12px',marginTop:'10px'}}>
                  <div style={{display:'flex',justifyContent:'space-between'}}>
                    <b style={{fontSize:'12px'}}>{shops.find(s=>s.id===o.shop_id)?.name||'Магазин'} • {o.total}€</b>
                    <span style={{fontSize:'10px',background:statusColor,color:'white',padding:'4px 8px',borderRadius:'10px',fontWeight:'bold'}}>{statusText}</span>
                  </div>
                  <div style={{fontSize:'11px',marginTop:'6px',color:'#666'}}>{new Date(o.created_at).toLocaleString('bg-BG')} • #{o.id.slice(0,8)} • 📍 {o.address}</div>
                  <div style={{background:'white',padding:'8px',borderRadius:'8px',marginTop:'8px',fontSize:'11px'}}>
                    {(o.items||[]).map((it:any,i:number)=><div key={i} style={{display:'flex',justifyContent:'space-between',padding:'4px 0',borderBottom:i<(o.items.length-1)?'1px solid #f3f4f6':'none'}}><span>{it.name} x{it.qty}</span><span>{(it.price*it.qty).toFixed(2)}€</span></div>)}
                  </div>
                  <div style={{marginTop:'8px',display:'flex',gap:'4px'}}>
                    {['new','accepted','preparing','on_the_way','delivered'].map((st,idx)=>{
                      const isDone = ['new','accepted','preparing','on_the_way','delivered'].indexOf(o.status) >= idx;
                      return <div key={st} style={{flex:1,height:'6px',borderRadius:'3px',background:isDone?statusColor:'#e5e7eb'}}></div>
                    })}
                  </div>
                </div>
              )
            })}
            {customerOrders.length===0 && <div style={{fontSize:'11px',color:'#666',marginTop:'8px'}}>Нямаш поръчки още - отиди в Пазар</div>}
          </div>
        </div>}
      </div>
    </main>
