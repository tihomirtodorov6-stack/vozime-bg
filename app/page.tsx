...
export default function ShopsFinal(){
  ...
  const startEditShop = (shop:any)=>{
    ...
    showToast(`✏️ Редактираш ${shop.name}`);
  };
  ...
  const updateMyShop = async ()=>{
    ...
    const {error}=await supabase.from('shops').update({ name: shopName, address: shopAddress, city: shopCity||shopAddress, delivery_fee: parseFloat(shopDeliveryFee)||4.99, phone: ownerPhone||myShopProfile.phone }).eq('id',myShopProfile.shop_id);
    if(error){ alert(error.message); return; }
  ...
  const updateOrderStatus = async (orderId:string,status:string)=>{
    ...
    // не чакаме load - realtime ще го обнови, но за сигурност оптимистично:
  ...
  // === НОВ REALTIME - БЕЗ РЕФРЕШ, ДИРЕКТЕН ЪПДЕЙТ ===
  useEffect(()=>{
    if(!currentUser?.phone) return;
    ...
    const channel = supabase.channel(channelName)
     .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, (payload) => {
        ...
        // 1. Ако съм клиент - обнови моите поръчки
        if(row.customer_phone && clean(row.customer_phone) === myPhoneClean){
          if(payload.eventType === 'INSERT'){
            setCustomerOrders(prev => [row,...prev.filter((o:any)=>o.id!==row.id)]);
          } else if(payload.eventType === 'UPDATE'){
            setCustomerOrders(prev => prev.map((o:any)=> o.id===row.id? {...o,...newRow} : o));
            showToast(`📦 Поръчка #${row.id.slice(0,4)} е ${newRow.status}`);
          } else if(payload.eventType === 'DELETE'){
            setCustomerOrders(prev => prev.filter((o:any)=>o.id!==row.id));
          }
        }

        // 2. Ако съм магазинер / шофьор - обнови магазинските поръчки
        setMyShopOrders(prev => {
          const myShopIds = [...new Set(shopProfiles
           .filter((p:any)=>{
              const p10=clean(p.phone); const pFull=cleanFull(p.phone);
              return p10===myPhoneClean || pFull===cleanFull(currentUser.phone) || p.phone===currentUser.phone;
            })
           .map((p:any)=>p.shop_id)
          )];

          const isRelevant =
            myShopIds.includes(row.shop_id) ||
            row.driver_requested === true ||
            prev.some((o:any)=>o.id===row.id) ||
            (myShopProfile as any)?.is_admin_view;

          if(!isRelevant) return prev;

          if(payload.eventType === 'INSERT'){
            if(!prev.some((o:any)=>o.id===row.id)){
              showToast(`🔔 Нова поръчка ${row.id.slice(0,4)}!`);
              return [row,...prev];
            }
            return prev;
          }
          if(payload.eventType === 'UPDATE'){
            // ако е нова заявка за шофьор която досега не съм виждал
            if(!prev.some((o:any)=>o.id===row.id) && row.driver_requested){
              showToast(`🚚 Нова заявка за шофьор!`);
              return [row,...prev];
            }
            return prev.map((o:any)=> o.id===row.id? {...o,...newRow} : o);
          }
          if(payload.eventType === 'DELETE'){
            return prev.filter((o:any)=>o.id!==row.id);
          }
          return prev;
        });
     ...
     .on('postgres_changes', { event: '*', schema: 'public', table: 'products' }, (payload) => {
        ...
        if(payload.eventType === 'INSERT' || payload.eventType === 'UPDATE'){
          setMyShopProducts(prev => {
            const exists = prev.some((p:any)=>p.id===newRow.id);
            if(exists) return prev.map((p:any)=> p.id===newRow.id? newRow : p);
            return [newRow,...prev];
          });
          if(selectedShop && newRow.shop_id === selectedShop.id){
            setMarketProducts(prev => {
              const exists = prev.some((p:any)=>p.id===newRow.id);
              if(newRow.active===false) return prev.filter((p:any)=>p.id!==newRow.id);
              if(exists) return prev.map((p:any)=> p.id===newRow.id? newRow : p);
              return [...prev, newRow];
            });
          }
      ...
      <div style={{flex:1,overflowY:'auto',padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
        ...
        {tab==='my_orders' && (
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}><b>📦 Моите поръчки LIVE - {customerOrders.length}</b><div style={{fontSize:'10px'}}>Обновява се само - без рефреш</div></div>
            ...
            {availableDriverOrders.map((o:any)=>{
              ...
              return (
                <div key={o.id} style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}>
                  ...
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#e0f2fe',padding:'8px',borderRadius:'8px'}}><b>🏪 {shop?.name}</b><br/>📍 {shop?.address}<br/><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(shop?.address||'')}`} target="_blank" style={{color:'#0F4C75',fontWeight:'bold'}}>🗺️ До магазина</a></div>
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#fff3cd',padding:'8px',borderRadius:'8px'}}><b>👤 {o.customer_name}</b> • {o.customer_phone}<br/>📍 {o.address}</div>
                  <div style={{display:'flex',gap:'8px',marginTop:'10px'}}>
                    ...
                    {o.driver_phone===currentUser.phone && o.status==='on_the_way_to_shop' && <button onClick={()=>updateOrderStatus(o.id,'picked_up')} style={{flex:1,background:'#8b5cf6',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}}>📦 Взех</button>}
