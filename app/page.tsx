...
export default function ShopsFinal(){
  ...
  const [myShopProfiles,setMyShopProfiles]=useState<any[]>([]); // само моите профили
  ...
  const isDriver = myShopProfiles.some((p:any)=>p.role==='driver');
  ...
  const handleAuth = async ()=>{
    ...
    if(codeErr){ alert('Грешка: '+codeErr.message); setIsSendingCode(false); return; }
    ...
    showToast(`Код на ${loginForm.email}`);
  };
  ...
  const loadMyShopProfile = async ()=>{
    ...
    if(!allProfiles){ setMyShopProfile(null); setMyShopProfiles([]); setMyShopProducts([]); setMyShopOrders([]); return; }
    ...
    setMyShopProfiles(myProfiles);

    // АДМИН БЕЗ ПРОФИЛ - НЕ Е МАГАЗИНЕР, НЕ ЗАРЕЖДАМЕ НИЩО КАТО "МОЯ"
    ...
    const ownerProfile = myProfiles.find((p:any)=>p.role==='shop_owner');
    setMyShopProfile(ownerProfile || null);

    // Зареждаме продукти и поръчки за ВСИЧКИ магазини на които съм собственик ИЛИ шофьор - за да има доставки
  ...
  const addProductWithImage = async ()=>{
    if(!isShopOwner){ alert('⛔ Само собственик може да пипа цени! Ти си админ.'); return; }
  ...
  const startEditProduct = (p:any)=>{ if(!isShopOwner){ alert('Админа не пипа цени!'); return; } setEditingProduct(p); setNewProdName(p.name); setNewProdPrice(p.price.toString()); setNewProdDesc((p.description||'').replace(/^\[.*?\]\s*/,'').trim()); const m=p.description?.match(/^\[(.*?)\]/); if(m) setNewProdCat(m[1]); setNewProdActive(p.active!==false); };
  const cancelEditProduct = ()=>{ setEditingProduct(null); setNewProdName(''); setNewProdPrice(''); setNewProdDesc(''); setNewProdFile(null); };
  const deleteProduct = async (id:string)=>{ if(!isShopOwner){ alert('Админа не трие!'); return; } if(!confirm('Изтрий?')) return; await supabase.from('products').delete().eq('id',id); setMyShopProducts(myShopProducts.filter(p=>p.id!==id)); };
  ...
  const sortByDistance = async ()=>{
    if(myShopOrders.length<=1) return; setIsSorting(true);
    try{
      const byShop: any = {}; myShopOrders.forEach((o:any)=>{ if(!byShop[o.shop_id]) byShop[o.shop_id]=[]; byShop[o.shop_id].push(o); });
      let allSorted:any[]=[]; for(const shopId in byShop){ const shop = shops.find(s=>s.id===shopId); const shopAddr = shop?.address || shop?.city || ''; const shopCoords = await geocodeAddress(shopAddr); if(!shopCoords){ allSorted=[...allSorted,...byShop[shopId]]; continue; } const withDist = await Promise.all(byShop[shopId].map(async (o:any)=>{ const custCoords = await geocodeAddress(o.address); const dist = custCoords ? calcDistance(shopCoords,custCoords) : 9999; return {...o,_dist:dist}; })); withDist.sort((a,b)=>a._dist-b._dist); allSorted=[...allSorted,...withDist]; }
      setSortedOrders(allSorted);
    }catch{ setSortedOrders(myShopOrders); } setIsSorting(false);
  };

  useEffect(()=>{ const cu=localStorage.getItem('vozime_current'); if(cu) setCurrentUser(JSON.parse(cu)); loadShops(); },[]);
  useEffect(()=>{ if(currentUser){ loadMyShopProfile(); loadCustomerOrders(); if(isAdmin && !isShopOwner && !isDriver) setTab('admin'); } },[currentUser, shops.length]);
  useEffect(()=>{
        ...
        <div style={{background:'white',padding:'20px',borderRadius:'24px',width:'100%',maxWidth:'380px'}}>
          ...
          {verificationStep==='form' ? (
            <>
              ...
              <button onClick={handleAuth} disabled={isSendingCode} style={{width:'100%',padding:'16px',background:'#22c55e',border:'none',borderRadius:'14px',fontWeight:'bold',color:'white'}}>{isSendingCode?'Пращам...':loginTab==='register'?'Изпрати код →':'Влез →'}</button>
            </>
          ) : (
            <>
              ...
              {codeForTest && (<div style={{background:'#fef3c7',border:'2px solid #f59e0b',borderRadius:'12px',padding:'12px',textAlign:'center',marginBottom:'12px'}}><div style={{fontSize:'10px'}}>КОД:</div><div style={{fontSize:'24px',fontWeight:'900'}}>{codeForTest}</div></div>)}
  ...
  return (
    <main style={{height:'100dvh',width:'100%',maxWidth:'480px',margin:'0 auto',background:'#f9fafb',display:'flex',flexDirection:'column',overflow:'hidden',fontFamily:'-apple-system, sans-serif'}}>
      <header style={{background:'#0F4C75',color:'white',padding:'10px 12px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div><b>🛒 Магазини {isAdmin && !isShopOwner ? '👑 АДМИН':''}</b><div style={{fontSize:'10px',opacity:0.8}}>{currentUser.firstName} • {currentUser.phone} {isAdmin?'👑':''} {isShopOwner?'🏪':''} {isDriver?'🚚':''}</div></div>
      ...
      <div style={{display:'flex',gap:'4px',padding:'6px',background:'white',borderBottom:'1px solid #e5e7eb'}}>
        ...
        {isDriver && <button onClick={()=>setTab('driver')} style={{flex:1,padding:'10px',borderRadius:'10px',border:'none',fontWeight:'bold',fontSize:'11px',background:tab==='driver'?'#0F4C75':'#f3f4f6',color:tab==='driver'?'white':'#666'}}>🚚 Доставки {driverOrders.length>0?`(${driverOrders.length})`:''}</button>}
      ...
      <div style={{flex:1,overflowY:'auto',padding:'10px',display:'flex',flexDirection:'column',gap:'12px'}}>
          ...
          <>
            {!selectedShop ? (
              ...
              <>
                ...
                <div style={{background:'#0F4C75',color:'white',padding:'14px',borderRadius:'14px'}}><b>{selectedShop.name}</b><div style={{fontSize:'11px'}}>Доставка {selectedShop.delivery_fee}€ • {selectedShop.address}</div></div>
                ...
                {cart.length>0 && (
                  <div style={{background:'white',border:'3px solid #22c55e',borderRadius:'16px',padding:'14px',position:'sticky',bottom:'0'}}>
                    <b>🛒 {getCartCount()} продукта</b>
                    <input value={customerAddr} onChange={e=>setCustomerAddr(e.target.value)} placeholder="📍 Адрес..." style={{width:'100%',padding:'12px',marginTop:'10px',borderRadius:'10px',border:'2px solid #22c55e'}}/>
                    <button onClick={placeOrder} style={{width:'100%',marginTop:'10px',padding:'14px',background:'#22c55e',color:'white',border:'none',borderRadius:'12px',fontWeight:'800'}}>✅ Поръчай {getCartTotal().toFixed(2)}€</button>
                  </div>
                )}
              </>
            )}
        ...
        {tab==='my_shop' && isShopOwner && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#22c55e',color:'white',padding:'14px',borderRadius:'14px'}}><b>🏪 Моят магазин - {shops.find(s=>s.id===myShopProfile?.shop_id)?.name||''}</b></div>
            ...
            <div style={{background:'white',border:'2px solid #22c55e',borderRadius:'14px',padding:'12px'}}>
              ...
              {myShopOrders.map((o:any)=>(
                <div key={o.id} style={{border:'2px solid #0F4C75',borderRadius:'12px',padding:'10px',marginTop:'10px'}}>
                  ...
                  <div style={{fontSize:'11px',marginTop:'4px'}}>{o.customer_name} • 📍 {o.address}</div>
                  <div style={{display:'flex',gap:'6px',marginTop:'8px'}}>
                    ...
                    {(o.status==='preparing' || o.status==='accepted') && !o.driver_requested && <button onClick={()=>requestDriverForOrder(o.id)} style={{flex:1,background:'#0F4C75',color:'white',border:'none',padding:'10px',borderRadius:'8px',fontWeight:'bold'}}>🚚 Поискай шофьор</button>}
                    {o.driver_requested && <div style={{flex:1,background:'#e0f2fe',padding:'10px',borderRadius:'8px',textAlign:'center',fontSize:'11px',fontWeight:'bold'}}>🚚 Поискан: {o.driver_phone||'чака...'}</div>}
                  </div>
                </div>
              ))}
            </div>
        ...
        {tab==='driver' && (
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            <div style={{background:'#0F4C75',color:'white',padding:'12px',borderRadius:'12px'}}>
              <div style={{display:'flex',justifyContent:'space-between'}}><b>🚚 Доставки LIVE - {driverOrders.length}</b><span style={{fontSize:'10px',background:'#22c55e',padding:'4px 8px',borderRadius:'10px'}}>LIVE {isDriver?'✅ Шофьор':''}</span></div>
              <div style={{background:activeDriverCount>=3?'#fee2e2':'#f0fdf4',border:'1px solid #fff',padding:'8px',borderRadius:'8px',marginTop:'8px',color:activeDriverCount>=3?'#991b1b':'#166534'}}>
                ...
                <div style={{fontSize:'10px'}}>{activeDriverCount>=3?'⛔ Лимит! Завърши поръчка.':`Можеш още ${3-activeDriverCount}`}</div>
              </div>
              {myShopOrders.length>1 && <button onClick={sortByDistance} disabled={isSorting} style={{width:'100%',marginTop:'8px',background:'#22c55e',color:'white',border:'none',padding:'10px',borderRadius:'10px',fontWeight:'bold'}}>{isSorting?'⏳...':'📍 Сортирай по близост'}</button>}
            </div>
            ...
            {availableDriverOrders.map((o:any)=>{
              ...
              return (
                <div key={o.id} style={{background:'white',border:'2px solid #0F4C75',borderRadius:'14px',padding:'12px'}}>
                  ...
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#e0f2fe',padding:'8px',borderRadius:'8px'}}><b>🏪 {shop?.name}</b> 📍 {shop?.address}<br/>📞 {shop?.phone}<br/><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(shop?.address||'')}`} target="_blank" style={{color:'#0F4C75',fontWeight:'bold'}}>🗺️ До магазина</a></div>
                  <div style={{fontSize:'11px',marginTop:'6px',background:'#fff3cd',padding:'8px',borderRadius:'8px'}}><b>👤 {o.customer_name}</b> 📍 {o.address}<br/><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(o.address)}`} target="_blank" style={{color:'#b45309',fontWeight:'bold'}}>🗺️ До клиента</a></div>
                  <div style={{background:'#f9fafb',padding:'6px',borderRadius:'8px',marginTop:'6px',fontSize:'11px'}}>{(o.items||[]).map((it:any)=><div key={it.id}>{it.name} x{it.qty}</div>)}</div>
                  <div style={{display:'flex',gap:'8px',marginTop:'10px'}}>
                    {!o.driver_phone && <><button onClick={()=>assignDriverToOrder(o.id)} style={{flex:1,background:activeDriverCount>=3?'#9ca3af':'#0F4C75',color:'white',border:'none',padding:'12px',borderRadius:'10px',fontWeight:'800'}} disabled={activeDriverCount>=3}>🚚 Вземи</button><button onClick={()=>refuseOrder(o.id)} style={{background:'#f3f4f6',border:'1px solid #ddd',padding:'12px',borderRadius:'10px'}}>❌</button></>}
                  ...
                  {o.status==='delivered' && <div style={{background:'#dcfce7',padding:'10px',borderRadius:'8px',textAlign:'center',fontWeight:'800',color:'#166534',marginTop:'10px'}}>✅ Завършена</div>}
                </div>
              )
            })}
            {availableDriverOrders.length===0 && <div style={{textAlign:'center',padding:'20px',background:'white',borderRadius:'12px',color:'#666'}}>Няма поръчки с поискан шофьор. Магазинара трябва да натисне "Поискай шофьор" в Моя таб.</div>}
          </div>
        )}

        {tab==='admin' && isAdmin && (
          <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
            <div style={{background:'#FFD60A',padding:'12px',borderRadius:'12px'}}><b>👑 Админ - само магазини, не пипаш цени</b><div style={{fontSize:'11px'}}>Създаваш магазин, пускаш/спираш. Цените ги пипа собственика от "Моя".</div></div>
